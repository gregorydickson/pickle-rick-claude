 ...
 * one.
 */
function enclosesProtectedPath(filePath, target) {
    if (!filePath || !target)
        return false;
    ...
    if (parts.length >= targetParts.length || parts.length * 2 < targetParts.length)
        return false;
 ...
 */
function touchesRepoGitDir(word, repoGitDir) {
 ...
 */
function detectGitDirWriteTarget(input, repoGitDir) {
    const toolName = input.tool_name || '';
    const filePath = input.tool_input?.file_path || '';
    const command = input.tool_input?.command || '';
    if ((toolName === 'Write' || toolName === 'Edit') && filePath) {
