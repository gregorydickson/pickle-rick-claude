 ...
 */
function killChildGroupOnTimeout(child, kill) {
    const pid = child.pid;
 ...
 */
function attachChildOutputDrain(child) {
    let stdout = '';
 ...
 */
function classifyChildClose(code, stdout) {
    if (code !== 0) {
        return { status: 'failed', reason: `shim-exit-${code ?? 'null'}` };
    }
    try {
 ...
 * settles the promise.
 */
function writeBatchToChild(child, input) {
