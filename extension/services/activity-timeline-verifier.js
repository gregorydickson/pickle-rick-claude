 ...
 */
export function unspawnedTerminalTickets(activity) {
    const spawned = collectSpawnMsByTicket(activity);
    const orphaned = new Set();
    for (const e of activity) {
        if (!WORKER_TERMINAL_EVENTS.has(e.event))
            continue;
        const ticket = getEventTicketId(e);
        if (ticket !== null && !spawned.has(ticket))
            orphaned.add(ticket);
    }
 ...
 */
function currentRunEvents(ticket, spawnMsList, activity) {
    if (spawnMsList.length === 0)
        return [];
    const latestSpawnMs = spawnMsList[spawnMsList.length - 1];
    return activity.filter((e) => {
        if (getEventTicketId(e) !== ticket)
            return false;
        const ms = parseTs(e);
...
function classifyTicketGate(ticket, spawnMsList, activity, narrowTierVacuity) {
    ...
    const gateFailedEvents = runEvents.filter((e) => e.event === GATE_FAILED_EVENT && e.gate_phase === 'test:fast');
