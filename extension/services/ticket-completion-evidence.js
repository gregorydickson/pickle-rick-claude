...
function readInferredArm(ctx, content, onUnmeasured) {
    ...
    process.stderr.write(`[ticket-completion-evidence] inferred sha ${inferredField} unreachable — falling through to scan attribution (R-AICF)\n`);
    return null;
}
/**
 ...
 */
function hasLifecycleArtifacts(ctx, onCouldNotMeasure) {
    const tPath = resolveTicketPath(ctx);
    if (!tPath)
        return false;
    try {
