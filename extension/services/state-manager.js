 ...
 */
export function schemaVersionDeployDriftMessage() {
    try {
...
export class SchemaVersionAheadError extends StateError {
    ...
    statePath;
    callerPid;
    constructor(statePath, writtenValue, maxSupported) {
        super('SCHEMA_MISMATCH', `[state-manager] FATAL: refusing to write state ${statePath} with ` +
 ...
 */
function emitSchemaVersionViolationActivity(statePath, writtenValue, maxSupported) {
    try {
        const ts = new Date();
        ...
        try {
            process.stderr.write(`[state-manager] failed to log state_write_schema_version_violation: ${safeErrorMessage(err)}\n`);
        }
        catch { /* stderr closed */ }
    }
}
 ...
 */
function assertSchemaVersionWithinCeiling(statePath, state, opts) {
    if (opts && opts._internalSchemaBump === true)
        return;
 ...
 */
function reportUninspectableLock(lockPath, err) {
...
export function releaseLockFile(lockPath, handle) {
    try {
...
export function stealLockFile(lockPath, snapshot) {
    ...
    try {
        if (!stolen || !sameLock(stolen, snapshot))
            return false; // not what we judged — never touched
        fs.unlinkSync(lockPath);
        return true;
    }
    catch {
 ...
 */
export function reclaimDeadLock(lockPath) {
 ...
 */
function isExemptActivityEvent(name) {
 ...
 */
function trimActivityRing(state) {
    const activity = state.activity;
    if (!Array.isArray(activity) || activity.length <= ACTIVITY_RING_MAX)
        return;
...
function normalizeUpToVersion(state, schemaVersion) {
    ...
    if (schemaVersion >= 5)
        normalizeV5StateDefaults(state);
}
 ...
 */
function adoptPromotedSnapshot(target, promoted) {
    const bag = target;
...
function isRecoverableStateSnapshotCandidate(value, maxSupportedSchemaVersion) {
    if (!isRecord(value))
        return false;
    if (REQUIRED_SNAPSHOT_STRING_FIELDS.some((field) => typeof value[field] !== 'string'))
        return false;
    if (!(typeof value.step === 'string' || value.step === null))
        return false;
    if (REQUIRED_SNAPSHOT_NUMERIC_FIELDS.some((field) => !Number.isFinite(Number(value[field]))))
        return false;
...
export class StateManager {
    ...
    parseStateOrRecover(statePath, raw) {
        let state;
        try {
     ...
     */
    recoverCorruptBaseOrThrow(statePath, message) {
    ...
    // -----------------------------------------------------------------------
    forceWrite(statePath, state, opts) {
        ...
        try {
            ...
            if (err instanceof SchemaVersionAheadError)
                throw err;
     ...
     */
    demotePhantomSession(statePath, state) {
     ...
     */
    demotePausedOrphanSession(statePath, state, preMigrationMtimeMs) {
    ...
    demoteDeadProcessSession(statePath, state) {
    ...
    recoverStaleActiveFlag(statePath, state, preMigrationMtimeMs = 0) {
        ...
        if (this.demotePhantomSession(statePath, state))
            return;
 ...
 */
export function graduationDecision(counts) {
    if (counts.ticketCount <= 0) {
 ...
 */
export function finalizeIfTrulyComplete(statePath, scan, opts = {}) {
    let counts;
    try {
        counts = scan();
    }
 ...
 */
function emitCompletionFinalizeRefused(gatePayload) {
    try {
        const ts = new Date();
        const activityDir = path.join(getDataRoot(), 'activity');
        fs.mkdirSync(activityDir, { recursive: true });
