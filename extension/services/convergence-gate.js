...
import { LockError, UNBOUNDED_READ_MAX_BUFFER, enumerationCompleted } from '../types/index.js';
 ...
 */
function readUsableBaseline(baselinePath) {
...
export function assertBaselineFresh(baselinePath, opts) {
    ...
    if (!(iterationAge >= 0 && iterationAge < opts.max_age_iterations)) {
        throw new BaselineStaleError(`baseline iteration age (${iterationAge}) is outside [0, ${opts.max_age_iterations})`);
    }
}
 ...
 */
function affectsAllWorkspacePackages(repoRelativePaths, repoRoot, workspaceRoot) {
 ...
 */
export function resolveLexicalRepoRoot(workingDir) {
...
function resolveGateTargetDirs(opts, workspacePackages, allowedPathsUsed, start, emit) {
    ...
    if (targetDirs.length === 0)
        return declaredSkip('no_target_dir_in_scope');
    return { targetDirs };
}
/**
 ...
 */
async function canRunTestScript(check, cmd, projectType, dir, emit) {
    if (check !== 'tests') {
 ...
 */
async function measureAndPartitionFailures(opts, targetDirs, cmdMap, projectType, emit) {
