 ...
 */
function dropRepo(repoPath, reason) {
 ...
 */
function describeSpawnFailure(proc) {
    if (proc.error)
        return `git log failed: ${safeErrorMessage(proc.error)}`;
 ...
 */
function forEachActivityEventInWindow(activityDir, since, until, label, onEvent) {
    let files;
    try {
