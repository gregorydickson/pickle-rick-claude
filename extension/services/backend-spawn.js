...
function warnMcpLayerDegraded(rejection, winningLayer) {
    ...
    process.stderr.write(`[backend-spawn] ${prominence}: MCP config degraded: ${rejection.layer} ${rejection.filePath} `
...
function resolveMcpConfigWithLayer(settingsBag, homeDir) {
    ...
    const decide = () => {
        ...
        if (typeof override === 'string' && override.trim()) {
            ...
            if (verdict !== 'valid')
                rejections.push({ layer: 'settings_override', filePath: overridePath, verdict });
        ...
        if (verdict !== 'missing')
            rejections.push({ layer: 'claude_json_fallback', filePath: claudeJson, verdict });
        return { path: null, layer: 'omitted' };
    };
    const resolved = decide();
    if (rejections.length > 0)
        warnMcpLayerDegraded(rejections[0], resolved.layer);
    return resolved;
}
