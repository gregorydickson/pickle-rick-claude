 ...
 */
function listPromotableTmpSiblings(filePath: string): { tmpPath: string; tmpPid: number }[] {
  ...
  const siblings: { tmpPath: string; tmpPid: number }[] = [];
 ...
 */
export function removeRecoverableJsonObject(filePath: string): boolean {
  let removed = false;
