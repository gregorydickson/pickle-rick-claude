...
export type TicketGateCompletion = {
  ...
  reason: 'narrow_tier_shortcircuit' | 'skipped' | 'timed_out' | 'failed' | 'observed' | 'unresolved';
};
...
export type GateCompletionReport = {
  ...
  summary: {
    ...
    timeouts: number;
    /** Spawned tickets whose run left NO manager-side terminal event — neither clean nor red. */
    unresolved: number;
    verdict: string;
  };
};
...
function gateReason(flags: {
  ...
  timedOut: boolean;
  failedNonTimeout: boolean;
}): TicketGateCompletion['reason'] {
  if (flags.narrowTierVacuity) return 'narrow_tier_shortcircuit';
  if (flags.observedCompletion) return 'observed';
  if (flags.skipped) return 'skipped';
  if (flags.timedOut) return 'timed_out';
  if (flags.failedNonTimeout) return 'failed';
  return 'unresolved';
}
