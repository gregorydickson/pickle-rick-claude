...
function headingMatchesDeclaredSection(line: string): boolean {
 ...
 */
export function readDeclaredFiles(content: string): string[] {
  const out: string[] = [];
  const seen = new Set<string>();
  const add = (token: string): void => {
    if (!isPathShaped(token)) return;
  ...
  for (const line of lines) {
    ...
    if (inDeclaredSection || inlineLabel) {
      for (const token of backtickedTokens(line)) add(token);
    }
  }
  return out;
}
