...
function quotedMessageTokens(message: string): string[] {
...
function fileAxisKey(value: string): string {
 ...
 * in the phase's diff. */
function changedFileKeys(failure: GateFailure, ctx: NoDisownContext): string[] {
...
export function isSelfIntroducedFailure(failure: GateFailure, ctx?: NoDisownContext): boolean {
  ...
  if (ctx.changedFiles.size > 0) {
    ...
    if (changedFileKeys(failure, ctx).some(k => changed.has(k))) return true;
  }
...
export function getChangedExportedSymbols(workingDir: string, sinceCommit: string): Set<string> | null {
  const result = spawnSync(
    'git',
    ['-c', 'diff.relative=false', 'diff', '--no-renames', `${sinceCommit}..HEAD`,
      '--', ':(top)*.ts', ':(top)*.tsx'],
...
export function resolveLexicalRepoRoot(workingDir: string): string {
  ...
  if (!enumerationCompleted(result)) return workingDir;
...
interface CheckResult {
  ...
  stderr: string;
  /**
   * `null` means the check produced NO exit status — the child was killed by a signal and
   * `close` fired with `code === null`. That is a THIRD state, distinct from both "exited 0"
   * and "exited non-zero", and it has to be representable HERE because every downstream
   * question ("did this run?", "what failed?") is answered from this record and nothing else.
   * Collapsing it onto a number is AP-EXT-ITER318-01.
   */
...
async function runCheckSubtree(
  ...
  return await new Promise<CheckResult>((resolve, reject) => {
    ...
    const settleWith = (exitCode: number | null): void => {
      settle(() => { resolve({ stdout, stderr, exitCode }); });
    };
    child.on('error', (err) => { stderr += `${err.message}\n`; settleWith(127); });
    child.on('close', (code, signal) => {
      if (code === null) stderr += `check terminated by signal ${signal ?? 'unknown'}\n`;
      settleWith(code);
    });
...
function classifyUnrunnableCheck(result: CheckResult): string | null {
  ...
  if (result.exitCode === null) return 'terminated by signal: no exit status';
