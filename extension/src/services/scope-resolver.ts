  ...
  if (parsed.mode !== 'diff' && baseSha === headSha) {
    ...
    if (baseRefTipSha && baseRefTipSha !== headSha) {
      ...
      if (forkPointBase) {
        ...
        throw new ScopeError(
          'SCOPE_BASE_AHEAD_OF_HEAD',
          `Base ref "${baseRef}" is ahead of HEAD (merge-base ${baseSha} equals HEAD, but ` +
...
function getDeclaredNoDiffPaths(paths: string[], repoRoot: string): Set<string> {
  ...
  for (let i = 0; i + 2 < t.length; i += 3) {
    if (t[i + 1] === 'diff' && t[i + 2] === 'unset') declared.add(t[i]);
  }
  return declared;
}
...
function _runGitGrepImportWalk(pattern: string, root: string, timeoutMs: number): string[] | null {
  ...
  const errorCode = (gitGrep.error as NodeJS.ErrnoException | undefined)?.code;
