...
export interface EvidenceResult {
  ...
  /**
   * AP-EXT-ITER327-01: true when this `absent` rests on a probe that NEVER RAN —
   * no repo on the dir ladder produced an exit status for the stamped sha, or the
   * ticket file itself could not be read. Absence-of-proof, not proof-of-absence:
   * a consumer that DISCARDS on `absent` (the phantom-Done watcher) must not act
   * on it (R-DSAN never-discard). A consumer that merely REFUSES (the Done flip)
   * ignores it and stays fail-closed.
   */
  unmeasured?: boolean;
}
...
export type RevertDecision = {
  ...
  /**
   * AP-EXT-ITER330-02: the accept's OWN basis, carried across this boundary so a
   * reporting consumer never has to re-derive it from `kind`. `kind: 'committed'`
   * is TRUE for a zero-diff accept — which carries no `sha` by construction — so
   * every claim derived from `kind` alone asserted `completion_commit` evidence for
   * a ticket that provably has none. Present on `keep` accepts; absent on `revert`.
   */
  via?: EvidenceVia | 'announcement' | 'zero-diff';
};
 ...
 */
function probeCatFile(
  workingDir: string,
  sha: string,
  onGitDidNotSpeak?: () => void,
): 'exists' | 'not-exists' | 'git-could-not-run' {
  try {
    ...
    if (!gitReportedExitStatus(err)) onGitDidNotSpeak?.();
...
function probeShaOverLadder(
  ...
  const markUnspoken = (): void => { gitDidNotSpeak = true; };
  ...
  if (gitDidNotSpeak) onUnmeasured?.();
  return null;
}
 ...
 */
function gitDirLadder(ctx: Pick<EvidenceCtx, 'workingDir' | 'fallbackDir'>): string[] {
 ...
 */
function showCommitMessage(sha: string, dirs: string[]): string {
  for (const dir of dirs) {
    try {
      const message = execFileSync(
        'git',
        ['-C', dir, 'show', '-s', '--format=%B', sha],
...
function scanGitLogByTrailer(args: {
  ...
  try {
    ...
    if (!gitReportedExitStatus(err)) args.onGitDidNotSpeak?.();
    return null;
  }
...
function isForeignAttributedExplicitSha(
  sha: string,
  ctx: Pick<EvidenceCtx, 'workingDir' | 'fallbackDir' | 'sessionDir' | 'ticketId' | 'ownAttributionTokens'>,
  content: string,
): boolean {
...
/** The rejection verdicts an accept arm can draw. Each arm maps them to its own absentReason. */
type AcceptRejection = 'baseline_sha' | 'foreign_attribution';
 ...
 */
function rejectsAccept(
  sha: string,
  ctx: Pick<EvidenceCtx, 'startCommit' | 'pinnedSha' | 'workingDir' | 'fallbackDir' | 'sessionDir' | 'ticketId' | 'ownAttributionTokens'>,
  content: string,
): AcceptRejection | null {
  if (rejectsAsBaseline(sha, ctx)) return 'baseline_sha';
  if (isForeignAttributedExplicitSha(sha, ctx, content)) {
    process.stderr.write(
      `[ticket-completion-evidence] sha ${sha} rejected — positively attributed to a different ticket (R-OMA foreign-attribution)\n`,
    );
...
function scanGitLog(args: {
  ...
  if (gitDidNotSpeak) args.onUnmeasured?.();
  return null;
}
...
export function readEvidence(ctx: EvidenceCtx): EvidenceResult {
  ...
  const markUnmeasured = (): void => { probeUnmeasured = true; };
  ...
  if (scan) {
    ...
    if (rejectsAccept(scan.sha, ctx, content)) return absent();
...
export type CompletionRefusal = {
  ...
  /**
   * AP-EXT-ITER327-01: the refused evidence was never MEASURED (see
   * `EvidenceResult.unmeasured`). Read ONLY by `gateForPhantomDoneRevert`, the one
   * consumer whose refusal DISCARDS shipped work; every refusing consumer ignores it.
   */
  unmeasured?: boolean;
};
...
export function gateForPhantomDoneRevert(ctx: EvidenceCtx, _policy?: RevertPolicy): RevertDecision {
  ...
  if (decision.unmeasured) return { action: 'keep', kind: 'absent' };
