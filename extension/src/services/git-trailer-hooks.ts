...
export interface MaterializeTrailerHooksOpts {
  repoRoot: string;
  managedDir: string;
}

export type MaterializeTrailerHooksResult =
  | { ok: true; managedDir: string }
  | { ok: false; reason: string };

function runGitBestEffort(args: string[], cwd: string): { status: number; stdout: string } {
  try {
...
function resolveAgainstRepoRoot(repoRoot: string, value: string): string {
 ...
 */
function resolveDirFromGitOutput(args: string[], repoRoot: string, suffix?: string): string | null {
  try {
 ...
 */
function resolvePreExistingHooksDir(repoRoot: string): string | null {
...
function isExecutableFile(entryPath: string): boolean {
  try {
 ...
 */
function listForwardableHooks(preExistingDir: string): string[] {
  try {
    return fs
 ...
 */
function buildTrailerHookScript(
  originalPrepareCommitMsgAbsPath: string | null,
  versionTrailerValue: string,
): string {
...
function buildForwardingStubScript(originalAbsPath: string): string {
 ...
 */
export function materializeTrailerHooks(
  opts: MaterializeTrailerHooksOpts,
): MaterializeTrailerHooksResult {
  ...
  try {
    ...
    writeExecutableScript(
      path.join(opts.managedDir, 'prepare-commit-msg'),
      buildTrailerHookScript(
        hasOriginalPrepareCommitMsg ? originalPrepareCommitMsg : null,
        resolvePickleRickVersionValue(),
      ),
    );

    for (const hookName of listForwardableHooks(preExistingDir)) {
      writeExecutableScript(
        path.join(opts.managedDir, hookName),
        buildForwardingStubScript(path.join(preExistingDir, hookName)),
      );
    }
