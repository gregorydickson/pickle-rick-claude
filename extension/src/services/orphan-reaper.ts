 ...
 */
function isStaleFixtureTmpDir(dir: string, now: number, maxAgeMs: number): boolean {
  try {
...
export type SweepDerivedTmpDirFixturesOpts = {
  ...
  prefixes?: string[];
  /** Staleness floor below which an attributed directory is spared. Defaults to `FIXTURE_TMPDIR_MAX_AGE_MS`. */
  maxAgeMs?: number;
};
 ...
 */
function derivedSweepNotRun(
  reason: ReapSweepSkipReason,
  prefixes: string[],
): SweepDerivedTmpDirFixturesResult {
...
export function sweepDerivedTmpDirFixtures(opts: SweepDerivedTmpDirFixturesOpts = {}): SweepDerivedTmpDirFixturesResult {
  ...
  for (const entry of entries) {
    ...
    scanned += 1;
    if (!isStaleFixtureTmpDir(resolved, now, maxAgeMs)) continue;
    try {
