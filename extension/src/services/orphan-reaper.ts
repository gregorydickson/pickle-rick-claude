 ...
 */
function readOwningSessionState(sessionDir: string): Record<string, unknown> | null {
  try {
    return readRecoverableJsonObject(path.join(sessionDir, 'state.json')) as Record<string, unknown> | null;
  } catch {
    return null;
  }
}
