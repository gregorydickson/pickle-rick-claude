...
export function assertMicroverseStateShape(
  ...
  if (parsed.baseline_score !== null) requireFiniteNumber(parsed.baseline_score, 'baseline_score');
...
export function getLastAcceptedScore(state: MicroverseSessionState): number {
  ...
  if (lastAccepted) return lastAccepted.score;
 ...
 */
export function deriveStallCause(
  state: MicroverseSessionState,
  iteration: number,
): StallDisposition {
  const signal = state.convergence.last_stall_signal ?? null;
  const cause: StallCause = signal ?? 'unknown';
 ...
 */
function mintUniqueViolationId(violation: Violation, taken: ReadonlySet<string>): string {
