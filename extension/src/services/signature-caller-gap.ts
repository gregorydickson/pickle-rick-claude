...
function gitTrackedFiles(repoRoot: string): string[] | null {
...
function isOptionalCompatibleChange(line: string): boolean {
...
function extractSchemaShapeSymbols(content: string): string[] {
  const symbols = new Set<string>();
  for (const rawLine of content.split(/\r?\n/)) {
    if (!SCHEMA_SHAPE_CUE_RE.test(rawLine)) continue;
    if (isOptionalCompatibleChange(rawLine)) continue;
    SCHEMA_SYMBOL_RE.lastIndex = 0;
    for (const match of rawLine.matchAll(SCHEMA_SYMBOL_RE)) symbols.add(match[1]);
  }
  return [...symbols];
}
...
function findFactoryBridgeNames(
  symbol: string,
...
function callerCandidateFiles(repoRoot: string, cache?: ResolverCache): string[] {
  ...
  for (const file of tracked) {
    ...
    if ((!isExistingCandidate && !isTrackedSource) || seen.has(file)) continue;
    seen.add(file);
    candidates.push(file);
    if (candidates.length >= CALLER_CANDIDATE_MAX) break;
  }
  return candidates;
}
