...
import { State, VALID_STEPS, UNBOUNDED_READ_MAX_BUFFER, type ActivityEvent, type PickleSettings, type Backend } from '../types/index.js';
...
export function resolveExtensionRoot(requestedRoot: string | undefined): string {
 ...
 */
export function getDeployedVersion(): string | null {
  try {
    const pkg = JSON.parse(
      fs.readFileSync(path.join(getExtensionRoot(), 'extension', 'package.json'), 'utf8'),
...
export {
  type DiffFileVisualStat,
  type DiffVisualStat,
  type LifecyclePhase,
  type TicketClassifierInfo,
  type TicketComplexityTier,
  type TicketTierBudget,
  type TierCapPartial,
  type TierCapsConfig,
  CLASSIFIER_EXPENSIVE_VERIFY_KEYWORDS,
  TICKET_TIER_BUDGETS,
  TIER_DIFF_ENVELOPE,
  TIER_LIFECYCLE,
...
export {
  type ScopeSettings,
  DEFAULT_BOUNDED_TERMINAL_ESCAPE_CAP,
  DEFAULT_BREAKER_RECOVERY_GRACE_SECONDS,
  DEFAULT_FAILED_FLIP_SUPPRESSION_CAP,
  DEFAULT_MAX_PARK_MINUTES,
  DEFAULT_RATE_LIMIT_PROBE_INTERVAL_MS,
  DEFAULT_SILENT_DEATH_RESPAWN_CAP,
  DEFAULT_TIER_STALL_THRESHOLD_MS,
  MIN_RATE_LIMIT_PROBE_INTERVAL_MS,
  RATE_LIMIT_PROBE_INTERVAL_ENV_VAR,
...
export {
  type EnsureMonitorWindowOptions,
  type EnsureMonitorWindowResult,
  type MonitorMode,
  type RespawnMonitorWindowForModeOpts,
  type WatcherPaneCommand,
...
export function wrapWithStderrRedirect(cmd: string, sessionDir: string, pane: MonitorPane): string {
...
export async function _killOldMonitorPid(smLocal: StateManager, statePath: string): Promise<void> {
  try {
