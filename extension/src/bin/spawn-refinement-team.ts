 ...
 */
export function terminateWorkerProcess(
  proc: import('child_process').ChildProcess,
  signal: NodeJS.Signals,
): boolean {
  const pid = proc.pid;
  if (typeof pid === 'number' && killProcessGroup(pid, signal)) return true;
  try {
 ...
 */
export function spawnAnalystProcess(
  cmd: string,
  args: string[],
...
export type SymbolAuditCategory = 'activity_event' | 'new_file' | 'helper_sentinel';
...
// preceded by `/`.
function matchesOnPathBoundary(trackedPath: string, token: string): boolean {
...
const ANALYST_PERSONA = `You are Pickle Rick — hyper-competent, arrogant, ruthlessly thorough.
...
const RISK_SCOPE_ROLE_INSTRUCTIONS = `## Your Role: Risk & Scope Auditor Morty
...
/**
 * The codebase analyst's role block is the only one that varies per invocation:
 * it interpolates the working directory, optionally cross-references portal-gun
 * artifacts, and carries the activity-event schema the analyst must cite against.
 */
function buildCodebaseRoleInstructions(workingDir: string, portalContext?: PortalContext): string {
  const portalSection = portalContext ? `
...
Read these artifacts to validate PRD claims against the actual donor code and target analysis.` : '';
  return `## Your Role: Codebase Context Analyst Morty
...
Use file:line references you have verified for every codebase claim; mark anything unverified as a hypothesis. If the codebase is empty/irrelevant, say so explicitly and note what the PRD should specify instead.` + portalSection + `\n\n${ACTIVITY_EVENT_SCHEMA_SECTION}`;
}

/** Resolve one analyst's role block. The Record keeps the RoleId union exhaustive. */
function buildRoleInstructions(roleId: RoleId, workingDir: string, portalContext?: PortalContext): string {
  const byRole: Record<RoleId, () => string> = {
    requirements: () => REQUIREMENTS_ROLE_INSTRUCTIONS,
    codebase: () => buildCodebaseRoleInstructions(workingDir, portalContext),
    'risk-scope': () => RISK_SCOPE_ROLE_INSTRUCTIONS,
  };
  return byRole[roleId]();
}

/**
 * Cycle 2+ analysts read every analyst's previous-cycle output so the deeper pass
 * can cross-reference instead of repeating. Cycle 1 (or a cycle with nothing on
 * disk to carry forward) contributes no section at all.
 */
function buildCrossReferenceSection(
  roleId: RoleId,
  cycle: number,
  previousAnalyses?: Map<RoleId, string>,
): string {
  if (cycle <= 1 || !previousAnalyses || previousAnalyses.size === 0) return '';
...
`;
  ...
  return crossRefSection;
}

/** The shared guidance + output-contract half of the prompt, identical for every role. */
function buildAnalystOutputInstructions(outputFile: string, cycle: number): string {
  ...
  return `${buildPromptGuidanceSections()}
...
export function buildWorkerPrompt(
  ...
  const roleInstructions = buildRoleInstructions(roleId, workingDir, portalContext);
  const crossRefSection = buildCrossReferenceSection(roleId, cycle, previousAnalyses);
  const outputInstructions = buildAnalystOutputInstructions(outputFile, cycle);
...
function createAnalystLogStream(
  refinementDir: string,
  roleId: RoleId,
  cycle: number,
...
/** Best-effort telemetry: record the manager backend this analyst was launched under. */
function emitWorkerBackendResolved(sessionDir: string): void {
...
/** Launch the analyst subprocess and wire its stdio into the log stream. */
function startAnalystProcess(
  opts: AnalystSpawnOptions,
  logStream: fs.WriteStream,
): { proc: import('child_process').ChildProcess; cmd: string } {
  ...
  const includes = [opts.extensionRoot, getDataRoot(), opts.workingDir];
  if (opts.sessionDir) includes.push(opts.sessionDir);
  ...
  const proc = spawnAnalystProcess(invocation.cmd, invocation.args, { cwd: opts.workingDir, env });
...
interface AnalystKillEscalation {
  /** True once the timeout fired — the evidence check treats a timed-out analyst as failed. */
  timedOut: () => boolean;
  /** Disarm both stages. ONE definition: the pair was previously hand-copied per call site. */
  clear: () => void;
}
 ...
 * because `spawnAnalystProcess` makes the child lead its own process group.
 */
function armAnalystKillEscalation(
  proc: import('child_process').ChildProcess,
  timeout: number,
): AnalystKillEscalation {
...
interface AnalystSettlementContext {
  proc: import('child_process').ChildProcess;
  escalation: AnalystKillEscalation;
  logStream: fs.WriteStream;
...
/**
 * Resolve exactly once, by whichever of exit / spawn-error / hang-guard arrives first.
 * Every path funnels through `settleWith`, so the process-set removal and the timer
 * teardown cannot drift apart per outcome.
 */
interface AnalystSettlementLatch {
  settleWith: (result: WorkerResult) => void;
  clearTimers: () => void;
  isSettled: () => boolean;
}

/** Wire the two subprocess outcomes onto the latch. Neither resolves directly. */
function attachAnalystOutcomeHandlers(
  ctx: AnalystSettlementContext,
  latch: AnalystSettlementLatch,
): void {
  const { proc, escalation, logStream, resultBase } = ctx;

  proc.on('error', (err) => {
    const msg = safeErrorMessage(err);
    console.error(
      `${Style.RED}Failed to spawn ${ctx.invocationCmd} (${resultBase.roleId}): ${msg}${Style.RESET}`
    );
    logStream.end();
    latch.settleWith({ ...resultBase, success: false, exitCode: null });
  });

  proc.on('close', (code) => {
    const exitCode = code ?? null;
    if (latch.isSettled()) return; // error handler already resolved
    // Disarm BEFORE the flush wait, or a hang guard due inside the next 5s
    // resolves failure while the flush callback is still pending.
    latch.clearTimers();

    finalizeAfterLogFlush(logStream, () => {
      const success = evaluateAnalystSuccess({
...
function awaitAnalystSettlement(ctx: AnalystSettlementContext): Promise<WorkerResult> {
  const { proc, escalation, resultBase } = ctx;
  ...
  return new Promise<WorkerResult>((resolve) => {
    ...
    const settleWith = (result: WorkerResult) => {
      if (settled) return;
      settled = true;
...
function spawnWorker(opts: AnalystSpawnOptions): Promise<WorkerResult> {
  ...
  if (opts.sessionDir) emitWorkerBackendResolved(opts.sessionDir);

  const { proc, cmd } = startAnalystProcess(opts, logStream);
  const escalation = armAnalystKillEscalation(proc, opts.timeout);

  return awaitAnalystSettlement({
    proc,
    escalation,
    logStream,
    invocationCmd: cmd,
  ...
  try {
    const workerPromises = WORKER_ROLES.map(({ id }) => {
      ...
      return spawnWorker({
        roleId: id,
        prompt,
        refinementDir: opts.refinementDir,
...
function strongerTier(
  a: TicketComplexityTier | undefined,
  b: TicketComplexityTier | undefined,
): TicketComplexityTier | undefined {
 ...
 */
function mergeAnalystTicketShape(
  first: RefinementTicketManifestEntry,
  next: RefinementTicketManifestEntry,
): RefinementTicketManifestEntry {
...
function ticketsForSmell(smell: AcShapeSmell, tickets: RefinementTicketManifestEntry[]): RefinementTicketManifestEntry[] {
  ...
  return collapseAnalystTicketCopies(tickets.filter((ticket) => (
    (explicitIds.size > 0 && explicitIds.has(ticket.id)) || ticket.source_ac_ids.includes(smell.ac_id)
  )));
}
...
function reportAdvisoryGateVerdict(gate: string, status: number): void {
  if (status === 0) return;
  process.stderr.write(
    `[pickle-rick] ${gate} advisory: exited ${status} — findings logged, NOT halting (advisory gate)\n`,
  );
}
...
const ACTIVITY_EVENT_CLAIM_RE =
  /\b(?:activity[-_\s]?events?|activity_event|event_type)\s*[:=]|\blogged\s+as\b|\b(?:emit|emits|emitted)\b|\blogActivity\b|\bVALID_ACTIVITY_EVENTS\b/i;
...
function lineRefs(content: string): { line: string; sourceLine: number }[] {
  ...
  const result: { line: string; sourceLine: number }[] = [];
  let inFence = false;
  let fenceChar = '';
  let fenceLen = 0;
...
export function detectBundleOfBundlesOverCollapse(
  ...
  try {
    ...
    if (distinctTickets.length > composedPaths.length) return { detected: false };
    return {
      detected: true,
