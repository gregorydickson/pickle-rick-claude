...
import { filterBySubsystem, resolveScope, ScopeError, type LaneRecord } from '../services/scope-resolver.js';
 ...
 * never pays for it.
 */
async function printSubsystems(args: string[]): Promise<void> {
  ...
  let gitRepoRoot: (cwd: string) => string = (cwd) => cwd;
  try {
  ...
  if (scopeFlag !== undefined) {
    ...
    const cleanup = () => { if (givenRoot === undefined) fs.rmSync(sessionRoot, { recursive: true, force: true }); };
    try {
...
function runScopeResolution(args: string[]): void {
  const scopeFlag = parseFlag(args, '--scope');
