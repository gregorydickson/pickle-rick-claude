 ...
 */
function createCodegraphActivitySink(sessionDir: string | undefined): (entry: ActivityLogEntry) => void {
  return (entry: ActivityLogEntry): void => {
 ...
 */
function buildCodexContractAdditions(ticket: TicketSpec): string {
  return `
...
export function buildWorkerPrompt(opts: BuildWorkerPromptOptions): string {
  ...
  if (!ticket.isReviewTicket) workerPrompt = applyTierLifecycleTemplate(workerPrompt, opts);
  ...
  if (ticket.backend === 'codex') workerPrompt += buildCodexContractAdditions(ticket);
...
function describeKillEscalation(sigtermSent: boolean, sigkillSent: boolean): string {
  if (sigkillSent) return 'sent SIGTERM to process tree and escalated to SIGKILL after 2000ms';
  if (sigtermSent) return 'sent SIGTERM to process tree';
...
): { stdoutChunks: string[]; stderrChunks: string[] } {
...
async function runWorkerGateStaticChecks(
  lintTargets: string[],
  extensionDir: string,
): Promise<{ lint: WorkerGateCheckPhase; tsc: WorkerGateCheckPhase }> {
 ...
 * gate as its author.
 */
function recordOffRepoGateNotRun(statePath: string, ticketId: string): void {
 ...
 */
function flagOffRepoGateRed(
  statePath: string,
  ticketId: string,
  project: ResolvedWorkerGateProject,
  failed: OffRepoDimension,
): void {
  ...
  console.error(
    `[spawn-morty] target-repo gate RED (${project.projectType} @ ${project.dir}, phase ${failed.phase}) — flagged, not halting`,
  );
}
...
/** Arguments shared by `runWorkerGate` and its per-stage helpers. */
type WorkerGateArgs = {
  workingDir: string;
  ticketId: string;
  statePath: string;
...
function warnIfTierDiffEnvelopeExceeded(args: WorkerGateArgs): void {
  if (!args.preWorkerHead || !args.ticketTier) return;
 ...
 */
function recordTierPhaseSkips(args: WorkerGateArgs, workerGateTier: WorkerGateTier): void {
  if (args.ticketTier !== 'small') return;
  writeActivityEntry(args.statePath, {
...
type WorkerGateChecksOutcome = {
  gateResult: WorkerGateCheckResult;
  retryCount: number;
  autofixApplied: boolean;
};
...
async function runGateChecksWithAutofixRetry(opts: {
  args: WorkerGateArgs;
  lintTargets: string[];
  reportedFileList: string[];
  extensionDir: string;
  workerGateTier: WorkerGateTier;
  workerTestGateTimeoutMs: number;
}): Promise<WorkerGateChecksOutcome> {
  const checkArgs = {
 ...
 */
function isFailedFlipSuppressed(args: WorkerGateArgs): boolean {
  try {
...
function resetWorkerTreeAfterGateFailure(args: WorkerGateArgs): void {
  if (!args.preWorkerHead) return;
  try {
...
function resolveGateFailurePhase(gateResult: WorkerGateCheckResult): WorkerGatePhase {
  if (gateResult.gatePhase !== null) return gateResult.gatePhase;
  if (gateResult.lintErrors > 0) return 'lint';
  if (gateResult.tscErrors > 0) return 'tsc';
  return 'test:fast';
}

/** Record the red gate, then either preserve the tree for triage or reset it. */
function finalizeFailedWorkerGate(
  fileList: string[],
  args: WorkerGateArgs,
  gateResult: WorkerGateCheckResult,
  outcome: WorkerGateChecksOutcome,
): WorkerGateResult {
  writeActivityEntry(args.statePath, {
    event: 'worker_gate_failed',
    ticket_id: args.ticketId,
...
function finalizePassingWorkerGate(
  fileList: string[],
  args: WorkerGateArgs,
  gateResult: WorkerGateCheckResult,
  reportedFileList: string[],
...
export async function runWorkerGate(changedFiles: string[], args: WorkerGateArgs): Promise<WorkerGateResult> {
...
function reconcileGitRaw(workingDir: string, args: string[]): string {
 ...
 */
function reconcileGitRawOrNull(workingDir: string, args: string[]): string | null {
  try { return reconcileGitRaw(workingDir, args); } catch { return null; }
}
...
function pickAttributionCommit(workingDir: string, windowShas: string[], declaredFiles?: string[]): string | null {
  ...
  if (declared.size > 0) {
    for (const sha of windowShas) {
      ...
      if (files !== null && files.split('\0').some((f) => declared.has(f))) return sha;
    }
  }
 ...
 */
function sliceAcceptanceCriteriaSection(content: string): string {
...
function startWorkerSpinner(startTime: number): ReturnType<typeof setInterval> {
  let idx = 0;
  return setInterval(() => {
 ...
 */
function armWorkerTimeoutKill(ctx: WorkerProcessContext, proc: ReturnType<typeof spawn>): { cancel: () => void } {
  let killEscalation: ReturnType<typeof setTimeout> | null = null;
...
async function handleWorkerSpawnError(params: {
  ctx: WorkerProcessContext;
  err: Error;
  invocation: ReturnType<typeof buildWorkerInvocation>;
  crumb: (label: string) => void;
}): Promise<void> {
...
function drainWorkerLogThenFinalize(params: {
  ...
  async function finalize(exitCode: number | null): Promise<void> {
...
export async function runWorkerProcess(ctx: WorkerProcessContext): Promise<{ exitCode: number; isSuccess: boolean }> {
  ...
  return new Promise(resolve => {
    ...
    const crumb = (label: string) => { if (process.env.PICKLE_DEBUG_SPAWN_MORTY === '1') process.stderr.write(`[SMTEST-1:SPAWN] ${label}\n`); };
    proc.on('error', async err => {
      crumb(`proc.error fired — code=${(err as NodeJS.ErrnoException).code}`);
...
export function workerSpawnLockPath(sessionRoot: string): string {
...
export class WorkerSpawnLockContendedError extends Error {
  incumbentPid: string | null;
  waitedMs: number;
  constructor(waitedMs: number, incumbentPid: string | null) {
...
export type WorkerSpawnLockAcquisition = { inert: true } | { inert: false; lockPath: string; handle: LockHandle };
 ...
 */
export async function acquireWorkerSpawnLock(
  sessionRoot: string,
  timeoutMs: number = WORKER_SPAWN_LOCK_TIMEOUT_MS,
  ...
  for (;;) {
    ...
    await new Promise<void>(resolve => setTimeout(resolve, WORKER_SPAWN_LOCK_RETRY_INTERVAL_MS));
  }
}

export function releaseWorkerSpawnLock(acquisition: WorkerSpawnLockAcquisition): void {
  if (acquisition.inert) return;
