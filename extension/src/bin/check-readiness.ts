...
function unresolvedContractFinding(ticketFile: string, ref: string, measured: boolean): ReadinessFinding {
  return {
    ticket: ticketFile,
...
function findPathFindings(ticket: TicketInfo, repoRoot: string, sessionDir: string, cache?: ResolverCache): ReadinessFinding[] {
  ...
  return [...refs].sort()
    ...
    .map((ref) => ({
      ticket: ticket.file,
      kind: measured ? ('file_path' as const) : ('performance' as const),
