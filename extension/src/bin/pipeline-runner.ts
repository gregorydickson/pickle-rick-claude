 ...
 */
function shouldGraceDrainDetached(
  runtime: PipelineRuntime,
  ...
  if (!dw || !isProcessAlive(dw.worker_pid)) return false;
...
let graceDrainSleep: (ms: number) => void = (ms: number) => {
...
export function _setGraceDrainSleep(fn: (ms: number) => void): void {
  graceDrainSleep = fn;
}
