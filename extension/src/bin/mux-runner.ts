...
import { printMinimalPanel, Style, formatTime, getExtensionRoot, getDataRoot, buildHandoffSummary, sleep, writeStateFile, markTicketDone, markTicketSkipped, collectTickets, getTicketStatus, runCmd, safeErrorMessage, ensureMonitorWindow, displayMacNotification, parseTicketFrontmatter, getTicketTierBudgetWithOverrides, readFrontmatterField, upsertFrontmatterField, ticketFilePath, VALID_TICKET_COMPLEXITY_TIERS, TICKET_TIER_BUDGETS, TIER_LIFECYCLE, composeManagerPromptFromSkill, resolveWorkerTestGateTimeoutMs, resolveCommandTemplate, loadPickleSettingsBag, resolveHardeningSettings, resolveCodegraphSettings, resolveRateLimitSettings, DEFAULT_MAX_PARK_MINUTES, type CompletionCommitEvidence, type TicketComplexityTier, type TicketInfo, type TicketStatus, type TicketTierBudget } from '../services/pickle-utils.js';
 ...
 */
export function workerTimeoutExceedsBashCeiling(
  state: Pick<State, 'worker_timeout_seconds' | 'current_ticket_tier'>,
): boolean {
  ...
  if (tier && (VALID_TICKET_COMPLEXITY_TIERS as readonly string[]).includes(tier)) {
    return TICKET_TIER_BUDGETS[tier as TicketComplexityTier].worker_timeout_seconds > BASH_TOOL_CEILING_SECONDS;
  }
  return false;
}
...
export interface SpawnDetachedLargeTierWorkerResult {
  spawned: boolean;
  pid?: number;
  logPath?: string;
}
 ...
 */
export function spawnDetachedLargeTierWorker(
  input: SpawnDetachedLargeTierWorkerInput,
): SpawnDetachedLargeTierWorkerResult {
 ...
 */
function persistRunnerAuthoredGreenVerdict(sessionDir: string, ticketId: string): void {
  try {
    const fp = ticketFilePath(sessionDir, ticketId);
    const raw = fs.readFileSync(fp, 'utf8');
 ...
 */
function resolveCleanTreeAttribution(
  sessionDir: string,
  workingDir: string,
  ticketId: string,
): string | null {
  ...
  try {
    ...
    const evidence = readEvidence({
      ...
      greenGate: () => {
        if (!fs.existsSync(ext)) return 'failing';
        try { return runBetweenTicketFastTests(ext).ok ? 'passing' : 'failing'; }
        catch { return 'errored'; }
      },
    });
    if (!evidence.sha) return null;
    if (evidence.kind === 'committed') return evidence.sha;
    return null;
  } catch {
    return null;
  }
...
export function routeDeadDetachedWorkerDisposition(input: {
  ...
  log: (msg: string) => void;
  progress: { spawnCount: number; zeroProgressCount: number };
  /** Test seam: inject deterministic salvage deps. Defaults to production wiring. */
  deps?: SalvageDeps;
}): { action: 'continue' | 'break'; exitReason?: ExitReason } {
  ...
  const applyRecoveryOutcome = (): { action: 'continue' | 'break'; exitReason?: ExitReason } => {
    const recovery = routeRecoveryBeforeTerminal({
      sessionDir, statePath, extensionRoot, workingDir, ticketId, iteration, flags, log, mode: 'worker',
    });
    const recovered = applyDetachedRecoveryOutcome({
  ...
  const extensionDir = path.join(workingDir, 'extension');
  const productionDeps: SalvageDeps = {
    reconcile: (i) => reconcileTicketTruth(i),
    gate: () => {
      if (!fs.existsSync(extensionDir)) return 'failing';
      try {
...
async function runMuxRunnerMain() {
  ...
  while (true) {
    ...
    const outcome: Awaited<ReturnType<typeof runIteration>> =
      workerTimeoutExceedsBashCeiling(state)
        ? (() => {
    ...
    if (exitType === 'api_limit') {
      ...
      const resumeTargetMs = resolveParkResumeTime(
        resetAtSec, parkStartMs, rateLimitWaitMinutes * 60 * 1000, drawParkResumeJitterMs(),
      );
