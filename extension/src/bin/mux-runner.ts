...
import { emitOrphanReapSummary, reapOrphanedWorkerProcs, killProcessGroup, type ReapOrphanedWorkerProcsOpts, type ReapSweepResult } from '../services/orphan-reaper.js';
...
import { getHeadBranch, gitReportedExitStatus, updateTicketFrontmatter, isWorkingTreeDirty, listWorkingTreeDirtyPaths, listWorkingTreeDirtyPathsExcludingCodegraph, probeTreeDirty, archiveBeforeDestructive, ArchiveAbortError, isCodegraphArtifact, gitCommitEpoch, CODEGRAPH_PATHSPEC_EXCLUDES, type ArchiveContext, type ArchiveResult } from '../services/git-utils.js';
...
/** `test-runner.ts`'s own stderr line when its spawned child died by signal (ticket 916b6489). */
const TEST_RUNNER_SIGNAL_RE = /\[test-runner\] child terminated by signal (\S+)/;
 ...
 */
function attributeSignalTermination(name: string, lines: string[], gateSignal: string | null): string {
  let signal: string | null = null;
  for (const line of lines) {
    const match = line.match(TEST_RUNNER_SIGNAL_RE);
    if (match) signal = match[1];
  }
  signal ??= gateSignal;
...
export function parseBetweenTicketFastGateFailures(
  output: string,
  workingDir: string,
  gateSignal: NodeJS.Signals | null = null,
): BetweenTicketGateFailure[] {
...
export type ClassifyPostFinalVerdictOutput = {
  ...
  degraded: boolean;
  dimensions: string[];
  /** M1: diagnostic tail for each `script_failure: true` entry, never concatenated into `dimensions`. */
...
/**
 * The answer plus whether git AUTHORISED it. `proven: false` means the fallback is a
 * GUESS shaped like an anchor, not that the answer is unusable — every consumer still
 * receives a directory, and each decides for itself whether a guess is good enough.
 */
type RepoRootAnchor = { root: string; proven: boolean };
 ...
 */
function anchorRepoRoot(dir: string, stableBase: string): RepoRootAnchor {
  ...
  try {
    ...
    if (out) return { root: out, proven: true };
  } catch (err) {
    // not a git repo / missing dir — fall back to the path itself, which is EXACT there.
    if (!gitReportedExitStatus(err)) return { root: absDir, proven: false };
  }
...
export function resolveTicketDesyncWinner(state: State, frontmatterStatuses: Map<string, TicketStatus>, _sessionDir = ''): TicketDesyncResolution {
  const currentTicket = typeof state.current_ticket === 'string' && state.current_ticket.length > 0
    ? state.current_ticket
    : null;
...
export function readTicketStatusMap(sessionDir: string): Map<string, TicketStatus> {
 ...
 */
function unmeasuredKeepClaim(ticketId: string): string {
 ...
 */
function tryAutoCloseSplitOriginal(
  status: string,
  input: CorrectPhantomDoneTicketsInput,
  ticket: TicketInfo,
  allTickets: TicketInfo[],
): boolean {
  if (status !== 'todo' && status !== 'failed') return false;
...
export function correctPhantomDoneTickets(input: CorrectPhantomDoneTicketsInput): number {
  ...
  for (const ticket of allTickets) {
    ...
    if (tryAutoCloseSplitOriginal(status, input, ticket, allTickets)) corrected++;
  }
  return corrected;
}
...
function applyInspectPhantomDoneDecision(
  ...
  switch (decision.action) {
    case 'keep': {
      ...
      if (decision.kind === 'absent') process.stderr.write(`${unmeasuredKeepClaim(ticketId)}\n`);
 ...
 */
export function isHeadAtOrBelowCommit(headSha: string, refSha: string, workingDir: string): boolean | null {
...
function resolveOrphanSha(input: {
  ...
  if (filtered.length === 1) return { sha: filtered[0].tip, discovered: true };
...
type CloserTerminalDecision =
  ...
  | { action: 'exit'; reason: Extract<ExitReason, 'closer_handoff_terminal' | 'recovery_exhausted'>; tracker: CloserHandoffTracker | null; detail: string };
 ...
 */
function emitManagerHandoffResidual(ticketId: string, conformanceFile: string | null): void {
  try {
    const ts = new Date();
    const activityDir = path.join(getDataRoot(), 'activity');
    fs.mkdirSync(activityDir, { recursive: true });
 ...
 */
function partitionPathsHeldByHead(
  repoRoot: string,
  paths: string[],
...
function preStashOutOfAllowlistResidue(
  ...
  try {
    ...
    for (const relPath of split.untracked) {
      try { fs.unlinkSync(path.join(repoRoot, relPath)); removed += 1; } catch { /* best-effort */ }
    }
...
function spawnRateLimitProbe(
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      if (settled) return;
      settled = true;
      if (typeof child.pid !== 'number' || !killProcessGroup(child.pid, 'SIGTERM')) child.kill('SIGTERM');
...
export interface MainLoopRateLimitParkInput {
  ...
  sleep?: (ms: number) => Promise<void>;
  jitterMs?: number;
  /** B2 seam: a test stubs the re-probe instead of spawning a real backend CLI. */
  probeApiCleared?: (ctx: {
    sessionDir: string;
    statePath: string;
    workingDir: string;
  ...
  /** B2 seam: how often (ms) the wait re-probes; defaults to the shared env-configured interval. */
 ...
 */
async function waitOutRateLimitParkWithProbe(
  input: MainLoopRateLimitParkInput,
  now: () => number,
  sleepFn: (ms: number) => Promise<void>,
  parkStartMs: number,
  resumeTargetMs: number,
): Promise<boolean> {
  ...
  while (now() < resumeTargetMs) {
    ...
    try {
      if (readRunnerState(statePath).active !== true) return true;
    } catch { /* proceed */ }
    if (now() - lastProbeMs < probeIntervalMs) continue;
    lastProbeMs = now();
    const verdict = await probeFn({
      sessionDir, statePath, workingDir: state.working_dir || process.cwd(), backend: resolveBackend(state),
    });
    if (verdict === 'cleared') break;
  }
  return false;
}
...
export async function runMainLoopRateLimitPark(
  ...
  if (cancelled) return exitRateLimitPark(statePath, 'cancelled');
...
/**
 * B-LOGEV: what the session log actually told us about the worker.
 *
 * An EMPTY log is an ABSENCE OF MEASUREMENT, not evidence of an absent worker. On the one
 * clean hands-off run ever recorded, 35 of 43 logs were 0 bytes while 11 of 15 tickets held
 * a full lifecycle artifact set — the logs were empty; the workers were not. Reading size 0
 * as "produced nothing" is the `failed`-vs-`empty` collapse this codebase names as its
 * dominant defect class.
 *
 *  - `measured` — the log carried bytes; the sub-class is read from its content, as before.
 *  - `empty`    — no measurement was taken AND independent evidence says the ticket worked
 *                 inside the iteration window. NOT sufficient for `worker_silent_death` or
 *                 `worker_produced_nothing`; the ordinary `worker_partial_lifecycle_exit`
 *                 still reports the exit honestly.
 *  - `failed`   — no measurement was taken and no work corroborates the worker, which IS
 *                 positive evidence it died.
 *
 * `subClass` is a PROJECTION of this, not a second axis: `subClass === 'log_empty'` iff
 * `measurement === 'failed'`. There is deliberately no fourth `SilentDeathSubClass` —
 * `log_empty` is the mechanism under indictment, so it is corroborated, not multiplied.
 */
export type WorkerLogMeasurement = 'measured' | 'empty' | 'failed';

/**
 * The iteration window an empty log is corroborated against. Named (not an inline object
 * type) because `tests/worker-partial-lifecycle-exit-schema-conformance.test.js:68` locates
 * `checkPartialLifecycleExit` with `/export function checkPartialLifecycleExit\b[^{]*\{/`
 * and brace-walks the body — a `{` in the signature would corrupt that walk.
 *
 * Both halves travel together, mirroring `recordBoundedEscapeAttempt`'s `commitWindow`, so a
 * caller cannot supply half a window. Omitting the window entirely is NOT evidence of work.
 */
export interface IterationWorkWindow {
  workingDir: string;
  /** Pre-iteration HEAD — base of the `preIterSha..HEAD` commit window. */
  preIterSha: string | null;
  /** Epoch ms the iteration began — freshness window for lifecycle artifacts. */
  iterationStartMs?: number;
}
...
export interface PartialLifecycleExitClassification {
  ...
  /** B-LOGEV: which of the three log states produced `subClass`. `'failed'` iff `subClass === 'log_empty'`. */
  measurement: WorkerLogMeasurement;
 ...
 */
function classifyWorkerSessionLogs(ticketDir: string, files: string[], workObserved: () => boolean): {
  subClass: SilentDeathSubClass | null;
  measurement: WorkerLogMeasurement;
  sessionLogSize: number;
  logPath: string;
  ...
  if (latest.size === 0) return unmeasured(logPath, pid);
  let content = '';
 ...
 */
function countSilentDeathRespawns(statePath: string, ticketId: string): number | null {
 ...
 * genuinely 0.
 */
function countLedgerSuccesses(statePath: string, ticketId: string, strategy: string): number | null {
  let s: State | null;
  try {
    s = readRecoverableJsonObject(statePath) as State | null;
  } catch { return null; }
  if (!s) return null;
  if (!Array.isArray(s.recovery_attempts)) return 0;
 ...
 */
export function checkPartialLifecycleExit(sessionDir: string, statePath: string, ticketId: string, window?: IterationWorkWindow): PartialLifecycleExitClassification | null {
  const ticketDir = path.join(sessionDir, ticketId);
  let files: string[];
  ...
  const { subClass, measurement, sessionLogSize, logPath, pid } = classifyWorkerSessionLogs(
    ticketDir,
    files,
    () => ticketWorkedInWindow(sessionDir, statePath, ticketId, window),
  );
...
export type SilentDeathRecoveryDecision =
  | { action: 'none' }
  | { action: 'hold'; subClass: SilentDeathSubClass; evidence: 'completion_commit' | 'scoped_commit' | 'fresh_artifacts' | 'archive_failed' | 'ledger_unmeasured' }
 ...
 */
function ticketWorkedInWindow(
  sessionDir: string,
  statePath: string,
  ticketId: string,
  window: IterationWorkWindow | undefined,
): boolean {
...
export type FailedFlipSuppressionDecision =
  ...
  | { action: 'proceed'; reason: 'no_evidence' | 'evidence_check_error' | 'ledger_unmeasured' }
...
export interface WorkerProductionBreadcrumbInput {
  ...
  artifactDelta: number | null;
  preIterSha: string | null;
  /** Epoch ms the iteration began (B-LOGEV). Absent → the empty log has no window to be corroborated against. */
  iterationStartMs?: number;
}
...
export function emitWorkerProductionBreadcrumb(
  ...
  const ticketDir = path.join(input.sessionDir, input.ticketId);
  const { measurement, sessionLogSize, pid } = classifyWorkerSessionLogs(
    ticketDir,
    fs.readdirSync(ticketDir),
    () => ticketWorkedInWindow(input.sessionDir, input.statePath, input.ticketId, {
      workingDir: input.workingDir,
 ...
 */
function loadMuxLoopSettings(extensionRoot: string, sessionDir: string) {
  const cbSettings = loadSettings(extensionRoot);
  const cbEnabled = cbSettings.enabled;
  const initialCbState: CircuitBreakerState | null = cbEnabled ? initCircuitBreaker(sessionDir, cbSettings) : null;
 ...
 */
function haltOnMissingTargetToolchain(
  statePath: string,
 ...
 */
function skipResumedDoneTicketCapCache(
  statePath: string,
  sessionDir: string,
  state: State,
  log: (msg: string) => void,
 ...
 */
function evaluateIterationCapExit(
  state: State,
  statePath: string,
  sessionDir: string,
  curIter: number,
...
function evaluateTimeLimitExit(
  state: State,
  statePath: string,
  iteration: number,
  log: (msg: string) => void,
...
function emitTicketRunnabilityAndRefund(
  statePath: string,
  sessionDir: string,
  currentTicket: State['current_ticket'],
  preTicket: string | null | undefined,
  iteration: number,
  log: (msg: string) => void,
): void {
  if (!preTicket) return;
...
function recordOrphanSessionsAtIterationBoundary(
  state: State,
  statePath: string,
  sessionDir: string,
  log: (msg: string) => void,
 ...
 */
function applyBundleBootstrapExemption(state: State, sessionDir: string, log: (msg: string) => void): State {
...
function runReadinessAdvisoryGate(state: State, sessionDir: string, extensionRoot: string, log: (msg: string) => void): void {
...
function runTicketAuditAdvisoryGate(state: State, sessionDir: string, extensionRoot: string, log: (msg: string) => void): void {
...
function warnOnMultiRepoSession(state: State, sessionDir: string, iteration: number, log: (msg: string) => void): void {
 ...
 */
function applySparkSmokeGateAtIteration(
  state: State,
  statePath: string,
  sessionDir: string,
  bypassEmitted: boolean,
  log: (msg: string) => void,
): { halt: boolean; bypassEmitted: boolean } {
...
/** What the main loop does after the rate-limit cycle: stop, skip the rest of the iteration, or carry on. */
type RateLimitCycleStep =
  | { kind: 'exit'; exitReason: ExitReason }
  | { kind: 'parked'; consecutiveRateLimits: number }
  | { kind: 'proceed'; consecutiveRateLimits: number };
 ...
 */
async function applyRateLimitCycleOutcome(
  input: Omit<MainLoopRateLimitParkInput, 'exitResult'> & { exitResult: ReturnType<typeof classifyIterationExit> },
): Promise<RateLimitCycleStep> {
...
/**
 * Pre-spawn half of the iteration: the iteration window (working dir, HEAD, start time) and the
 * R-WSWA-2 per-ticket review/conformance artifact count, taken BEFORE the worker spawn; the AFTER
 * snapshot + delta persistence happen once it exits (`recordPostSpawnWorkerProgress`).
 */
function snapshotPreSpawnIteration(
  state: State,
  sessionDir: string,
): {
  iterWorkingDir: string;
  preIterSha: ReturnType<typeof readHeadCommit>;
  iterStartMs: number;
  ...
  return { iterWorkingDir, preIterSha, iterStartMs, apTicketId, apCreditEarlyPhases, apBeforeCount };
}
...
function recordPostSpawnWorkerProgress(input: {
  ...
  try {
    ...
    if (exitCommit.committed) exitCommitProgressMs = Date.now();
  ...
  let apProgressResult: ReturnType<typeof recordWorkerArtifactProgress> | null = null;
  try {
...
function evaluateIterationPartialLifecycleExit(input: {
  sessionDir: string;
  statePath: string;
  iterTicket: string | null;
  iterWorkingDir: string;
  iteration: number;
  preIterSha: ReturnType<typeof readHeadCommit>;
  iterStartMs: number;
  apProgressResult: ReturnType<typeof recordWorkerArtifactProgress> | null;
  apBeforeCount: number;
  log: (msg: string) => void;
...
interface TicketTransitionInput {
  sessionDir: string;
  statePath: string;
  state: State;
  previousTicket: string | null;
...
type TicketInfoRow = ReturnType<typeof collectTickets>[number];
 ...
 */
function recoverHeadRegressionAfterModelDone(input: TicketTransitionInput & {
  ticketId: string;
  ticketWorkingDir: string | null | undefined;
  startCommit: string;
  completionCommitSha: string | null;
}): boolean {
...
/** True when the ticket the loop moved past already reads Done in its frontmatter. */
function isModelAttestedDone(
  sessionDir: string,
  prevTicketInfo: TicketInfoRow | undefined,
): prevTicketInfo is TicketInfoRow & { id: string } {
  if (!prevTicketInfo?.id) return false;
  return normalizedStatus(getTicketStatus(sessionDir, prevTicketInfo.id)) === 'done';
}

/**
 * A model-attested Done must carry completion evidence (F3 / R-DWC guard). `continue` parks
 * the ticket and continues the phase loop (the caller records the wasted iteration);
 * `return` is the head-regression suppression-cap halt. The drift branch's
 * `applyAutoTicketCompletionValidation` call site stays in the loop body.
 */
function validateModelAttestedDone(input: TicketTransitionInput & {
  previousTicket: string;
  prevTicketInfo: TicketInfoRow & { id: string };
}): { kind: 'continue' } | { kind: 'return' } | { kind: 'proceed' } {
  const { sessionDir, statePath, state, previousTicket, previousTicketStartCommit, prevTicketInfo, log } = input;
...
interface CompletedTicketBoundary {
  ticketId: string;
...
function describeCompletedTicketBoundary(
  input: TicketTransitionInput & { previousTicket: string },
  prevTicketInfo: ReturnType<typeof collectTickets>[number] | undefined,
  postState: State,
  postTicket: string | null,
): CompletedTicketBoundary {
  return {
    ticketId: input.previousTicket,
...
function runCompletedBoundaryFastGate(completedBoundary: CompletedTicketBoundary, statePath: string, log: (msg: string) => void): void {
  try {
    runBetweenTicketFastGate({
      statePath,
...
function resolveNextTicketStartCommit(
  input: TicketTransitionInput,
  nextTicket: string | null,
  lifecycleState: State,
): string | null {
  if (nextTicket === input.previousTicket) return input.previousTicketStartCommit;
...
/**
 * Tail of the ticket transition, after the prior ticket's completion was validated: persist
 * the post-iteration lifecycle, run the between-ticket fast gate for a completed boundary, and
 * hand back the next `previousTicket` / `previousTicketStartCommit` pair. Runs inside the
 * loop's swallowing try, so a throw here leaves the caller's pair unchanged.
 */
function completeTicketTransition(input: TicketTransitionInput & {
  postState: State;
  postTicket: string | null;
  prevTicketInfo: TicketInfoRow | undefined;
}): { previousTicket: string | null; previousTicketStartCommit: string | null } {
  const { sessionDir, statePath, previousTicket, postState, postTicket, log } = input;
  const completedBoundary = previousTicket && postTicket !== previousTicket
    ? describeCompletedTicketBoundary({ ...input, previousTicket }, input.prevTicketInfo, postState, postTicket)
    : null;
  ...
  return { previousTicket: nextTicket, previousTicketStartCommit: resolveNextTicketStartCommit(input, nextTicket, lifecycleState) };
}
...
function recordCircuitBreakerIteration(input: {
  ...
  cbSettings: ReturnType<typeof loadSettings>;
  cbPath: string;
  statePath: string;
  sessionDir: string;
  state: State;
  ...
  iterLogFile: string;
  log: (msg: string) => void;
}): { cbState: CircuitBreakerState; tripped: boolean } {
...
/**
 * First-pass seed of the ticket the transition check compares against, and HEAD of its repo.
 * With no current ticket the prior start commit is handed back unchanged.
 */
function seedPreviousTicket(
  state: State,
  sessionDir: string,
  previousTicketStartCommit: string | null,
): { previousTicket: string | null; previousTicketStartCommit: string | null } {
  const previousTicket = state.current_ticket || null;
  if (!previousTicket) return { previousTicket, previousTicketStartCommit };
...
/**
 * FR-B3/B4/B12/B14 per-ticket timeout bookkeeping (the counter is loop-local, never persisted):
 * resolve the ticket the iteration ended on, advance the counter, and write the timeout stub for
 * a timed-out spawn. The caller assigns the counter and runs the `counterNext.halt` block.
 */
function advancePerTicketTimeoutCounter(input: {
  statePath: string;
  sessionDir: string;
  state: State;
  outcome: Awaited<ReturnType<typeof runIteration>>;
  iteration: number;
  iterLogFile: string;
  prev: { count: number; ticket: string | null };
}): { ticketForTimeout: string | null; counterNext: ReturnType<typeof applyTimeoutCounterForLoop> } {
  const { state, outcome } = input;
  let ticketForTimeout: string | null = state.current_ticket || null;
  try {
    const postState = readRunnerState(input.statePath);
  ...
  if (outcome.timedOut) {
    writeTimeoutStub(input.sessionDir, {
      ticketId: ticketForTimeout,
      iteration: input.iteration,
      wallSeconds: outcome.wallSeconds,
      workerTimeoutSeconds: Number(state.worker_timeout_seconds) || 0,
      timeoutCount: counterNext.count,
      logFile: input.iterLogFile,
    });
  }
  return { ticketForTimeout, counterNext };
}

/** R2c: a loop disposition a `runMuxRunnerMain` seam hands back so the loop body keeps its own `continue`/`break`. */
type MainLoopStep = { kind: 'continue' } | { kind: 'exit'; exitReason: ExitReason };
...
function resolveCodexManagerNoProgressStep(input: {
  ...
  pendingCount: number;
  log: (msg: string) => void;
}): { kind: 'relaunch' | 'recovery_exhausted' } | MainLoopStep {
  ...
  if (!noProgress.halt) return { kind: 'relaunch' };
  ...
  if (codexRecovery.kind === 'advanced') return { kind: 'continue' };
  if (codexRecovery.kind === 'recovery_exhausted') return { kind: 'recovery_exhausted' };
...
function routeCloserHandoffRecovery(input: {
  ...
  iteration: number;
  log: (msg: string) => void;
}): { kind: 'fall_through' } | MainLoopStep {
...
/** R2c: an idle-stall watchdog pass — the consecutive recovery count travels in and back out of the seam. */
type IdleStallWatchdogStep =
  | { kind: 'none' | 'recovered'; idleStallRecoveryCount: number }
  | { kind: 'exit'; exitReason: ExitReason };
...
function selfRecoverIdleStall(input: {
  ...
  curIter: number;
  idleSeconds: number;
  idleStallThresholdSeconds: number;
  idleStallRecoveryCount: number;
  idleStallRecoveryCap: number;
  log: (msg: string) => void;
}): IdleStallWatchdogStep {
...
function runIdleStallWatchdog(input: {
  ...
  try {
    ...
    if (!idleDecision.stalled) return { kind: 'none', idleStallRecoveryCount };
...
/** R2c: the EPIC_COMPLETED decisions that recover in-loop instead of finalizing or halting. */
type FalseEpicRecoveryDecision = Extract<EpicCompletionDecision, { kind: 'recover_advance' | 'recover_retry' }>;
...
function haltOnPersistentHallucination(input: {
  decision: Extract<EpicCompletionDecision, { kind: 'persistent_hallucination' }>;
  statePath: string;
  sessionDir: string;
  iterLogFile: string;
  log: (msg: string) => void;
...
function persistFalseEpicRecoveryState(input: {
  statePath: string;
  sessionDir: string;
  decision: FalseEpicRecoveryDecision;
  curState: State;
  recoveredCurrentTicket: string | null;
  log: (msg: string) => void;
}): void {
...
function markFinalTicketDoneBeforeFinalize(input: {
  ...
  if (!curState.current_ticket) return { kind: 'proceed' };
  const guard = guardCompletionCommitBeforeDone({
    sessionDir,
    ticketId: curState.current_ticket,
    // AP-EXT-ITER124-01: ladder, not `||` selection.
...
function relaunchOrEscapeAfterCleanManagerExit(input: {
  ...
  iteration: number;
  cbState: CircuitBreakerState | null;
  exitKind: Parameters<typeof evaluateManagerRelaunch>[3];
  attemptWindow: Parameters<typeof recordBoundedEscapeAttempt>[4];
  log: (msg: string) => void;
}): { kind: 'fall_through' } | MainLoopStep {
...
/** R2c: the once-per-run iteration-0 gate latches `runMuxRunnerMain` carries across passes. */
interface IterationZeroGateLatches {
  bundleBootstrapApplied: boolean;
  readinessGateChecked: boolean;
  ticketAuditGateChecked: boolean;
}
 ...
 */
function runIterationZeroGates(
  state: State,
  latches: IterationZeroGateLatches,
  input: { curIter: number; sessionDir: string; extensionRoot: string; log: (msg: string) => void },
): State {
 ...
 */
function advanceCommitPendingTracker(
  tracker: { lastObservedStateIteration: number; lastProgressOuterIteration: number },
  curIter: number,
  iteration: number,
): void {
...
/** 7c5a706b: how the top-of-iteration read resolves — retry after a 1s sleep, exit the loop, or run the pass on `state`. */
type IterationStartStep = { kind: 'retry' } | { kind: 'exit'; exitReason: ExitReason } | { kind: 'ready'; state: State };
 ...
 */
function readIterationStartState(input: {
  statePath: string;
  sessionDir: string;
  toolchainPreflight: { checked: boolean };
  log: (msg: string) => void;
}): IterationStartStep {
  const { statePath, sessionDir, log } = input;
  let state: State;
  try {
    state = readRunnerState(statePath);
  } catch (err) {
    const decision = classifyCapCheckReadError(err, sessionDir, log);
    return decision === 'continue' ? { kind: 'retry' } : { kind: 'exit', exitReason: 'error' };
  }
  ...
  if (!input.toolchainPreflight.checked) {
    input.toolchainPreflight.checked = true;
    if (haltOnMissingTargetToolchain(statePath, state.working_dir, log)) {
      return { kind: 'exit', exitReason: 'toolchain_unavailable' };
    }
  }
...
/** 7c5a706b: the circuit-breaker gate — if CB is OPEN, record `circuit_open`, deactivate, and hand back the exit. */
function exitOnOpenCircuitBreaker(
  cbEnabled: boolean,
  cbState: CircuitBreakerState | null,
  statePath: string,
  log: (msg: string) => void,
): ExitReason | null {
  if (!cbEnabled || !cbState || canExecute(cbState)) return null;
...
/**
 * 7c5a706b: stall detection fallback (only when CB is disabled). On `halt` the `stall` exit is already recorded and
 * the caller breaks without adopting the trackers; otherwise the caller adopts both.
 */
function evaluateStallFallback(input: {
  cbEnabled: boolean;
  curIter: number;
  lastStateIteration: number;
  stallCount: number;
  ...
  statePath: string;
  log: (msg: string) => void;
}): { halt: boolean; stallCount: number; lastStateIteration: number } {
  const { curIter, statePath, log } = input;
  if (input.cbEnabled) return { halt: false, stallCount: input.stallCount, lastStateIteration: input.lastStateIteration };
  let stallCount = input.stallCount;
  if (curIter === input.lastStateIteration) {
    stallCount++;
    if (stallCount >= 2) { // Stall threshold only consulted when !cbEnabled; CB-enabled sessions use CB's own progress threshold
      log(`WARNING: state.iteration has not advanced in 2 outer-loop iterations (stuck at ${input.state.iteration}). Exiting to avoid wasted API calls.`);
      recordExitReason(statePath, 'stall');
      safeDeactivate(statePath);
      return { halt: true, stallCount, lastStateIteration: input.lastStateIteration };
    }
  } else {
    stallCount = 0;
  }
  return { halt: false, stallCount, lastStateIteration: curIter };
}

/** 7c5a706b: the two best-effort iteration-start reapers (orphaned managers, then orphaned worker procs). */
function reapOrphansAtIterationStart(statePath: string, sessionDir: string, log: (msg: string) => void): void {
  try {
...
/** 7c5a706b: the R-WTB-A1 halt-block outcome; `snapshot` is the refreshed artifact-progress snapshot in every case. */
type TimeoutHaltStep = { kind: 'extended' | 'recovered' | 'halt'; snapshot: ArtifactProgressSnapshot };

interface TimeoutHaltInput {
  statePath: string;
  sessionDir: string;
  state: State;
  extensionRoot: string;
  iteration: number;
  ticketForTimeout: string | null;
  timeoutCount: number;
...
/**
 * 7c5a706b: R-WTB-A1 artifact-progress probe. If the worker produced new artifacts or commits within the no-progress
 * window, emit `ticket_timeout_progress_extension` and report `progressed` so the caller resets the counter.
 */
function probeTimeoutArtifactProgress(
  input: TimeoutHaltInput & { noProgressWindowS: number },
): { progressed: boolean; snapshot: ArtifactProgressSnapshot } {
  const { statePath, sessionDir, state, ticketForTimeout, timeoutCount, lastArtifactProgressSnapshot, noProgressWindowS, log } = input;
  const ticketDir = ticketForTimeout ? path.join(sessionDir, ticketForTimeout) : null;
  const scopeJsonPath = path.join(sessionDir, 'scope.json');
  if (!ticketDir || !fs.existsSync(ticketDir)) return { progressed: false, snapshot: lastArtifactProgressSnapshot };
  const pResult = detectArtifactProgress(ticketDir, lastArtifactProgressSnapshot, {
    workingDir: state.working_dir || sessionDir,
    scopeJsonPath,
  });
  const snapshot: ArtifactProgressSnapshot = { latestMtimeEpoch: pResult.latestMtimeEpoch, latestCommitSha: pResult.latestCommitSha };
  if (!pResult.progressed) return { progressed: false, snapshot };
...
/**
 * 7c5a706b: no progress inside the window — emit `ticket_timeout_halted_no_progress`, then route through the single
 * recovery choke point before the bare timeout park. `recovered` means the ladder advanced the ticket.
 */
function routeTimeoutNoProgress(
  input: TimeoutHaltInput & { noProgressWindowS: number; snapshot: ArtifactProgressSnapshot },
): 'recovered' | 'halt' {
  const { statePath, sessionDir, state, ticketForTimeout, timeoutCount, snapshot, log } = input;
  writeActivityEntry(statePath, {
    ...
    gate_payload: {
      timeout_count: timeoutCount,
      no_progress_window_seconds: input.noProgressWindowS,
      latest_mtime_epoch: snapshot.latestMtimeEpoch,
      latest_commit_sha: snapshot.latestCommitSha,
    },
  });
...
/** 7c5a706b: the whole `counterNext.halt` decision — probe for progress first, then the no-progress recovery route. */
function resolveTimeoutHaltStep(input: TimeoutHaltInput): TimeoutHaltStep {
  const noProgressWindowS = resolveNoProgressWindowSeconds();
  const probe = probeTimeoutArtifactProgress({ ...input, noProgressWindowS });
  if (probe.progressed) return { kind: 'extended', snapshot: probe.snapshot };
  const kind = routeTimeoutNoProgress({ ...input, noProgressWindowS, snapshot: probe.snapshot });
  return { kind, snapshot: probe.snapshot };
}

/** 7c5a706b: a manager-exit branch outcome — `relaunch` means reset the stall trackers, sleep 1s and continue. */
type ManagerExitLoopStep = { kind: 'relaunch' } | { kind: 'exit'; exitReason: ExitReason };

type LoopIterationOutcome = Awaited<ReturnType<typeof runIteration>>;

interface ManagerExitBranchInput {
  ...
  iteration: number;
  iterLogFile: string;
  runnerMaxTurns: Parameters<typeof classifyManagerRelaunchExit>[3];
  cbState: CircuitBreakerState | null;
  log: (msg: string) => void;
}

/** 7c5a706b: the `inactive` result branch — codex session-inactive relaunch, clean-exit relaunch/escape, else cancel. */
function handleInactiveIterationResult(
  input: ManagerExitBranchInput & { attemptWindow: Parameters<typeof recordBoundedEscapeAttempt>[4] },
): ManagerExitLoopStep {
  const { outcome, state, statePath, sessionDir, extensionRoot, iteration, iterLogFile, log } = input;
  if (detectManagerInactiveExit(outcome)) {
    let postState: State = state;
    try { postState = readRunnerState(statePath); } catch { /* fall back */ }
    const inactiveExitKind = classifyManagerRelaunchExit(postState, outcome, iterLogFile, input.runnerMaxTurns);
    if (inactiveExitKind === 'codex_session_inactive') {
      const inactiveDecision = evaluateManagerRelaunch(postState, collectTickets(sessionDir), input.cbState, inactiveExitKind);
      ...
      if (inactiveDecision.shouldRelaunch) {
        ...
        if (codexStep.kind === 'exit') return { kind: 'exit', exitReason: codexStep.exitReason };
        if (codexStep.kind === 'continue') return { kind: 'relaunch' };
        const relaunchBackend = resolveBackendFromStateFileWithSource(statePath).backend;
    ...
    if (inactiveExitKind !== 'codex_session_inactive') {
      const cleanExitStep = relaunchOrEscapeAfterCleanManagerExit({
        postState, state, statePath, sessionDir, extensionRoot, iteration, cbState: input.cbState, exitKind: inactiveExitKind, log,
        attemptWindow: input.attemptWindow,
      });
      if (cleanExitStep.kind === 'exit') return { kind: 'exit', exitReason: cleanExitStep.exitReason };
      if (cleanExitStep.kind === 'continue') return { kind: 'relaunch' };
    }
  }
...
/**
 * 7c5a706b: the `error` result branch. Codex tmux_mode runs ONE long-lived manager subprocess that loops
 ...
 */
function handleErrorIterationResult(input: ManagerExitBranchInput): ManagerExitLoopStep {
  const { outcome, state, statePath, sessionDir, extensionRoot, iteration, iterLogFile, log } = input;
  let postState: State = state;
  try { postState = readRunnerState(statePath); } catch { /* fall back */ }
  const exitKind = classifyManagerRelaunchExit(postState, outcome, iterLogFile, input.runnerMaxTurns);
  ...
  safeDeactivate(statePath);
  removeRunnerSessionMapEntry(statePath, log);
  return { kind: 'exit', exitReason: 'error' };
}

/**
 * 7c5a706b: how an EPIC_COMPLETED / TASK_COMPLETED claim resolves before the finalize: `park` continues at once,
 * `retry` resets the stall trackers, sleeps and continues, `finalize` proceeds on the re-read state.
 */
type TaskCompletedClaim =
  | { kind: 'exit'; exitReason: ExitReason }
  | { kind: 'park' }
  | { kind: 'retry' }
  | { kind: 'finalize'; curState: State };

/** 7c5a706b: the pre-finalize half of the `task_completed` branch. */
function resolveTaskCompletedClaim(input: {
  state: State;
  statePath: string;
  sessionDir: string;
  iteration: number;
  ...
  const { state, statePath, sessionDir, iteration, iterLogFile, log } = input;
  let curState: State;
  try {
    ...
    return { kind: 'exit', exitReason: 'success' };
  }
  ...
  if (decision.kind === 'persistent_hallucination') {
    return { kind: 'exit', exitReason: haltOnPersistentHallucination({ decision, statePath, sessionDir, iterLogFile, log }) };
  }

  if (decision.kind === 'recover_advance' || decision.kind === 'recover_retry') {
    const falseEpicStep = recoverFalseEpicCompletion({ decision, curState, state, statePath, sessionDir, iteration, iterLogFile, log });
    return falseEpicStep.parked ? { kind: 'park' } : { kind: 'retry' };
  }

  if (markFinalTicketDoneBeforeFinalize({ curState, state, statePath, sessionDir, log }).kind === 'continue') return { kind: 'park' };
  return { kind: 'finalize', curState };
}

/** 7c5a706b: how a review_clean claim resolves before the finalize; `below_min` falls through to the loop's sleep. */
type ReviewCleanClaim = { kind: 'exit' } | { kind: 'below_min' } | { kind: 'finalize'; curState: State };

/** 7c5a706b: the pre-finalize half of the `review_clean` branch — state re-read and the min_iterations gate. */
function resolveReviewCleanClaim(input: {
  state: State;
  statePath: string;
  sessionDir: string;
  iteration: number;
  ...
  if (minIter > 0 && curIterNow < minIter) {
    ...
    return { kind: 'below_min' };
  }
  return { kind: 'finalize', curState };
}

/**
 * 7c5a706b: the pre-spawn gates and probes that precede the preskip check — iteration-0 gates, the multi-repo
 * advisory, the commit-pending tracker + probe, and the R-CNAR-6 spark smoke-run gate. `halt` means the smoke gate
 * stopped the run with `codex_unhealthy_consecutive_failures` already recorded.
 */
function runPreSpawnGatesAndProbes(input: {
  state: State;
  statePath: string;
  sessionDir: string;
  extensionRoot: string;
  ...
  commitPendingProbeThreshold: Parameters<typeof runPreSpawnCommitPendingProbe>[0]['threshold'];
  smokeGateBypassEmitted: boolean;
  log: (msg: string) => void;
}): { state: State; smokeGateBypassEmitted: boolean; halt: boolean } {
  const { statePath, sessionDir, extensionRoot, curIter, iteration, commitPendingTracker, log } = input;
  const state = runIterationZeroGates(input.state, input.iterationZeroGates, { curIter, sessionDir, extensionRoot, log });
  ...
  runPreSpawnCommitPendingProbe({
    ...
    threshold: input.commitPendingProbeThreshold,
    log,
  });
  ...
  const smoke = applySparkSmokeGateAtIteration(state, statePath, sessionDir, input.smokeGateBypassEmitted, log);
  return { state, smokeGateBypassEmitted: smoke.bypassEmitted, halt: smoke.halt };
}

/** 7c5a706b: circuit-breaker recording — only on a CB-enabled run, and skipped for subprocess failures. */
function maybeRecordCircuitBreakerIteration(
  input: Omit<Parameters<typeof recordCircuitBreakerIteration>[0], 'cbState'> & {
    cbEnabled: boolean;
    cbState: CircuitBreakerState | null;
    result: LoopIterationOutcome['completion'];
  },
): { cbState: CircuitBreakerState | null; tripped: boolean } {
  const { cbEnabled, cbState, result, ...recordInput } = input;
  if (!cbEnabled || !cbState || result === 'error' || result === 'inactive') return { cbState, tripped: false };
  return recordCircuitBreakerIteration({ ...recordInput, cbState });
}

/**
 * 7ac3038c: how one wmw-auto-skip stage resolves. `continue` advances the loop (`resetStall` says whether the loop
 * resets its stall trackers first, exactly as the inline arm did); `exit` ends the run with the reason already
 * recorded; `fall_through` hands off to the next stage.
 */
type WmwAutoSkipStep =
  | { kind: 'continue'; resetStall: boolean }
  | { kind: 'exit'; exitReason: ExitReason }
  | { kind: 'fall_through' };

/** 7ac3038c: the loop facts every wmw-auto-skip stage reads. */
interface WmwAutoSkipInput {
  state: State;
  statePath: string;
  sessionDir: string;
  extensionRoot: string;
  iteration: number;
  apTicketId: string;
  apProgressResult: NonNullable<ReturnType<typeof recordPostSpawnWorkerProgress>['apProgressResult']>;
  skipK: number;
  iterStartMs: ReturnType<typeof snapshotPreSpawnIteration>['iterStartMs'];
  preIterSha: ReturnType<typeof snapshotPreSpawnIteration>['preIterSha'];
  log: (msg: string) => void;
}
 ...
 */
function routeWmwRecoveryLadder(input: WmwAutoSkipInput): WmwAutoSkipStep {
 ...
 */
function routeWmwFailedFlipSuppression(input: WmwAutoSkipInput): WmwAutoSkipStep {
...
function flipWmwTicketFailed(input: WmwAutoSkipInput): void {
 ...
 */
function runWmwAutoSkip(input: WmwAutoSkipInput): Exclude<WmwAutoSkipStep, { kind: 'fall_through' }> {
  const ladderStep = routeWmwRecoveryLadder(input);
  if (ladderStep.kind !== 'fall_through') return ladderStep;
  const suppressionStep = routeWmwFailedFlipSuppression(input);
  if (suppressionStep.kind !== 'fall_through') return suppressionStep;
  flipWmwTicketFailed(input);
  return { kind: 'continue', resetStall: false };
}

/**
 * 7ac3038c: how the iteration-head lifecycle advance resolves. `progressEpochMs` is the R-MWIS-2 forward-progress
 * mark, taken immediately after the lifecycle write — the point where the loop used to take it.
 */
type IterationLifecycleStep =
  | { kind: 'exit'; exitReason: ExitReason }
  | { kind: 'ready'; state: State; preTicket: ReturnType<typeof resolvePreTicket>; progressEpochMs: number };
...
/** 7ac3038c: how the iteration-head closer check resolves; `continue` restarts the pass after a ladder recovery. */
type CloserTerminalStep = { kind: 'exit'; exitReason: ExitReason } | { kind: 'continue' } | { kind: 'ready'; state: State };
...
function resolveCloserTerminalStep(input: {
  ...
  if (closerDecision.action === 'exit') {
    ...
    if (closerDecision.reason === 'closer_handoff_terminal') {
      ...
      if (closerRecovery.kind === 'exit') return { kind: 'exit', exitReason: closerRecovery.exitReason };
    }
...
function exitOnFinishedRoster(input: {
  ...
  sessionDir: string;
  iteration: number;
  preTicket: ReturnType<typeof resolvePreTicket>;
  log: (msg: string) => void;
}): ExitReason | null {
  const { state, statePath, sessionDir, iteration, preTicket, log } = input;
  if (applyAllTicketsDoneCompletion(statePath, sessionDir, iteration, log, state.working_dir || '')) return 'success';
 ...
 */
function cpuLivenessWaitGates(
  state: State,
  sessionDir: string,
  cbEnabled: boolean,
  cbState: CircuitBreakerState | null,
): Pick<
  Parameters<typeof evaluateCpuLivenessWatchdog>[0],
  'rateLimitWaiting' | 'circuitBreakerExecutable' | 'lastError' | 'consecutiveSubprocessErrors'
> {
  return {
    rateLimitWaiting: rateLimitParkStillLive(sessionDir),
    circuitBreakerExecutable: !cbEnabled || !cbState || canExecute(cbState),
...
function treatRunIterationThrowAsSpawnError(log: (msg: string) => void) {
  return (err: unknown): Awaited<ReturnType<typeof runIteration>> => {
    const msg = err instanceof Error ? err.message : String(err);
...
function bindWastedIterEmitter(input: {
  sessionDir: string;
  iteration: number;
  action: ReturnType<Parameters<typeof createWastedIterEmitter>[0]>['action'];
  preIterSha: ReturnType<typeof snapshotPreSpawnIteration>['preIterSha'];
  iterWorkingDir: ReturnType<typeof snapshotPreSpawnIteration>['iterWorkingDir'];
  apProgressResult: ReturnType<typeof recordPostSpawnWorkerProgress>['apProgressResult'];
  apBeforeCount: ReturnType<typeof snapshotPreSpawnIteration>['apBeforeCount'];
}): ReturnType<typeof createWastedIterEmitter> {
...
/**
 * 7ac3038c: how the post-iteration ticket-transition check resolves. `continue` parks this iteration (the loop records
 * its wasted_iter verdict and continues); `return` leaves `runMuxRunnerMain` exactly as the inline `return` did;
 * `proceed` carries the previous-ticket pair forward — unchanged when state.json could not be read.
 */
type TicketTransitionStep =
  | { kind: 'continue' }
  | { kind: 'return' }
  | { kind: 'proceed'; previousTicket: string | null; previousTicketStartCommit: string | null };
...
function resolveTicketTransition(input: {
  ...
  try {
    ...
    let prevTicketInfo: TicketInfoRow | undefined;
    if (previousTicket && postTicket !== previousTicket) {
      ...
      if (isModelAttestedDone(sessionDir, prevTicketInfo)) {
        ...
        if (verdict.kind === 'continue') return { kind: 'continue' };
        if (verdict.kind === 'return') return { kind: 'return' };
      } else {
...
/** 08394aa1: the main loop's stall trackers, shared by reference with the seams that reset them. */
interface MuxStallTrackers {
  lastStateIteration: number;
  stallCount: number;
  /** R-MWIS-2: bumped on every forward-progress marker; the idle-stall watchdog reads it before each spawn. */
  lastProgressEpoch: number;
}

/**
 * 08394aa1: how the iteration head resolves. `exit` ends the loop, `continue` starts the next pass with the stall
 * trackers already reset, `ready` hands the loop this pass's state. `iteration` is the value the loop held at that point.
 */
type IterationHeadStep =
  | { kind: 'exit'; exitReason: ExitReason; iteration: number }
  | { kind: 'continue'; iteration: number }
  | { kind: 'ready'; iteration: number; curIter: number; state: State; preTicket: ReturnType<typeof resolvePreTicket> };
...
function resolveIterationHead(input: {
  ...
  if (limitExit) return { kind: 'exit', exitReason: limitExit, iteration: curIter };
  ...
  if (stall.halt) return { kind: 'exit', exitReason: 'stall', iteration: curIter };
  ...
  if (lifecycle.kind === 'exit') return { kind: 'exit', exitReason: lifecycle.exitReason, iteration };
  ...
  if (closerStep.kind === 'exit') return { kind: 'exit', exitReason: closerStep.exitReason, iteration };
...
/**
 * 08394aa1: the C6 (B-MRSW) CPU/artifact-liveness window anchor. Seeded per OBSERVED WORKER PID (not per ticket — one
 * ticket spans many worker pids); the delta is only evaluated once the window reaches the idle-stall threshold. NOT
 * persisted to state.json — pure loop-local liveness truth.
 */
interface CpuLivenessAnchor {
  cpuLivenessTicketId: string | null;
  cpuLivenessAnchorPid: number | null;
  cpuLivenessAnchorEpoch: number;
  cpuLivenessAnchorCpuSeconds: number | null;
  cpuLivenessAnchorMtimeMs: number;
}

/** 08394aa1: `exit` ends the loop, `recovered` starts the next pass with the trackers reset, `proceed` goes on to spawn. */
type CpuLivenessWatchdogStep = { kind: 'exit'; exitReason: ExitReason } | { kind: 'recovered' } | { kind: 'proceed' };

interface CpuLivenessWatchdogSeamInput {
  state: State;
  statePath: string;
  sessionDir: string;
  extensionRoot: string;
...
interface CpuLivenessWindowInput extends CpuLivenessWatchdogSeamInput {
  cpuTicket: string;
  workerPid: number | null;
  windowSeconds: number;
  cpuSecondsDelta: number;
 ...
 */
function runCpuLivenessWatchdog(input: CpuLivenessWatchdogSeamInput): CpuLivenessWatchdogStep {
  const { state, sessionDir, idleStallThresholdSeconds, anchor, log } = input;
  const muxNow = input.now;
  const {
    cpuLivenessTicketId, cpuLivenessAnchorPid, cpuLivenessAnchorEpoch, cpuLivenessAnchorCpuSeconds, cpuLivenessAnchorMtimeMs,
  } = anchor;
  try {
 ...
 */
function runCpuLivenessWindow(input: CpuLivenessWindowInput): CpuLivenessWatchdogStep {
  const {
    state, statePath, sessionDir, extensionRoot, iteration, curIter, idleStallThresholdSeconds, cbEnabled, cbState,
    anchor, stallTrackers, cpuTicket, workerPid, windowSeconds, log,
  } = input;
  const muxNow = input.now;
  ...
  if (!cpuDecision.stalled) return { kind: 'proceed' };
 ...
 */
function resolveMuxRunExitCode(input: RunTerminalReportInput): number {
...
/**
 * R2b: the three effects the loop cannot perform in-process — the manager spawn, the inter-iteration wall sleep and
 * process termination. `runMuxRunnerMain` destructures them under their original names, so the body text is unchanged.
 */
interface MuxRunnerMainDeps {
...
export const MUX_RUNNER_MAIN_DEPS: MuxRunnerMainDeps = { runIteration, sleep, exit: (code) => process.exit(code) };

/** R2b: drives the real mux loop with caller-supplied seams, so tests can observe its dispositions (the loop stays unexported). */
export function driveMuxRunnerMain(deps: MuxRunnerMainDeps): Promise<void> {
...
async function runMuxRunnerMain({ runIteration, sleep, exit }: MuxRunnerMainDeps) {
  ...
  while (true) {
    ...
    const { iterWorkingDir, preIterSha, iterStartMs, apTicketId, apCreditEarlyPhases, apBeforeCount } =
      snapshotPreSpawnIteration(state, sessionDir);
    ...
    const outcome: Awaited<ReturnType<typeof runIteration>> =
      await runIteration(sessionDir, iteration, extensionRoot).catch(treatRunIterationThrowAsSpawnError(log));
    const result = outcome.completion;
    ...
    if (transition.kind === 'return') return;
    ...
    if (result === 'task_completed') {
      ...
      const reviewClaim = resolveReviewCleanClaim({ state, statePath, sessionDir, iteration, log });
      if (reviewClaim.kind === 'exit') {
        exitReason = 'success';
        break;
      }
      if (reviewClaim.kind === 'finalize') {
