...
import { State, Defaults, MicroverseExitReason, UNBOUNDED_READ_MAX_BUFFER, enumerationCompleted, normalizeMicroverseExitReason } from '../types/index.js';
import type { ActivityEventType, MetricMeasurementPhase, Backend, IterationExitType, MicroverseSessionState, MicroverseHistoryEntry, ViolationLedger, FailureClass, GateResult, GateFailure, GateBaselineFile, StallClassification, StallRecoveryAction, JudgeResult, Violation, PickleSettings, RateLimitPark, RateLimitAction, RateLimitProbeVerdict } from '../types/index.js';
...
type JudgeMeasurementFailureExitReason = Extract<ExitReason, 'judge_timeout' | 'judge_cli_missing' | 'metric_unmeasurable_unrecoverable' | 'metric_unmeasurable_transient' | 'all_judge_backends_exhausted'>;
...
export interface RunContext {
  ...
  /** Cumulative rate-limit park ceiling (minutes), resolved from `pickle_settings.json:rate_limit.max_park_minutes` (R-MVPARK). Optional so pre-existing test-fixture RunContext literals outside this ticket's scope fence stay valid; production always sets it via buildRunContext. */
...
export const _deps = {
  ...
  runGate: runGate as typeof runGate,
 ...
 */
let testBackendOverridePath: string | null = null;

/** The final report's executor line: a positive claim in BOTH directions, never an absence. */
function describeIterationExecutor(): string {
...
export async function applyTestBackendOverrideFromEnv(): Promise<boolean> {
  ...
  process.stderr.write(
    `microverse-runner: iterations executed by PICKLE_TEST_BACKEND_PATH override: ${resolvedPath}\n`,
  );
  return true;
}
 ...
 */
export function selectAllowedPathsForPrompt(
  ...
  for (const p of allowedPaths) {
    ...
    if (shown.length > 0 && bytes + lineBytes > MAX_ALLOWED_PATHS_PROMPT_BYTES) break;
    shown.push(p);
    bytes += lineBytes;
  }
...
export function selectLedgerEntriesForPrompt(
  ledger: ViolationLedger[] | null | undefined,
): ViolationLedger[] {
...
export function buildJudgePrompt(input: JudgePromptInput): string {
  ...
  parts.push(
    ...
    '`resolved`/`new`/`remaining` hold violation ids relative to the prior-violations list above; when there is no such list, `resolved` and `remaining` are `[]` and every id goes in `new`.',
 ...
 */
function killSpawnedSubtree(child: ChildProcess, signal: NodeJS.Signals): void {
  const pid = child.pid;
...
type JudgeBackoffContext = {
  ...
  settings: PickleSettings | null;
  primaryWorkerBackend: Backend;
  probeKind: ProbeJudgeResult['kind'];
  isNested: boolean;
...
type JudgeBackoffState = {
  ...
  workerFallbackActivated: boolean;
  lastError: string | null;
  exhaustedFailureKind: JudgeMeasurementExhaustedFailureKind;
...
// pre_spawn_env_key_names telemetry, so the XDG_RUNTIME_DIR buildJudgeEnv creates for it is
// removed immediately rather than left leaking (do NOT change `isNested` or skip this call —
// the key NAMES it produces are the R-SJET-3 observable and must stay identical).
function collectJudgePreSpawnEnvKeyNames(isNested: boolean): string[] {
  ...
  return preSpawnEnvKeyNames;
}

function classifyJudgeAttemptTimeout(
  failureKind: JudgeMeasurementAttempt['failureKind'],
  probeKind: ProbeJudgeResult['kind'],
): 'probe_timeout' | 'attempt_timeout' | null {
  if (failureKind !== 'timeout') return null;
  return probeKind === 'timeout' ? 'probe_timeout' : 'attempt_timeout';
}

function emitJudgeAttemptTelemetry(
  ctx: JudgeBackoffContext,
  state: JudgeBackoffState,
  result: JudgeMeasurementAttempt,
  elapsedMs: number,
): void {
  const attemptActivity = ctx.attemptActivity;
  if (!attemptActivity) return;
  try {
    _deps.logActivity({
      ...
      backend: state.attemptBackend,
      judge_backend: 'claude',
      model: ctx.judgeModel || DEFAULT_JUDGE_MODEL,
      fallback_activated: state.workerFallbackActivated || ctx.primaryWorkerBackend !== 'claude' || ctx.probeKind === 'timeout',
      spawn_context: attemptActivity.spawnContext,
      gate_payload: {
        attempt: state.totalAttempts,
        elapsed_ms: elapsedMs,
        outcome: result.metric ? 'success' : result.failureKind,
        timeout_class: classifyJudgeAttemptTimeout(result.failureKind, ctx.probeKind),
...
function emitBaselineAttemptTimeout(
  attemptActivity: JudgeAttemptActivity | undefined,
  attempt: number,
  elapsedMs: number,
): void {
...
// Precedence ladder for the kind reported once every backoff round is spent:
// `failed` outranks `timeout` outranks `rate_limited`, and a kind never demotes.
function resolveExhaustedFailureKind(
  current: JudgeMeasurementExhaustedFailureKind,
  failureKind: JudgeMeasurementAttempt['failureKind'],
): JudgeMeasurementExhaustedFailureKind {
  if (failureKind === 'failed') return 'failed';
  if (failureKind === 'timeout' && current !== 'failed') return 'timeout';
  if (failureKind === 'rate_limited' && current !== 'failed' && current !== 'timeout') return 'rate_limited';
  return current;
}

function maybeActivateWorkerFallback(ctx: JudgeBackoffContext, state: JudgeBackoffState, result: JudgeMeasurementAttempt): void {
  if (state.workerFallbackActivated) return;
  ...
  state.workerFallbackActivated = true;
  state.attemptBackend = fallbackBackend;
  persistWorkerIterationFallback(ctx.attemptActivity, fallbackBackend);
}

function judgeCliMissingResult(attempts: number, lastError: string | null): JudgeMeasurementResult {
...
function judgeExhaustedResult(state: JudgeBackoffState): JudgeMeasurementResult {
  return {
    metric: null,
    exitReason: state.workerFallbackActivated ? 'all_judge_backends_exhausted' : 'judge_timeout',
    attempts: state.totalAttempts,
    lastError: state.lastError,
    exhaustedFailureKind: state.exhaustedFailureKind,
  };
}

function createJudgeBackoffState(backend: Backend, probeKind: ProbeJudgeResult['kind']): JudgeBackoffState {
  return {
    attemptBackend: backend,
    workerFallbackActivated: false,
    lastError: null,
    exhaustedFailureKind: probeKind === 'failed' ? 'failed' : 'rate_limited',
    totalAttempts: 0,
  };
}

// One full backoff round: every attempt plus the sleeps between them. Returns the terminal
// JudgeMeasurementResult when the round settles the call (a measurement, or a missing CLI),
...
async function runJudgeBackoffRound(
  ctx: JudgeBackoffContext,
  state: JudgeBackoffState,
  cumulativeParkedMs: number,
  ...
  for (let attempt = 0; attempt <= ctx.backoffsMs.length; attempt++) {
    ...
    if (result.metric) {
      if (cumulativeParkedMs > 0) { clearMetricParkWait(ctx.attemptActivity); }
...
// Rounds separated by rate-limit parks. Every exit funnels through one place so the park-wait
// file is cleared exactly once, on whichever path ends the call.
async function runJudgeBackoffLoop(ctx: JudgeBackoffContext, state: JudgeBackoffState): Promise<JudgeMeasurementResult> {
  let cumulativeParkedMs = 0;
  while (true) {
    const terminal = await runJudgeBackoffRound(ctx, state, cumulativeParkedMs);
    if (terminal) return terminal;
    if (state.exhaustedFailureKind !== 'rate_limited') break;
    const remainingMs = _deps.metricParkMaxMs - cumulativeParkedMs;
    if (remainingMs <= 0) break;
    const parkMs = Math.min(_deps.metricParkWaitMs, remainingMs);
    emitMetricParkWait(ctx.attemptActivity, parkMs, cumulativeParkedMs);
...
export async function measureLlmMetricWithBackoff(
  ...
  const preSpawnEnvKeyNames = collectJudgePreSpawnEnvKeyNames(isNested);
  const probe = await probeJudgeBackendAvailability('claude', cwd);
  if (probe.kind === 'missing') return judgeCliMissingResult(0, probe.message);

  const backoffsMs = [10_000, 30_000, 60_000];
  const ctx: JudgeBackoffContext = {
    goal,
    timeoutSeconds,
    cwd,
    judgeModel,
...
function appendViolationLedgerHandoff(parts: string[], mvState: MicroverseSessionState): void {
 ...
 */
function isRenderableCitadelFinding(value: unknown): value is CitadelFinding {
...
): Exclude<JudgeMeasurementFailureExitReason, 'metric_unmeasurable_transient'> {
...
async function measureLlmBaseline(
  ...
  const phase: MetricMeasurementPhase = 'baseline';
...
export async function measureLlmIteration(
  ...
  const phase: MetricMeasurementPhase = 'iteration';
  const error = measured.lastError ?? `${exitReason} after ${measured.attempts} attempt(s)`;
...
export function applyLedgerPartialProgress(
  ...
  priorEntries: readonly LedgerFigureEntry[] | undefined,
  currentEntries: readonly LedgerFigureEntry[] | undefined,
  committedPaths: () => ReadonlySet<string> | null,
): ReportedComparison {
  ...
  let touched: ReadonlySet<string> | null | undefined;
...
function committedPathSet(ctx: Pick<RunContext, 'workingDir' | 'preIterSha' | 'postIterSha'>): ReadonlySet<string> | null {
  if (!ctx.preIterSha || !ctx.postIterSha) return null;
 ...
 */
function listOwnedDirtyPaths(ctx: RunContext): string[] {
...
const CAP_GATE_CHECKS = ['typecheck', 'lint'] as const;
...
function resolveMaxParkMinutes(ctx: RunContext): number {
...
function readPriorRateLimitPark(statePath: string): RateLimitPark | null {
  return readRunnerState(statePath).rate_limit_park ?? null;
}

function parkCumulativeMs(priorPark: RateLimitPark | null): number {
...
function buildRateLimitParkArm(
...
export async function handleRateLimitExit(
  state: MicroverseState,
  ctx: RunContext,
  exitResult: ClassifiedIterationExit,
...
export function classifyMicroverseDisposition(exitReason: string): MicroverseDisposition {
  ...
  return (MICROVERSE_DISPOSITIONS as Record<string, MicroverseDisposition>)[current] ?? DEFAULT_MICROVERSE_DISPOSITION;
}
