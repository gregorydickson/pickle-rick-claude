...
// C4: codegraph index at session setup. Exported for integration tests.
export type CodegraphIndexAtSetupDeps = CodegraphDeps;

function cgApplyGitExclude(workingDir: string): void {
  ...
  if (existing.split('\n').map((l) => l.trim()).includes('.codegraph/')) return;
 ...
 */
function cgResolveIndexAction(
  workingDir: string,
  dbPath: string,
  staleMs: number,
  getCurrentHeadSha: (workingDir: string) => string | null = defaultGetHeadSha,
): 'full' | 'sync' | 'noop' {
  let mtimeMs: number;
  try {
  ...
  if (!indexedSha || !currentSha || indexedSha !== currentSha) return 'full';
  const ageMs = Date.now() - mtimeMs;
  return ageMs >= staleMs ? 'sync' : 'noop';
}

function cgMakeEmit(injected: CodegraphDeps['emit']): (evt: CodegraphEmitEvent) => void {
  if (injected) return injected;
  return (evt) => {
    const p: Parameters<typeof logActivity>[0] = { event: evt.event, source: 'pickle', ts: evt.ts };
    if (evt.reason) p.reason = evt.reason;
    if (evt.error) p.error = evt.error;
...
export async function runCodegraphIndexAtSetup(
  workingDir: string,
  settings: CodegraphSettings,
  _isResume: boolean,
  deps: CodegraphIndexAtSetupDeps = {},
  env: NodeJS.ProcessEnv = process.env,
): Promise<void> {
  if (env['PICKLE_CODEGRAPH'] === 'off') return;
  if (!settings.enabled || !settings.index_at_setup) return;

  try { cgApplyGitExclude(workingDir); } catch { /* best-effort */ }

  const dbPath = deps.dbPath ?? path.join(workingDir, '.codegraph', 'codegraph.db');
  const indexAction = cgResolveIndexAction(
    workingDir,
    dbPath,
    settings.staleness_max_age_minutes * 60_000,
    deps.getHeadSha ?? defaultGetHeadSha,
  );
  if (indexAction === 'noop') return;
 ...
 */
export function resolveClaudeVersionForSetup(): string | null {
  try {
...
function requireFlagValue(args: string[], index: number, errorMessage: string): string {
  const value = args[index + 1];
  if (!value || value.startsWith('--')) die(errorMessage);
  return value;
}

function parseBackendFlag(args: string[], index: number, flagName: string): Backend {
...
function applyResumeConfig(
  ...
  if (claudeVersionSeen) s.claude_version_seen = claudeVersionSeen;
  ...
  if (!s.start_commit) {
    if (s.pinned_sha) {
      ...
      process.stderr.write(
        '[setup] WARNING: start_commit still unresolved on --resume (cwd is not a git repository) — ' +
 ...
 */
export function readTicketWorkerGateTestsVerdict(sessionRoot: string, ticketId: string): 'green' | 'red' | 'not_run' | null {
  try {
...
function shaDescendsFromHead(workingDir: string, currentHead: string, sha: string): boolean {
  ...
  if (isHeadAtOrBelowCommit(currentHead, sha, workingDir) !== true) return false;
...
function scaffoldInTreeSessionNotes(sessionId: string): void {
  ...
  try {
    if (!fs.existsSync(inTreeSessionDir)) { fs.mkdirSync(inTreeSessionDir, { recursive: true }); }
 ...
 */
function assertTmuxBuildLoopRequired(config: SetupArgs): void {
  if (config.tmuxMode || config.pausedMode) return;
  if (config.teamsMode) {
    die(
      '/pickle --teams (in-session Teams Mode) was removed. ' +
      'Use /pickle-tmux --teams to run Teams Mode under tmux (morty-phase-* subagents preserved).',
    );
  }
  die(
    'The in-session /pickle build loop was removed (no /clear between iterations). ' +
...
export function scanPausedOrphans(sessionsRoot: string, config: SetupArgs, smInstance: StateManager): void {
  ...
  for (const entry of entries) {
    ...
    if (now - mtime <= PAUSED_ORPHAN_MIN_AGE_MS) continue;
