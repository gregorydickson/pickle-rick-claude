...
import {
  reconcileTicketTruth,
  type TicketTruth,
} from './reconcile-ticket-truth.js';
...
export interface SalvageOutcome {
  ...
  /** Commit sha for `committed-done` / `ff-reattached`. */
  sha?: string;
  /** True when a dirty diff was archived before a reset. */
  archived?: boolean;
  reason: string;
}
...
export interface SalvageTicketInput {
  ...
  workingDir: string;
  ticketId: string;
  /** Base commit for HEAD-regression detection (session start_commit). */
  startCommit?: string | null;
  /** Frontmatter completion sha, when the seam already holds it. */
  completionCommitSha?: string | null;
  log?: (msg: string) => void;
}

/**
 * Injectable seams so `salvageTicket` is exercised across the full disposition
 * matrix WITHOUT a real git repo. Production wires the existing git-utils +
 * mux-runner adapters; the matrix test injects fakes.
 */
export interface SalvageDeps {
  /** Ground-truth read (tree-state + ticket statuses). */
  reconcile: (input: { sessionDir: string; workingDir: string }) => TicketTruth;
  /** Gate verdict for the in-scope tree. */
  gate: (input: SalvageTicketInput) => GateVerdict;
  /** Commit SCOPED ticket-owned paths + flip Done; returns the real sha (never `add -A`). */
  commitScoped: (input: SalvageTicketInput) => { committed: boolean; sha?: string };
  /** Archive the dirty diff (archive-before-destructive). Returns the patch result. */
  archive: (input: SalvageTicketInput) => ArchiveResult | null;
  /** Reset the ticket frontmatter to Todo (no git mutation here). */
  resetTodo: (input: SalvageTicketInput) => void;
  /**
   * Auto-ff-reattach an orphaned commit when HEAD regressed off a committed
   * ticket. Returns the reattached tip sha, or null when not a regression /
   * not reattachable.
   */
  ffReattach: (input: SalvageTicketInput) => { recovered: boolean; sha?: string | null };
  /**
   * Clean-tree back-fill: a worker committed attributable green work then died
   * BEFORE flipping the ticket Done (no `completion_commit` stamp). The
   * attribution sha is resolved by the production caller (an already-permitted
   * completion-evidence oracle caller) and carried in
   * `input.completionCommitSha`; this dep performs the actual `status: Done` +
   * `completion_commit:` frontmatter write. Default is inert so the matrix test
   * and any non-attributable clean tree stay a `no-op`. Returns the stamped sha,
   * or null when nothing was back-filled. Optional: seams that never see a
   * clean-tree-with-attribution case (exit-path, bounded-escape) omit it.
   */
  backfillDone?: (input: SalvageTicketInput, sha: string) => { done: boolean; sha?: string | null };
}

const defaultDeps: SalvageDeps = {
