 ...
 */
function resolveStateFileFromSessionsDir(dataDir: string, rejected: string | null): string | null {
  ...
  return selectScannedStateFile(
    entries
      ...
      .filter((stateFile) => rejectedKey === null || normalizeWorkingDir(stateFile) !== rejectedKey),
    process.cwd(),
  );
}
...
export function resolveStateFile(dataDir: string): string | null {
  ...
  let rejectedOrphanStateFile: string | null = null;
