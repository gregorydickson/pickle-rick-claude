 ...
 */
function pathEntersGitDir(filePath: string): boolean {
  if (!filePath) return false;
...
function findWriteTargetInScope<T>(
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (!names.some((name) => anchorWritesPositionalArg(name, argsInScope))) continue;
 ...
 */
function detectBashStateWriteTarget(command: string, stateFile: string | null): { matched: string; isSettings: boolean } | null {
 ...
 */
function detectTargetedStateFile(input: PreToolUseInput, stateFile: string | null): { matched: string; isSettings: boolean } | null {
  const toolName = input.tool_name || '';
  const filePath = input.tool_input?.file_path || '';
  const command = input.tool_input?.command || '';
 ...
 */
function gitConfigValueNamesProhibitedOp(value: string): boolean {
  return detectProhibitedGitVerb(`git ${value}`, false) !== null;
}

function configValueRunsProhibitedCommand(tokens: readonly ShellToken[]): boolean {
  ...
  return toolDeliveredValues(tokens).some(value =>
    ...
    gitConfigValueNamesProhibitedOp(value)
 ...
 */
function findGitVerb(command: string, readValues: boolean): { verb: string; afterVerb: string[] } | null {
  const tokens = tokenizeShellTokens(command);
  ...
  if (readValues && configValueRunsProhibitedCommand(tokens)) return { verb: GIT_CONFIG_VERB, afterVerb: [] };
  let firstBare = -1;
  ...
  if (segmentDefinesGitAlias(tokens)) return { verb: GIT_CONFIG_VERB, afterVerb: [] };
...
export function detectProhibitedGitVerb(command: string, readValues = true): { verb: string } | null {
  if (!command) return null;
  ...
  for (const segment of splitShellSegments(command)) {
    ...
    if (verb === GIT_CONFIG_VERB || PROHIBITED_GIT_VERBS_SIMPLE.has(verb)) return { verb };
    ...
    // measured the same and block now, and so does the BRACE spelling
    // `--{amend,amend}`: AP-EXT-ITER93-06 landed 2026-08-29 and `expandWord`
    // now expands the word at `pushWordBoundaryTokens`' flush, UPSTREAM of this
    // seam, so the flag test sees `--amend`. Re-measured on the shipped handler
    // 2026-09-19 — `--{amend,amend}` and `--{prune,prune}` both block, at
    // brace/literal parity. The `--` of `isCheckoutRefOperation`
    ...
    if (verb === 'commit' && afterVerb.some(t => execNameIs(t, '--amend'))) return { verb: 'commit --amend' };
    if (verb === 'fetch' && afterVerb.some(t => execNameIs(t, '--prune'))) return { verb: 'fetch --prune' };
  }
  return null;
}
...
function extractNodeTestPathsFromSegment(segment: string): string[] {
  ...
  for (let idx = anchor + 1; idx < tokens.length; idx++) {
    ...
    if (execNameIs(t, '--test')) { foundTestFlag = true; continue; }
...
/**
 * R-WSRC-GR: ONE record per gated OPERATION — the operator override flag AND the two
 * audit events the seam emits for it. Narrowly scoped: one entry each, NO SECOND TABLE.
 *
 * AP-EXT-ITER254-01: the Git Boundary Rules gate two axes, a VERB and a PATH, and the
 * `.git/` write rides in here as `GIT_DIR_WRITE_KEY` rather than in a table of its own.
 * The key is deliberately unspellable as a verb (`detectProhibitedGitVerb` returns bare
 * words plus `commit --amend` / `fetch --prune`, never one carrying a `/`), so the two
 * detectors cannot collide on a row. Sharing the table is what makes the new names
 * machine-checked for free: `gitVerbGateEventNames` derives from `Object.values` here,
 * so the AP-EXT-ITER110-01 registry test covers them without being told they exist.
 *
