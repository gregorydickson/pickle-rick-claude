...
function segmentIsGitCommit(segment: string): boolean {
  ...
  if (segmentDefinesGitAlias(tokens)) return true;
 ...
 */
function linkDependencyEnvironment(projectSource: string, projectDestination: string): void {
 ...
 */
function resolveProjectPrefix(treeRoot: string, stagedPaths: string[]): string {
  for (const relativePath of stagedPaths) {
    ...
    for (let parent = path.dirname(directory); parent !== directory; parent = path.dirname(directory)) {
      if (fs.existsSync(path.join(treeRoot, directory, 'tsconfig.json'))) return directory;
      directory = parent;
    }
  }
  return '';
...
function formatBlockReason(kind: GateFailureKind, details: string): string {
  ...
  if (kind === 'cold_cache_timeout') {
    return (
      `R-WACT: tsc --noEmit failed with cold_cache_timeout${suffix} ` +
