 ...
 */
const LITERAL_PART_RE = new RegExp(
  `${UNQUOTED_ESCAPE}|${ANSI_C_QUOTED_SPAN}|${SINGLE_QUOTED_SPAN}`, 'y',
);
const EXPANDING_QUOTED_SPAN_RE = new RegExp(
  `${LOCALE_QUOTED_SPAN}|${DOUBLE_QUOTED_SPAN}`, 'y',
);

/** The sticky match at exactly `index`, or null when the pattern is not there. */
function matchAt(sticky: RegExp, text: string, index: number): string | null {
 ...
 */
const DOT_SOURCE_BUILTINS: readonly string[] = ['.', 'source'];
 ...
 */
function execsScriptArgument(token: string | undefined): boolean {
 ...
 */
export function patternNamesACommand(pattern: string): boolean {
...
export function segmentDefinesGitAlias(tokens: readonly ShellToken[]): boolean {
...
/** One piece of a word, as `splitWordAtGluedSeparators` partitions it. */
interface WordPiece {
  /** The piece's own characters, verbatim — joining every `text` rebuilds the word. */
  readonly text: string;
  /** Whether bash reads this piece as an operator rather than as a word. */
 ...
 * hiding the reset).
 */
function splitWordAtGluedSeparators(word: string): WordPiece[] {
  const pieces: WordPiece[] = [];
  let buffer = '';
  const flush = (): void => {
    if (buffer.length > 0) pieces.push({ text: buffer, separator: false });
    buffer = '';
  };
  for (const part of word.match(WORD_PART_RE) ?? []) {
...
function pushWordBoundaryTokens(word: string, tokens: string[]): void {
  for (const piece of splitWordAtGluedSeparators(word)) {
    ...
    if (piece.separator) tokens.push(piece.text);
    else tokens.push(...expandWord(piece.text));
  }
}
 ...
 */
function expansionSpanEnd(text: string, start: number): number {
  const ch = text[start];
  if (ch === '`') {
  ...
  if (next === '{') return balancedSpanEnd(text, start + 1, '{', '}');
  if (next === '(') return balancedSpanEnd(text, start + 1, '(', ')');
  if (!PARAMETER_NAME_CHAR_RE.test(next)) return start + 2;
  let i = start + 1;
  while (i < text.length && PARAMETER_NAME_CHAR_RE.test(text[i])) i++;
  return i;
}
 ...
 */
function readExpansions(command: string, render: (span: string) => string): string {
  let out = '';
  let i = 0;
  while (i < command.length) {
    const literal = matchAt(LITERAL_PART_RE, command, i);
    if (literal !== null) { out += literal; i += literal.length; continue; }
    const expanding = matchAt(EXPANDING_QUOTED_SPAN_RE, command, i);
    if (expanding !== null) { out += readExpansionsInText(expanding, render); i += expanding.length; continue; }
    const end = expansionSpanEnd(command, i);
    if (end === -1) { out += command[i]; i++; continue; }
 ...
 */
function readExpansionsInText(text: string, render: (span: string) => string): string {
  let out = '';
  let i = 0;
  while (i < text.length) {
 ...
 */
function substitutionOutput(span: string): string {
  const body = substitutionBody(span);
  if (body === null) return span;
...
function inlineSubstitutionOutput(command: string): string {
...
function resolveParameterReference(
  ...
  if (!indirect || value === undefined) return value;
 ...
 */
function recordAssignedValues(piece: string, values: Map<string, string>): void {
...
function inlineAssignedValues(command: string): string {
  ...
  for (const match of command.matchAll(TOKEN_SCAN_RE)) {
    ...
    for (const piece of splitWordAtGluedSeparators(word)) {
      ...
      if (!piece.separator) recordAssignedValues(rendered, values);
    }
  }
...
export function gitConfigDeliveredValues(tokens: readonly ShellToken[]): string[] {
  const values: string[] = [];
  for (const token of tokens) {
    const eq = token.value.indexOf('=');
    if (eq <= 0 || token.value.startsWith('-')) continue;
    if (!GIT_CONFIG_DELIVERY_NAME_RE.test(token.value.slice(0, eq))) continue;
 ...
 */
function inlineGitConfigValues(command: string): string {
  ...
  if (!command.includes('git')) return command;
  const tokens = tokenizeShellTokens(command);
  if (execAnchorIndex(tokens, 'git') < 0) return command;
