 ...
 */
export function countSessionTickets(sessionDir) {
 ...
 */
function resumeReattachDoneRefusal(fullSessionPath, statePath, ticketId, workingDir, completionCommitSha) {
    ...
    if (isAdvisoryWorkerGateVerdict(gate.verdict, workingDir)) {
        ...
        emitWorkerGateNotRunResidual(statePath, ticketId, advisoryWorkerGateResidualDetail(gate.verdict, 'tryResumeOrphanReattach'));
        return null;
    }
...
function resolveResumeSessionPath(config) {
 ...
 */
function adoptSessionPrdPath(config, fullSessionPath) {
 ...
 */
function claimSessionMapForResume(fullSessionPath) {
    try {
 ...
 */
function applyExplicitBudgetOverrides(state, config) {
    if (config.explicitFlags.has('max-time')) {
 ...
 * asserted for both.
 */
function applyGitProvenanceToState(state) {
    const startCommit = resolveStartCommit();
    if (startCommit) {
        state.start_commit = startCommit;
    }
    else if (isInsideGitRepo(process.cwd())) {
        process.stderr.write('[setup] WARNING: git repo has no commits yet — start_commit deferred; ' +
    ...
    try {
        state.pinned_branch = getHeadBranch(process.cwd());
    }
    catch {
        state.pinned_branch = null;
    }
    if (startCommit)
 ...
 */
function scaffoldInTreeSessionNotes(sessionId) {
 ...
 */
function emitSessionStartActivity(sessionId, state, config, taskStr) {
    logActivity({
        event: 'session_start',
        source: 'pickle',
        session: sessionId,
 ...
 */
async function runSetupMcpSnapshot(session, resumeMode) {
    try {
        const settingsBag = loadPickleSettingsBag();
        const snapshotServers = Array.isArray(settingsBag?.worker_mcp_snapshot_servers)
            ? settingsBag.worker_mcp_snapshot_servers
            : [];
        await runMcpSnapshot(session.sessionRoot, snapshotServers, resolveMcpConfigPath(settingsBag ?? undefined), session.state.original_prompt || '', async () => null, resumeMode);
    }
...
function updateSessionMapTolerantOfLock(sessionsMap, sessionRoot) {
    try {
