 ...
 */
export function runPipelineOrphanWorkerReap(statePath, sessionsRoot, log, deps = {}) {
 ...
 */
export function findLiveCommandForPid(psOutput, pid) {
    for (const rawLine of psOutput.split(/\r?\n/)) {
 ...
 */
function declaresBetweenTicketGateScript(dir) {
    try {
 ...
 */
function resolveGateProjectDir(workingDir) {
    if (workingDir.trim() === '')
        return null;
 ...
 */
function measureApplicablePostFinalTier(input) {
    const ts = (input.now ?? Date.now)();
    let finalCommitTs;
 ...
 */
function probePostFinalMeasurement(input) {
...
export function classifyNoProgressFailureReason(sessionDir) {
    try {
        ...
        if (readScopeAllowedPaths(sessionDir).kind === 'unresolvable')
 ...
 */
export function collectVerdictRoster(sessionDir) {
    const tickets = collectTickets(sessionDir);
    if (tickets.length === 0 && collectRickTicketPaths(sessionDir) === null)
        return null;
 ...
 */
function phantomDoneKeepClaim(ticketId, decision) {
    if (decision.kind === 'absent')
        return unmeasuredKeepClaim(ticketId);
 ...
 */
function logPhantomDoneKept(input, ticketId, workingDir, decision) {
 ...
 */
function applyInspectPhantomDoneDecision(content, filePath, sessionDir, ticketId, workingDir, priorStatus, fallbackDir) {
 ...
 */
export function inspectPhantomDoneTicketFile(filePath, sessionDir, workingDir, priorStatus = 'Todo', fallbackDir) {
    let content;
    try {
 ...
 */
export function moveCurrentTicket(s, sessionDir, ticket, step) {
    if (s.current_ticket === ticket)
        return false;
...
function updateMuxLifecycleState(statePath, patch) {
    return sm.update(statePath, s => {
        ...
        if (patch.step !== undefined && !ticketChanged)
            s.step = maxLifecycleStep(s.step, patch.step);
    });
}
...
function collectRickTicketPaths(sessionDir) {
    let dirEntries;
    try {
...
function attemptOrphanChainReattach(input) {
    ...
    if (enumerationCompleted(statusR) && (statusR.stdout || '').trim().length > 0) {
 ...
 */
function emitOrphanUnreattachable(statePath, ticketId, candidateSha, prevHead) {
    try {
        writeActivityEntry(statePath, {
...
export function detectAndRecoverHeadRegression(input) {
    ...
    if (!recovered) {
        ...
        if (candidateSha)
            emitOrphanUnreattachable(statePath, ticketId, candidateSha, prevHead);
 ...
 */
function commitEpochOverLadder(dirs, sha) {
...
fallbackDir) {
    ...
    const dirs = { workingDir, fallbackDir };
...
export function resolveQualityGateSkipReason(state, _log, _sessionName, _callsite) {
 ...
 */
export const isHaltExit = (r) => classifyExitReason(r).haltEligible;
export const isFailureExit = (r) => classifyExitReason(r).verdict === 'failure';
/**
 ...
 */
export const isPerTicketVerdictReason = (r) => classifyExitReason(r).verdict === 'incomplete';
 ...
 */
export function recomputeAbsentWorkerGateVerdict(extensionDir, runCheck = defaultRecomputeCheck) {
    if (!runCheck('npx', ['eslint', 'src/', '--max-warnings=0'], extensionDir))
        return 'red';
 ...
 */
function buildCompletionCtx(args, decision) {
 ...
 */
export function isAdvisoryWorkerGateVerdict(verdict, workingDir) {
    if (verdict === 'not_run')
        return true;
    if (verdict !== 'red')
        return false;
 ...
 */
export function advisoryWorkerGateResidualDetail(verdict, site) {
 ...
 */
export const ACCEPTANCE_ASSERTION_FAILED = 'acceptance_assertion_failed';
...
export function readExecutableAcceptanceAssertions(content) {
    return executableAcceptanceSection(content)
        ...
        .map((m) => ({ command: m[1].trim(), kind: m[2] === 'exits' ? 'exits' : 'returns', expected: Number(m[3]) }));
}
/**
 ...
 * one integer.
 */
function measureAcceptanceAssertion(assertion, workingDir) {
    ...
    if (!/^\d+$/.test(count))
        return { verdict: 'unmeasured', observed: `output ${JSON.stringify(count.slice(0, 80))}` };
...
function findFailedAcceptanceAssertion(sessionDir, ticketId, workingDir) {
    let content;
    try {
...
function refuseDoneOnFailedAcceptanceAssertion(args, failed, testsVerdict) {
    try {
...
export function isAcceptanceAssertionRefusal(guard) {
...
export function guardCompletionCommitBeforeDone(args) {
    ...
    if (gateAdvisory) {
        emitWorkerGateNotRunResidual(path.join(args.sessionDir, 'state.json'), args.ticketId, advisoryWorkerGateResidualDetail(resolvedGate.verdict, 'guardCompletionCommitBeforeDone'));
    }
    if (decision.ok) {
        ...
        if (failedAssertion)
            return refuseDoneOnFailedAcceptanceAssertion(args, failedAssertion, testsVerdict);
 ...
 */
function flagManagerHandoffResidual(sessionDir, state) {
 ...
 */
function ticketOwnedAddArgs(stagePaths) {
...
export function commitAndContinueDoneFlip(input) {
    ...
    const commitMsg = stampPickleTicketTrailer(input.workingDir, `fix(${input.ticketId}): commit-and-continue recovery (R-ORSR-2)`, input.ticketId);
 ...
 */
function dirtyPathsInWorkingDirSpace(workingDir) {
 ...
 */
function resolveTicketOwnedStaging(workingDir, sessionDir, ticketId, logPrefix, log) {
    try {
...
function attributeBoundaryHeadMoved(sessionDir, ticketId, workingDir) {
    ...
    const decision = evaluateCompletionEvidence(buildCompletionCtx({ sessionDir, ticketId, workingDir, rereadBackoffMs: 0 }, 'attribution'));
 ...
 */
export function assessRecoveryEvidence(sessionDir, workingDir, ticketId) {
...
function describeRemediatorStderr(stderr) {
    if (stderr === '')
        return '<empty>';
 ...
 */
function formatRemediatorSpawnOutcome(stage, ticketId, outcome) {
...
export function spawnRecoveryRemediator(input, gateFailures) {
    try {
        ...
        if (briefPrepReport !== null)
            input.log(briefPrepReport);
        if (briefPrep.kind !== 'ok')
            return false;
 ...
 */
export function buildRemediatorWorkerInvocation(backend, briefPath, addDirs) {
 ...
 */
function commitConvergedPlanPhase(input, phase) {
    ...
    const staging = resolveTicketOwnedStaging(input.workingDir, input.sessionDir, input.ticketId, '[converged-plan]', input.log);
 ...
 */
function convergedPlanCommitLanded(workingDir, headBefore) {
 ...
 */
function appendRecoveryAttempt(statePath, attempt, discriminantTag) {
    try {
        sm.update(statePath, s => {
 ...
 */
function isBoundedEscapeCharge(a, ticketId) {
...
function countBoundedEscapeAttempts(ledger, ticketId) {
    ...
    for (const a of ledger) {
        if (isBoundedEscapeCharge(a, ticketId))
            n++;
    }
    return n;
}
 ...
 * case to enumerate.
 */
export function evaluateBoundedEscape(state, sessionDir, cap) {
    const ticketId = typeof state.current_ticket === 'string' && state.current_ticket.length > 0
        ? state.current_ticket
        : null;
    if (!ticketId)
...
export function refundRecoveryBudgetOnReset(statePath, sessionDir, ticketId, iteration, log = () => { }) {
    ...
    const chargedToThisTicket = (a) => a.ticket === ticketId;
    let cleared = 0;
    try {
 ...
 * left terminal.
 */
export function executeBoundedEscape(statePath, sessionDir, workingDir, ticketId, iteration, cap, log = () => { }) {
    const deps = {
        reconcile: (i) => reconcileTicketTruth(i),
        gate: () => 'failing',
...
export function writePickleIncompleteSentinelIfRemaining(sessionDir, statePath, log) {
    try {
        ...
        if (tickets.length === 0 || remaining.length === 0) {
            ...
            catch { /* not present — nothing to reconcile */ }
            return false;
        }
        const ts = new Date().toISOString();
        try {
...
export async function runMainLoopRateLimitPark(input) {
    const { exitResult, consecutiveRateLimits, rateLimitWaitMinutes, statePath, sessionDir, state, iteration, } = input;
 ...
 */
function exitRateLimitPark(statePath, exitReason) {
 ...
 * drift apart.
 */
function planRateLimitPark(input, session) {
    const { exitResult, consecutiveRateLimits, maxRateLimitRetries, rateLimitWaitMinutes, maxParkMinutes, statePath, log, } = input;
...
function breakWithExitReason(ctx, reason) {
    recordExitReason(ctx.statePath, reason);
    ctxDeactivate(ctx);
    return { kind: 'break', reason };
}
function recordAndReturnRelaunch(ctx, decision, pendingTickets, message) {
    ctx.log(message);
...
function handleCodexNoProgressRecovery(state, postState, ctx, decision) {
    ...
    if (!workingDir)
        return breakWithExitReason(ctx, 'state_working_dir_missing');
...
function handleCodexInactiveCompletion(state, postState, ctx, decision, exitKind) {
    if (decision.reason === 'time_limit')
        return breakForLimit(ctx);
    ...
    return recordAndReturnRelaunch(ctx, decision, decision.pendingCount, `${backend} manager subprocess exited via ${exitKind} with ${decision.pendingCount} ticket(s) still pending — relaunching (count ${decision.nextRelaunchCount}/${decision.cap}).`);
}
function handlePendingInactiveCompletion(state, postState, ctx, exitKind) {
    const decision = evaluateManagerRelaunch(postState, withFreshTicketStatuses(ctx.sessionDir, collectTickets(ctx.sessionDir)), ctx.cbState ?? null, exitKind);
    if (decision.reason === 'time_limit')
        return breakForLimit(ctx);
    if (!decision.shouldRelaunch) {
    ...
    if (esc.ticketId) {
        recordBoundedEscapeAttempt(ctx.statePath, esc.ticketId, ctx.iteration, ctx.log, { sessionDir: ctx.sessionDir, iterationStartMs: ctx.iterationStartMs });
    }
    const backend = resolveBackendFromStateFileWithSource(ctx.statePath).backend;
    return recordAndReturnRelaunch(ctx, decision, decision.pendingCount, `${backend} manager exited via ${exitKind} with ${decision.pendingCount} pending — relaunching (count ${decision.nextRelaunchCount}/${decision.cap}).`);
}
function handleInactiveCompletion(state, ctx) {
...
function handleErrorCompletion(state, ctx) {
    const postState = readPostIterationState(state, ctx);
    const exitKind = classifyManagerRelaunchExit(postState, ctx.outcome, ctx.iterLogFile || path.join(ctx.sessionDir, `tmux_iteration_${ctx.iteration}.log`), ctx.maxTurns ?? null);
    ...
    if (decision.shouldRelaunch && !isGenuineCrashOrSpawnFailure(decision, ctx.outcome, ctx.iterLogFile || path.join(ctx.sessionDir, `tmux_iteration_${ctx.iteration}.log`))) {
        ...
        return recordAndReturnRelaunch(ctx, decision, decision.pendingCount, `${backend} manager subprocess ${detail} with ${decision.pendingCount} ticket(s) still pending — relaunching (count ${decision.nextRelaunchCount}/${decision.cap}).`);
    }
...
export async function processCompletionBranch(state, result, ctx) {
    ...
    if (result === 'inactive')
        return handleInactiveCompletion(state, ctx);
    if (result === 'error')
        return handleErrorCompletion(state, ctx);
 ...
 */
function finalizeCurrentTicketBeforeEpicExit(state, ctx, curState) {
 ...
 */
function refuseEpicFinalizeWhileWorkPending(state, ctx, curState) {
 ...
 */
function readScopeAllowedPathSpecsFromFile(scopeJsonPath, workingDir) {
    const pathSpecs = [];
    if (!scopeJsonPath)
        return pathSpecs;
    try {
        const raw = readRecoverableJsonObject(scopeJsonPath);
 ...
 */
function defaultDoneGuard(sessionDir, ticketId, workingDir) {
    try {
        if (normalizeTicketStatus(getTicketStatus(sessionDir, ticketId)) !== 'done')
            return false;
        if (!workingDir)
            return false;
        const decision = evaluateCompletionEvidence(buildCompletionCtx({ sessionDir, ticketId, workingDir, rereadBackoffMs: 0 }, 'phantom-watch'));
        return decision.ok;
    }
    catch {
        return false;
 ...
 */
function isSourceSignatureProgress(sourceSignature, priorSignature) {
 ...
 */
function computeArtifactProgressCharge(priorZero, input) {
    if (input.doneGuard || input.progressed)
        return { nextZero: 0, incremented: false };
    if (input.suppressIncrement)
        return { nextZero: priorZero, incremented: false };
 ...
 * was no prior at all.
 */
function carryForwardSourceSignature(sourceSignature, priorSignature) {
 ...
 */
function resolveDoneGuard(doneGuardFn, sessionDir, ticketId, workingDir) {
    try {
        return doneGuardFn(sessionDir, ticketId, workingDir);
    }
    catch {
        return false;
    }
 ...
 */
function persistArtifactProgressEntry(statePath, ticketId, spawn) {
...
function emitArtifactProgressZero(statePath, ticketId, entry, k, log) {
    writeActivityEntry(statePath, {
...
function findNewestMatchingFile(dir, re) {
    let newest = null;
    try {
...
function detectAutoHandoffPhase(ticketDir) {
...
function detectAutoHandoffSessionLog(ticketDir) {
    const name = findNewestMatchingFile(ticketDir, /^worker_session_\d+\.log$/);
    if (!name)
        return { pid: null, logPath: null };
...
function readAutoHandoffLogTail(logPath, n = 20) {
    if (!logPath)
        return [];
    try {
 ...
 */
function appendAutoHandoffFallback(sessionDir, ticketDir, workingDir) {
    try {
        ...
        catch { /* absent — first note */ }
        if (existing.includes(marker))
            return; // AC-TCHN-2B: idempotent per spawn
...
export function recordWorkerArtifactProgress(statePath, sessionDir, ticketId, beforeCount, opts = {}) {
    ...
    if (fired)
        emitArtifactProgressZero(statePath, ticketId, entry, k, opts.log);
    return {
...
function classifyWorkerSessionLogs(ticketDir, files, workObserved) {
    ...
    const unmeasured = (unmeasuredLogPath, unmeasuredPid) => {
 ...
 */
function silentDeathGit(args, cwd, input) {
    try {
...
function readScopeAllowedPaths(sessionDir) {
    ...
    try {
        ...
        if (!scope)
            return fs.existsSync(scopePath) ? { kind: 'unresolvable' } : { kind: 'unscoped' };
        if (!Array.isArray(scope.allowed_paths))
            return { kind: 'unresolvable' };
        const allowed = scope.allowed_paths.filter((p) => typeof p === 'string' && p.length > 0);
 ...
 */
function resolveAttributableFrontmatterSha(sessionDir, ticketId, workingDir) {
    let raw;
    try {
    ...
    try {
        ...
        for (const file of fs.readdirSync(ticketDir)) {
            if (!LIFECYCLE_ARTIFACT_RE.test(file))
                continue;
            try {
                if (fs.statSync(path.join(ticketDir, file)).mtimeMs >= sinceMs)
                    return true;
            }
 ...
 */
const everythingButCommitClaimed = new Set();
/**
...
const emitBackfillEvent = (ctx, ticketId, commit, ts) => {
 ...
 */
const emitRevertEvent = (ctx, ticketId, result, ts) => {
...
const resolveCodegraphSessionDeps = (opts) => ({
    ...
    createService: opts.deps?.createService
        ?? ((s) => CodegraphService.create(opts.workingDir, s, { emit: (ev) => writeActivityEntry(opts.statePath, ev) })),
});
/**
 ...
 */
const resolveCodegraphIndexStatus = (ctrs) => (ctrs.latched > 0 ? 'latched' : ctrs.degraded > 0 ? 'degraded' : 'healthy');
/**
...
const emitCodegraphSessionSummary = (args) => {
    try {
        ...
        const persisted = readRecoverableJsonObject(args.statePath);
 ...
 */
const handlePhantomDoneTicketEvent = (ctx, ticketId, ticketFile, dirs, isRecheck) => {
    ...
    if (!isRecheck)
        ctx.scheduleRecheck(ticketId, ticketFile, dirs);
};
/**
...
function startPhantomDoneWatchers(opts) {
    ...
    const scheduleRecheck = (ticketId, ticketFile, dirs) => {
 ...
 */
export function runTerminalReport(input) {
 ...
 * already the answer.
 */
export function rateLimitParkStillLive(sessionDir, nowMs = Date.now()) {
    const waitPath = path.join(sessionDir, 'rate_limit_wait.json');
    if (!fs.existsSync(waitPath))
        return false;
    try {
        const parsed = JSON.parse(fs.readFileSync(waitPath, 'utf-8'));
 ...
 */
function snapshotPreSpawnIteration(state, sessionDir) {
...
function isModelAttestedDone(sessionDir, prevTicketInfo) {
    if (!prevTicketInfo?.id)
        return false;
 ...
 */
function validateModelAttestedDone(input) {
 ...
 */
function completeTicketTransition(input) {
 ...
 */
function seedPreviousTicket(state, sessionDir, previousTicketStartCommit) {
    const previousTicket = state.current_ticket || null;
    if (!previousTicket)
        return { previousTicket, previousTicketStartCommit };
...
function exitOnOpenCircuitBreaker(cbEnabled, cbState, statePath, log) {
...
function evaluateStallFallback(input) {
    ...
    if (input.cbEnabled)
        return { halt: false, stallCount: input.stallCount, lastStateIteration: input.lastStateIteration };
...
function reapOrphansAtIterationStart(statePath, sessionDir, log) {
    try {
 ...
 */
function probeTimeoutArtifactProgress(input) {
    ...
    if (!ticketDir || !fs.existsSync(ticketDir))
        return { progressed: false, snapshot: lastArtifactProgressSnapshot };
 ...
 */
function routeTimeoutNoProgress(input) {
...
function handleInactiveIterationResult(input) {
    ...
    if (detectManagerInactiveExit(outcome)) {
        ...
        if (inactiveExitKind === 'codex_session_inactive') {
            ...
            if (inactiveDecision.shouldRelaunch) {
                ...
                if (codexStep.kind === 'exit')
                    return { kind: 'exit', exitReason: codexStep.exitReason };
        ...
        if (inactiveExitKind !== 'codex_session_inactive') {
            ...
            if (cleanExitStep.kind === 'exit')
                return { kind: 'exit', exitReason: cleanExitStep.exitReason };
 ...
 */
function handleErrorIterationResult(input) {
    const { outcome, state, statePath, sessionDir, extensionRoot, iteration, iterLogFile, log } = input;
    let postState = state;
    try {
        postState = readRunnerState(statePath);
    ...
    const relaunchDecision = evaluateManagerRelaunch(postState, collectTickets(sessionDir), input.cbState, exitKind);
...
function maybeRecordCircuitBreakerIteration(input) {
...
function createMuxLoopTrackers(now) {
    return {
        counters: {
...
async function openIterationPass(input) {
    ...
    if (startStep.kind === 'exit')
        return { kind: 'exit', exitReason: startStep.exitReason, iteration: input.iteration };
    ...
    if (rosterExit)
        return { kind: 'exit', exitReason: rosterExit, iteration };
...
function readPreskipTerminalStatus(sessionDir, preskipTicket) {
 ...
 */
async function runIterationEndCycles(input) {
    ...
    if (cbStep.tripped)
        return { kind: 'exit', exitReason: 'circuit_open' };
 ...
 */
async function settleTaskCompletedClaim(input) {
...
async function runMuxRunnerMain({ runIteration, sleep, exit }) {
    ...
    const { counters, commitPendingTracker, stallTrackers, cpuLivenessAnchor, iterationZeroGates, toolchainPreflight } = createMuxLoopTrackers(muxNow);
