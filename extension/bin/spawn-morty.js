...
import { PromiseTokens, Defaults, hasLifecycleArtifact, BACKENDS, WORKER_GATE_VERDICT_FIELD, reportedTestResults } from '../types/index.js';
 ...
 */
async function collectCodegraphHits(service, terms, emitSkipped) {
 ...
 */
function formatStallMessage(stallThresholdMs, sigtermSent, sigkillSent) {
    ...
    return await new Promise((resolve) => {
        ...
        const finalize = (status, signal, extraStderr = '') => {
            ...
            resolve({
                ...
                timeoutMessage: stalled
                    ? formatStallMessage(stallThresholdMs, sigtermSent, sigkillSent)
        ...
        if (stallThresholdMs !== undefined) {
            ...
            stallCheckHandle = setInterval(() => {
                if (Date.now() - lastActivityAt < stallThresholdMs)
                    return;
                stalled = true;
                killChild();
            }, pollMs);
...
function parseWorkerGateTestFailures(output, extensionDir) {
    ...
    console.error(`[spawn-morty] worker gate tier produced no parseable failure (unmeasured)${firstLine ? `: ${firstLine}` : ''}`);
    return [];
}
 ...
 */
function tierRunVerdict(result) {
    if (!result.ok)
        return 'red';
 ...
 */
async function acquireWorkerSpawnLockOrExit(sessionRoot, statePath, ticketId) {
    try {
 ...
 */
async function runWorkerProcessUnderSpawnLock(ctx, lock) {
 ...
 */
function makeCodegraphActivityEmitter(sessionRoot) {
    return (event) => {
        try {
...
function _smCrumb(label) { if (SM_DEBUG)
 ...
 */
function resolveTierAndTimeout(parsed, runtime, ticketInfo) {
    if (ticketInfo) {
...
function emitTierPhaseSkipped(statePath, parsed, effectiveTier) {
 ...
 */
function resolveAndAssertSpawnBackend(parsed, runtime, ticketInfo, statePath) {
...
function emitBackendResolutionTelemetry(statePath, parsed, resolution) {
 ...
 */
async function buildCodegraphSectionForSpawn(args, runtime, effectiveTier) {
...
async function launchWorker(args, runtime, statePath, budget, model, codegraphSection) {
...
async function main() {
    ...
    printMinimalPanel(args.isReviewTicket ? 'Spawning Review Worker' : 'Spawning Morty Worker', { Request: args.ticket, Ticket: args.ticketId, Type: args.isReviewTicket ? 'review' : 'implementation', Format: args.outputFormat, Backend: resolution.backend, Timeout: `${budget.effectiveTimeout}s (Req: ${budget.requestedTimeout}s)`, PID: process.pid }, args.isReviewTicket ? 'MAGENTA' : 'CYAN', '🥒');
