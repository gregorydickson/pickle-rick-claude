...
export function runVerifyActivityTimeline(sessionDir) {
    const lines = [];
    ...
    if (incomplete.length > 0) {
        lines.push(`WINDOW: INCOMPLETE — ${incomplete.length} ticket(s) carry a terminal event with no spawn in state.activity (${incomplete.join(', ')}). ` +
