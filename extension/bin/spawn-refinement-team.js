...
// carries one.
export function resolveTrackedSuffixMatches(workingDir, token, spawnSyncFn = spawnSync) {
 ...
 */
function buildCodebaseRoleInstructions(workingDir, portalContext) {
...
function buildRoleInstructions(roleId, workingDir, portalContext) {
    const byRole = {
 ...
 */
function buildCrossReferenceSection(roleId, cycle, previousAnalyses) {
...
function buildAnalystOutputInstructions(outputFile, cycle) {
...
function createAnalystLogStream(refinementDir, roleId, cycle) {
...
function emitWorkerBackendResolved(sessionDir) {
...
function startAnalystProcess(opts, logStream) {
 ...
 */
function finalizeAfterLogFlush(logStream, finalize) {
...
function attachAnalystOutcomeHandlers(ctx, latch) {
    const { proc, escalation, logStream, resultBase } = ctx;
    proc.on('error', (err) => {
        const msg = safeErrorMessage(err);
        console.error(`${Style.RED}Failed to spawn ${ctx.invocationCmd} (${resultBase.roleId}): ${msg}${Style.RESET}`);
        logStream.end();
...
function awaitAnalystSettlement(ctx) {
...
function spawnWorker(opts) {
    ...
    if (opts.sessionDir)
        emitWorkerBackendResolved(opts.sessionDir);
 ...
 */
export function collapseAnalystTicketCopies(tickets) {
    const byId = new Map();
...
export function computeRequirementCoverageGap(prdPath, tickets) {
    ...
    for (const ticket of tickets) {
        for (const id of ticket.source_ac_ids)
            coveredIds.add(id);
        for (const id of ticket.mapped_requirements ?? [])
            coveredIds.add(id);
    }
...
export function reportSymbolAuditFindings(report) {
    if (report.ok)
        return;
...
export function buildRefinementManifest(args, results, ticketQualityWarnings) {
    ...
    if (requirementCoverage.missingRequirementIds.length > 0) {
        process.stderr.write(`[pickle-rick] requirement coverage gap: ${requirementCoverage.missingRequirementIds.length} of ` +
...
export function countWrittenAnalyses(refinementDir) {
    let entries;
    try {
...
export function resolveRefinementDisposition(cycleResults) {
