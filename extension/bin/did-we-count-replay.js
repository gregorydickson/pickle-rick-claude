...
const pickleModule = (await import(pickleModuleUrl));
const pickle = pickleModule.default;
...
export function extractEnclosingFunctionSnippet(content, anchor) {
    ...
    let found;
    const visit = (node) => {
        if (node.getStart(sourceFile) > anchorIndex || node.getEnd() < anchorIndex)
            return;
...
export function buildAstCheckRegistry(repoRoot = resolveReplayRepoRoot()) {
    const registry = {};
    for (const spec of AST_CHECKS) {
        registry[spec.sha] = (parentOrFix) => {
