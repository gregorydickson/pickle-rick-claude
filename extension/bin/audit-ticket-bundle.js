...
export function checkPathDrift(t, gitFiles, creationIndex = new Set()) {
    ...
    for (const tok of extractBacktickedPaths(t.body)) {
        ...
        if (gitFiles.has(stripped) || gitFilesArr.some((f) => suffixRe.test(f)))
            continue;
        if (forwardCreatePaths.has(stripped))
            continue;
        ...
        if (isForwardCreated(stripped, creationIndex))
            continue;
        const ctx = lineContext(t.body, tok);
        if (hasForwardRefPathAnnotation(ctx, tok) || hasForwardRefPathAnnotation(ctx, stripped))
            continue;
        findings.push({
            ticket_id: t.id,
