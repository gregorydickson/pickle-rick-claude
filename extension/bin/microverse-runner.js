 ...
 */
function enumerateInterfaceSweepAxes(workingDir, startCommit, getSymbols, getFiles) {
...
export async function runInterfaceChangeSweep(opts) {
    ...
    if ('skipped' in axes)
        return { ran: false, skipped: axes.skipped, selfIntroduced: [] };
 ...
 */
function sweepBaseCommit(startCommit) {
    if (typeof startCommit !== 'string')
        return null;
 ...
 */
function renderInterfaceSweepNotRun(skipped, iteration, log) {
    log(`Iteration ${iteration} — interface-change sweep NOT RUN (${skipped}): a measurement ` +
...
export const _deps = {
    ...
    probeRateLimitCleared: probeRateLimitCleared,
 ...
 */
function degradedJudgeResult(shape, rawOutput, parseErrorMessage) {
 ...
 */
export function emitJudgeParseDiagnostic(judgeResult, rawOutput, writeActivity = logActivity) {
    if (judgeResult.parse_error_message === undefined)
        return;
    writeActivity({
 ...
 */
export function emitJudgeLegacyShapeDiagnostic(judgeResult, writeActivity = logActivity) {
    if (judgeResult.legacy_raw_keys === undefined)
        return;
    writeActivity({
...
const judgeStringArray = (arr) => Array.isArray(arr) ? arr.filter((s) => typeof s === 'string') : [];
/**
 ...
 */
function normalizeJudgeViolations(raw) {
    return raw
 ...
 */
function classifyJudgeErrorByMessage(err) {
    if (isMissingCliError(err))
        return { failureKind: 'cli_missing' };
    const msg = safeErrorMessage(err);
    if (/\bETIMEDOUT\b/i.test(msg))
        return { failureKind: 'timeout' };
    if (/\b(529|429)\b/.test(msg))
...
function spawnMeasurementShell(validation, cwd) {
 ...
 */
function createMeasurementSettler(resolve) {
    let settled = false;
    return (result) => {
        if (settled)
            return;
...
function buildExitFailureAttempt(code, signal, captured) {
    const failureKind = isMissingCommandExit(code, captured.stdout, captured.stderr)
        ? 'cli_missing'
        : 'failed';
    return {
        metric: null,
        failureKind,
        message: summarizeCommandFailure(`command exited with code ${code}${signal ? ` (signal ${signal})` : ''}`, captured.stdout, captured.stderr),
    };
}
/** The score is the LAST line of stdout — every other line is the command's own chatter. */
function parseMeasurementOutput(stdout) {
 ...
 */
function judgeStdoutFromError(err) {
    if (!err || typeof err !== 'object')
        return '';
...
export function judgeAttemptFromOutput(output) {
 ...
 */
async function rateLimitWaitClearedByProbe(ctx) {
    let verdict;
    try {
...
function collectJudgePreSpawnEnvKeyNames(isNested) {
...
function classifyJudgeAttemptTimeout(failureKind, probeKind) {
    if (failureKind !== 'timeout')
        return null;
    return probeKind === 'timeout' ? 'probe_timeout' : 'attempt_timeout';
}
function emitJudgeAttemptTelemetry(ctx, state, result, elapsedMs) {
...
function emitBaselineAttemptTimeout(attemptActivity, attempt, elapsedMs) {
    if (!attemptActivity)
        return;
    try {
        _deps.logActivity({
...
function resolveExhaustedFailureKind(current, failureKind) {
    if (failureKind === 'failed')
        return 'failed';
    if (failureKind === 'timeout' && current !== 'failed')
        return 'timeout';
    if (failureKind === 'rate_limited' && current !== 'failed' && current !== 'timeout')
        return 'rate_limited';
    return current;
}
function maybeActivateWorkerFallback(ctx, state, result) {
    if (state.workerFallbackActivated)
        return;
    const fallbackBackend = resolveWorkerIterationFallbackBackend(state.attemptBackend, state.totalAttempts, result.typedFailure, ctx.attemptActivity, ctx.settings);
    if (!fallbackBackend)
        return;
...
function judgeCliMissingResult(attempts, lastError) {
...
function createJudgeBackoffState(backend, probeKind) {
    return {
    ...
    for (let attempt = 0; attempt <= ctx.backoffsMs.length; attempt++) {
        ...
        if (result.metric) {
            if (cumulativeParkedMs > 0) {
                clearMetricParkWait(ctx.attemptActivity);
            }
...
export async function measureLlmMetricWithBackoff(goal, timeoutSeconds, cwd, judgeModel, history, prdPath, judgeContextPath, backend = 'claude', priorViolations = [], attemptActivity, allowedPaths = [], extensionRoot) {
    ...
    if (probe.kind === 'missing')
        return judgeCliMissingResult(0, probe.message);
 ...
 */
function readCitadelFindingsForHandoff(sessionDir) {
    if (!sessionDir)
        return [];
 ...
 */
function appendCitadelFindingsHandoff(parts, sessionDir) {
 ...
 */
export function deriveJudgeReviewSurface(sessionDir) {
    const scopeJsonPath = path.join(sessionDir, 'scope.json');
    const scope = readRecoverableJsonObject(scopeJsonPath);
 ...
 */
function computeTouchedLineNumbers(workingDir, base, paths) {
    if (paths.length === 0)
        return new Map();
    ...
    ['-c', 'core.quotePath=false', '-c', 'diff.relative=false', 'diff', '--no-renames', '--no-color',
        ...
        ...paths.map((p) => `:(top)${p}`)], { cwd: workingDir, encoding: 'utf-8', timeout: 30_000, maxBuffer: UNBOUNDED_READ_MAX_BUFFER });
...
export function dropOutOfSurfaceViolations(violations, surface, workingDir) {
    ...
    for (const v of violations) {
        if (!hasUsableLocator(v)) {
            kept.push(v);
            continue;
        }
        if (touched.get(v.path)?.has(v.line)) {
            kept.push(v);
            continue;
        }
        droppedCount++;
...
function applyOutOfSurfaceDrop(state, ctx, judgeResult, surface, phase) {
...
function recordJudgeSurfaceDerivationFailure(reason, ctx, phase) {
...
export function resetStoppedMicroverseState(state, sessionDir, log) {
 ...
 */
function adoptLateBaseline(state, metricResult, metricConv, ctx) {
...
export async function measureLlmIteration(state, ctx, backend) {
    if (state.key_metric.type !== 'llm') {
...
export function readLedgerSizeFigures(description) {
    ...
    for (const kind of Object.keys(LEDGER_SIZE_FIGURE_PATTERNS)) {
        ...
        if (values.length > 0)
            figures[kind] = Math.max(...values);
    }
    return figures;
}
function largestLedgerFiguresByPath(entries) {
 ...
 */
export function convergenceExitReason(branch, state, ctx) {
    if (branch === 'target')
        return 'converged';
 ...
 */
export function findUnmovableLedgerEntries(state, iteration) {
...
function describeLedgerEntries(entries) {
...
function resolveScopeAuditInputs(ctx) {
    ...
    if (committedFiles !== null && committedFiles.length === 0)
        return null;
...
export function auditPostIterationScope(ctx, state) {
    try {
        ...
        ctx.log(`[R-SSOC] post-iteration scope drift: ${(scopeResult.staged_paths_outside_scope ?? []).length} ` +
...
function parseSubsystemList(raw) {
    return raw && typeof raw === 'object' && Array.isArray(raw.subsystems)
        ? raw.subsystems.filter((v) => typeof v === 'string' && v.trim().length > 0)
        : [];
}
...
function parseCurrentIndex(raw) {
...
const DEFAULT_MICROVERSE_DISPOSITION = { reportAs: 'non-success', exitCode: 1 };
