...
import { resolveScope, refreshScope, filterBySubsystem, computeBaselineStartCommit, ScopeError, } from '../services/scope-resolver.js';
...
function readPickleReadinessHalt(statePath) {
    try {
...
if (process.argv[1] && path.basename(process.argv[1]) === 'pipeline-runner.js') {
    ...
    main(sessionDir, { scopeFlag, scopeBase, strictPhases, designSafeFlag }).catch((err) => {
        ...
        try {
            const prior = readRecoverableJsonObject(path.join(sessionDir, 'pipeline-status.json'));
            if (prior) {
                if (typeof prior.completed_phases === 'number')
                    fatalCompletedPhases = prior.completed_phases;
                if (typeof prior.skipped_phases === 'number')
                    fatalSkippedPhases = prior.skipped_phases;
                if (typeof prior.total_phases === 'number')
                    fatalTotalPhases = prior.total_phases;
            }
        }
