 ...
 */
const DEGRADED_PHASE_SKIP_REASONS = new Set([
 ...
 * dirty file.
 */
function splitPorcelainStatusPaths(stdout) {
    const trackedPaths = [];
 ...
 */
export function gitRepoRoot(cwd, onUnprovenAnchor) {
    try {
...
const notStarted = (reason) => ({ reason, passes: 0, started_at: null, ended_at: null });
 ...
 */
export function writeSkippedByScope(sessionDir, scopePhase, scope, target, workingDir, log = (msg) => { process.stderr.write(`${msg}\n`); }) {
    ...
    try {
        ...
        catch { /* no tmp to clean */ }
        log(`scope-audit: could not write skipped_by_scope.${scopePhase}.json: ${safeErrorMessage(err)} `
...
const REMEDIATION_SEVERITY_THRESHOLD = 'High';
...
function readCitadelBriefFile(briefPath) {
...
async function remediateCitadelFindings(runtime, findings, remediatorTimeoutMs, cycle) {
    ...
    try {
        ...
        runtime.log(`citadel: remediation ${step} failed on cycle ${cycle + 1}: ${safeErrorMessage(err)} `
 ...
 */
function isStrictPhasePolicy(runtime) {
    try {
...
function runPhaseSetup(runtime, phaseConfig, scope) {
    ...
    try {
        ...
        runtime.log(`${phaseConfig.name}: setup failed: ${safeErrorMessage(err)} `
 ...
 */
function resolveGitRepoRoot(workingDir, onUnprovenAnchor) {
 ...
 */
function stampPipelineTerminalReason(runtime, verdict, phaseIncomplete, phaseIncompleteReason) {
    ...
    finalizeIfTrulyComplete(runtime.statePath, () => pipelineBundleScan(runtime), finalizeDegradedCompleteOpts(runtime.statePath, verdict.unsuccessful));
}
/**
 ...
 */
function formatPhasesSummary(counters, totalPhases) {
    if (counters.skipped === 0)
        return `${counters.completed}/${totalPhases}`;
...
function maybeRunCloserRelease(runtime, unsuccessful, handoffStop) {
 ...
 */
function writeTerminalPipelineStatus(runtime, counters, effectiveFailed) {
...
function resolvePhaseIncompleteOutcome(runtime, rawPhase, exitCode, log) {
    ...
    // without touching it.
    if (isCrashFloorExitReason(priorExitReasonForIncomplete))
        return null;
 ...
 */
function reportDoneOverRedTestVerdict(runtime, counters, log) {
    const { measured, unmeasured } = collectDoneTicketsWithRedTestVerdict(runtime);
    const names = (list) => list.map((o) => `${o.id}${o.title ? ` (${o.title})` : ''}`).join(', ');
    ...
    if (unmeasured.length > 0) {
        ...
        log(`Finalize: ${unmeasured.length} ticket(s) Done over an empty-because-unmeasured ` +
 ...
 */
function cancelledOutcome(cancelMarker, log) {
