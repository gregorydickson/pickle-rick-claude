...
- `AC-DPGT-2` — When `largeTierDetachedEnabled()` is true and every still-unfinished ticket has a live `state.detached_worker` (`isProcessAlive` true), the runner performs a bounded grace-drain before emitting `pipeline_phase_incomplete`. Two distinct windows: (a) **eligibility** is keyed to the existing detached-poll / worker-timeout window (`state.worker_timeout_seconds`, fallback `Defaults.WORKER_TIMEOUT_SECONDS`) — the worker must still be within its own poll budget to qualify, NOT a new constant; (b) the **wait** itself is a deliberately-bounded small fixed sub-slice (`GRACE_DRAIN_MAX_PASSES * GRACE_DRAIN_PASS_MS` = 15s), intentionally NOT the full worker-timeout budget, so the pipeline never blocks its exit for up to an hour on a rare cap race. Test: with the detached lifecycle enabled and the grace-drain sleep injected (no real wait), simulate a live detached worker inside its eligibility window and assert `reportPhaseIncomplete` DEFERS — fires the grace-drain pass loop — until the worker either commits (→ AC-DPGT-1 excludes it, no `pipeline_phase_incomplete` stamp) or the bounded drain cap elapses with the worker still uncommitted (→ stamp IS written).
