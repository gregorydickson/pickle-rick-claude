...
|---|------|-----|-------|--------|
| **B-PCOMP** | **B-PCOMP — beta 2.0 pipeline completion** — ground-truth gates at both boundaries (composes WS-D1=B-RFCU start gate + WS-D2 finish gate) | **P1** | **PLAN DRAFTED 2026-06-20 — THE GA-track release.** Collapses R-RCFF (D1, start) + R-CECB (D2, finish) — one defect, two faces: gates trust an LLM bookkeeping artifact over the repo. Ground-truth fix: readiness derives forward-created from the bundle's creation set; completion attributes from the branch (not just the stamp), salvage reconciles-before-archiving, bystander work preserved. **Done = 4/4 phases hands-off.** Refine + launch next. | `p1-beta2-pipeline-completion-2026-06-20.md` |
| B-RFCU | **B-RFCU** readiness forward-created unification (= B-PCOMP **WS-D1**, start gate; R-RGO/R-RPRA/R-QGSK/**R-RCFF** family) | P2 | **PLAN DRAFTED 2026-06-20.** Subtraction: wire the existing `buildBundleCreationIndex` into the contract resolver + populate field-paths + demote forward-created refs to advisory; no new grammar, no new skip flag. Drain as B-PCOMP WS-D1. | `p2-readiness-forward-created-unification-2026-06-20.md` |
