...
**▶ STATE: `origin/main` = v2.0.0-beta.35 (HEAD `9f2a0592`); tree clean; no active pickle-rick pipeline. ✅ B-WSPU SHIPPED beta.35 — the dual worker-spawn model is COLLAPSED (detached lifecycle fully deleted, all tiers now synchronous re-spawn-resume). Deployed runtime = beta.34 (B-SSVR); ⏸️ B-WSPU install.sh deploy HELD for field-soak (validate ≥2 bundles incl. a >600s ticket before deploying a spawn-model change to the shared runtime). Field evidence FOR the collapse: WS-1 as a detached worker silent-died building its own deletion; the synchronous path built the whole thing clean. ⚠️ On eventual B-WSPU deploy: update the project `CLAUDE.md` R-PSRB dodge line (`complexity_tier: large → detached path`) — detached is gone. Review phases were skipped (review-subprocess SIGHUP instability this session); full release gate was the arbiter (all tiers green).**
...
| Item | Value |
|---|---|
| Version (source = deployed) | **v2.0.0-beta.35 (source) / v2.0.0-beta.34 (deployed)** — beta.35 = B-WSPU collapse the dual worker-spawn model (delete detached lifecycle, unify synchronous re-spawn-resume; ~1000+ LOC pure subtraction; WS-1 `f2e795e6` … bump `9f2a0592`); pushed + GitHub-released, **install.sh deploy HELD for field-soak** (spawn-model change — do NOT blind-deploy to the shared runtime; deployed runtime stays beta.34). Prior: beta.34 B-SSVR (deployed) · beta.33 gate-overreach subtraction · beta.32 R-LTDM detached-poll throttle. |
