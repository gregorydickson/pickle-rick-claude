...
**▶ THE IMMEDIATE NEXT STEP is a field-soak, not a fix (unchanged, now on beta.37):** the deployed runtime is
single-lifecycle (beta.35) AND single-completion-seam (beta.37) with the orphan reaper live. Run the next real
bundles via `/pickle-pipeline` — ≥1 with a genuine >600s ticket, and ≥1 `--backend codex` rep (R-CXHANG was
the blocker; the untagged/hallucinated-sha class is now reconciled wrapper-side). The GA bar stays the north
star: N bundles hands-off in a row; anything surfaced is a clean bug on a much smaller runtime.
