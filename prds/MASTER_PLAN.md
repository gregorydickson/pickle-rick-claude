...
## ⏯ RESUME HERE (updated 2026-06-28 — B-APNC SHIPPED beta.27)
...
**Next action: R-WPEX↻ (P2) — HAND-BUILD in-process, repro-first.** Full plan + diagnosis in
`BUG-REPORT-2026-06-28-rwpex-detached-worker-logflush-recurrence.md`. Sharpened trigger (n=2): the stall fires
ONLY when the manager spawns a **detached `spawn-morty` `claude -p` worker** (B-APNC: 3 spawned, all 0-byte
flush-died → 0/4; B-CWGE: 0 spawned, all in-process → clean 4/4). The in-process `morty-implementer` path is
IMMUNE — which is why hand-building in-process is safe AND why a pipeline run is risky (its own workers run the
buggy path). Likely NOT load (idle-box repro). **Step 1 = a deterministic repro** (detached `claude -p` + large
stdout → assert `worker_session` drains on exit; `flushAndExit`/`once(sessionLog,'close')` in
`services/worker-shutdown.ts` + detached spawn in `bin/spawn-morty.ts`), w/ and w/o load. Then fix the drain
race; secondary = manager inline-recovery commit-per-ticket. **[[R-SIGF]] full scope-auto-extension** remains the
separate codex GA track. Pre-build: re-`git log`/grep HEAD for ACs (`feedback_prelaunch_residual_check_stale_findings`).

## Status

| Item | Value |
|---|---|
| Version (source = deployed) | **v2.0.0-beta.27** — B-APNC anatomy-park convergence/complexity guard + subtract-pass discipline (R-APNC); built on claude (R-WPEX log-flush recovery mid-build), deployed via install.sh 2026-06-28. |
| Latest GitHub release | **v2.0.0-beta.27** (B-APNC; prerelease). Prior: beta.26 B-CWGE · beta.25 B-PXBO · beta.24 B-RPGT · beta.23 B-DURA + reliability program. |
...
|---|------|-----|-------|--------|
| R-WPEX↻ | **R-WPEX RECURRENCE + inline-recovery gap (2026-06-28 B-APNC build; initially mis-filed R-WUDC, corrected)** pickle exited `pipeline_phase_incomplete` 0/4. ROOT CAUSE: the detached `claude -p` spawn-morty workers **died on log-flush** (manager: "worker died on log-flush, not on the work") — the KNOWN R-WPEX/worker-silent-exit class, previously MONITOR-only ([[project_wpex_worker_silent_death_monitor]]), now a fresh non-load repro. Manager inline-recovered both tickets (committed `5dc68f98`→`4ad6d2d9` Done; delivered WS-3 `6803d887` via in-process morty-implementer) but the pickle loop EXITED after committing only 1/2 → WS-3 left uncommitted+Todo → 0/4. NOT a new doc-only/B-DURA-boundary bug (that hypothesis retracted). | P2 (NEXT) | **OPEN — restart-ready, HAND-BUILD in-process (R-PSRB).** TRIGGER (cross-session, n=2): the stall fires ONLY when the manager spawns a detached `spawn-morty` `claude -p` worker — B-APNC spawned 3 (all 0-byte/flush-died) → 0/4; B-CWGE spawned 0 (all in-process) → clean 4/4. The in-process `morty-implementer` path is IMMUNE. Likely NOT load (B-APNC pickle ran 21:07-22:02Z on an idle box; gates ran before/after). (1) **Repro FIRST** (test-driven): detached `claude -p` + large stdout, assert `worker_session` drains on exit (`flushAndExit`/`once(sessionLog,'close')` in `worker-shutdown.ts` + detached spawn in `spawn-morty.ts`), w/ and w/o load; distinct from R-WSE-2 SIGKILL/OOM 0-byte class. (2) Fix the drain race. (3) Secondary: manager inline-recovery commit PER-TICKET. Hand-build in-process (the immune path) since the fix edits the path a pipeline's own workers run on. | `BUG-REPORT-2026-06-28-rwpex-detached-worker-logflush-recurrence.md` |
...
> **Recently shipped + swept to `archive/`:** **B-APNC (R-APNC, beta.27, 2026-06-28)** — anatomy-park
