...
## ▶▶ NEXT FIX RUN (2026-07-02) — B-1SEAM: collapse the asymmetric-fix siblings [READ FIRST — build target for the next context]
...
|---|------|-----|-------|--------|
| B-1SEAM | **B-1SEAM** (P1) — collapse the 3 asymmetric-fix siblings into single seams. **WS-1 [[R-AICF]] (P1, R-PSRB hand-build):** unify done-guard + phantom-Done watcher + `readEvidence()` Done-flip onto ONE predicate (honor frontmatter `completion_commit` + `allow_inferred_completion_commit`; or deterministic hash-tag trailer injection) — reopens the completion-oracle-plurality meta-defect + [[R-CCC]]/[[B-PDBL]]. **WS-2 [[R-PSCG]] (P2, pipeline-safe):** symmetric citadel self-heal (`start_commit` too) + `setup.js --resume` recompute + non-git-cwd WARN. **WS-3 [[R-MACB]] (P2, pipeline-safe):** port B-PCOMP bystander-stash to `autoRescueDirtyTree` (owned-paths-only). Pin each collapse with a call-site audit. Build on **claude**; hand-build all three (WS-1 dominates) or hand-build WS-1 + pipeline WS-2/WS-3. | P1 | **PLANNED — author PRD next context** (see `## ▶▶ NEXT FIX RUN` top section). NEW TOP OF QUEUE. | 3× `BUG-REPORT-2026-07-02-*.md` |
| R-CXHANG | **R-CXHANG** (codex orphaned-worker-proc reaper) — codex worker procs from crashed/force-killed sessions orphan + accumulate across days (R-SLEAK amplified — codex `exec` blocks on API reads, never self-exits), saturate the machine, starve new pipelines. **Killed run 1 of the B-SIGFH soak.** The negative-PID subtree reap exists but runs only on CLEAN teardown/timeout; no collector for orphans-from-crashed-sessions. Fix (reuse-first): a setup-time `reapOrphanedWorkerProcs` that REUSES `killProcessTree`/`reapChildSubtree` to reap procs no live session owns (positive-ownership required; `PICKLE_ORPHAN_REAP=off` kill-switch; no schema bump). Closes the R-SLEAK session-GC gap. | P2 | **PRD READY (authored 2026-07-01 from the B-SIGFH soak).** #1 codex-GA-evidence unblocker — build on claude before the next codex soak. Pipeline-safe (spawn/reaper paths, not R-PSRB salvage). | `p2-bug-fix-bundle-r-cxhang-codex-orphan-proc-reaper-2026-07-01.md` |
| B-RSHM | **B-RSHM** subtraction — retire stop-hook dead-code + the chain_meeseeks subsystem. **WS-1:** strip the stop-hook's dead non-tmux interactive branches (default/review-clean/checkpoint continuations, degenerate-nudge BLOCKs, `active:false` mutation, `session-end`) — KEEP `evaluateManagerIdleBackoff` + `maybeSpawnUpdateCheck` + tmux-passthrough (both run under tmux — wholesale removal REJECTED). **WS-2:** retire the whole chain_meeseeks/meeseeks review loop (de-wire `--meeseeks`/`--chain-meeseeks` + ~15 mux-runner `meeseeks.md` template branches + `transitionToMeeseeks` + MonitorMode + settings, then delete the 2 already-deprecated command files). SHARED (do NOT touch): `morty-reviewer`/`morty-phase-reviewer` agents, `meeseeks_pass` event. No schema bump (retain optional fields). Pure subtraction; pipeline-safe (not R-PSRB). | P2 | **PRD READY (authored 2026-07-01 from a blast-radius map).** Operator-requested subtraction (north star). Build when the machine is clear (deploy-held beta.36 + concurrent LOA-1570 share the runtime). | `p2-subtraction-retire-stophook-deadcode-and-chain-meeseeks-2026-07-01.md` |
