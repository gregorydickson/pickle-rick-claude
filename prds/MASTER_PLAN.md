...
## ⏯ RESUME HERE (updated 2026-06-28 — R-WPEX SHIPPED beta.28)
...
pipeline-safe. **⏸ STALLED + PARTIALLY SHIPPED (2026-06-29):** refined into 10 tickets (readiness GREEN); pickle
shipped **ticket 1 `6d0892db` → commit `18f2ccc1`** (shared-detector extraction, local/unpushed) then **stalled
on ticket 2 (medium tier) and exited `pipeline_phase_incomplete` 0/4** — NOT a B-SIGF defect: a NEW manager
failure mode **[[R-MWBG]]** (the `claude -p` pickle manager Bash-`run_in_background`ed the medium worker + ended
its turn → background task killed at turn-end → 0-byte log → no progress). Babysitter recovered (ticket 1 Done
preserved; ticket 2 reset → Todo). **Relaunch DEFERRED**: blocked on (a) the 5-hour rate limit (0.95 util at
incident, ~3h to reset) and (b) the R-MWBG manager-prompt fix (else any relaunch re-stalls on medium tickets).
B-SIGF resumes after R-MWBG's cheap half ships + rate headroom returns. Session resumable at
`2026-06-29-e68fda19`.
...
| Item | Value |
|---|---|
| Version (source = deployed) | **v2.0.0-beta.28** — R-WPEX detached-worker session-log fsync on the success/close drain (reuse `bestEffortFdatasync`); built on claude (hand-built in-process via ultracode Workflow), deployed via install.sh 2026-06-28. |
| Latest GitHub release | **v2.0.0-beta.28** (R-WPEX; prerelease). Prior: beta.27 B-APNC · beta.26 B-CWGE · beta.25 B-PXBO · beta.24 B-RPGT · beta.23 B-DURA + reliability program. |
...
> **Recently shipped + swept to `archive/`:** **R-WPEX (beta.28, 2026-06-28)** — detached-worker
