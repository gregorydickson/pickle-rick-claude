...
[[R-DPGT]]. So on codex: *completion-evidence* soak ✅ / *phase-completion* soak 🔴. The reliability *code* is
proven on claude AND now (completion-class) on codex; the remaining GA gap is R-SIGF + R-DPGT, not the
completion oracle.
...
not just signature/type callers)** + the new **[[R-DPGT]] detached-phase-gate grace** — together they own the
residual `0/N phases → downstream skipped`. See
...
> the phase-completion blockers. The 2026-06-24 sweep also archived ~30 older shipped PRDs/design-notes.
>
