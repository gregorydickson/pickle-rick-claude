...
## 📐 Review-toll note: high, but not fixed (measured 2026-09-19, operator: not a big deal)

The bundle-sizing clause calls ANATOMY-PARK + SZECHUAN a *"near-fixed ~300-minute toll"*. Measured on
two bundles of near-identical ticket count: **B-VERDICT 202m, B-INVENTED 1031m** (anatomy-park alone
134m/7 iter vs 995m/41 iter; 21 vs 45 commits). The toll tracks **findings**, i.e. diff size, not
tickets. PICKLE linearity holds at ~27 min/ticket.

**Sizing policy is unchanged** — one review cycle still beats two. The only consequence: plan a
bundle's wall-clock from its expected diff, not its ticket count.
...
2. **Bundle B-LANES:** (a) delete the dead teams mode (manager Phase 3.B, `--teams`/`--max-parallel`, the `morty-implementer`/`morty-reviewer` agents, docs) and close #43 on evidence; (b) `discoverSubsystems` partitions one level deeper; rotation stays sequential. One bundle, because both land in the single `extension` review lane. Then read E1 off this and later bundles: passes and wall-clock per lane vs the B-INVENTED baseline of 34 passes / 995 min.
   **Update (2026-09-24):** refinement measured that this repo's 34/50-pass runs were UNSCOPED; scoped self-runs converge in 2 passes and 1–20 min. The operator reports long anatomy-park/szechuan runs in the field even when branch-scoped, and the local corpus is self-hosted only, so WS-2 stays but is redesigned to be language-agnostic. **B-LANES is EXPERIMENTAL: built on branch `exp/b-lanes`, not merged to `main` until E1 on field-like bundles shows a gain.** E3 ran: inconclusive (saturated); see the spike report.
...
| **3** | **B-CITADEL (#42, PREMISE CORRECTED 2026-09-21)** | **NOT** "build an iterate-until-resolved loop" — that loop EXISTS (`citadel_max_remediation_cycles`, default **3**, `finalize-gate.ts:34`). Measured cause: `remediationSeverityThreshold` is `strict ? 'High' : 'Critical'` (`pipeline-runner.ts:3059`), `citadel_strict` defaults **false** and `pickle_settings.json` has no `citadel` key — while the dominant class `citadel-ac-coverage-<AC>-test` is emitted at **`High`** (`ac-coverage-scorecard.ts:303`). The dominant class is excluded by ONE severity notch on every run. Prefer SUBTRACTION (collapse or flip the strict/non-strict distinction) over a new mechanism | #42's binding mutation-verification constraint still applies |
...
| **NEXT** | **B-REFMODEL (#46)** — refinement is the ONLY spawn surface with no model selection | Census at HEAD `7db46c6d`: `spawn-morty` **40** model refs, `microverse-runner` **20** (`judgeModel` + `DEFAULT_JUDGE_MODEL`), `mux-runner` **11**, **`spawn-refinement-team` ZERO**. The operator convention already ships (`judge_model_claude`/`judge_model_codex` in `pickle_settings.json`), so the fix is **widening an established pattern**, not inventing a surface. Autonomy root: a model-level refusal took the whole dogfood path to zero because refinement alone had no knob to turn | small bundle — backlog is drained to zero bugs, do NOT pad it with invented roots |
| ~~prev~~ | ~~**B-MEASURED**~~ — `prds/p1-b-measured-what-the-instrument-knew-but-could-not-say.md` | Four roots, one thesis (*the instrument discards what it holds*): **R1** `baseline_score !== 0` in-band sentinel · **R2** judge output contract unenforced · **R3** citadel threshold excludes the dominant `High` class (#42 corrected) · **R4** no version stamp on authored commits (#44). Composed by SHARED SURFACE — all four land in `bin/`, so the ~300-min review toll is paid once | every mechanism re-grepped at HEAD `cbce36cd`; every path and symbol verified to resolve before commit |
...
**Open `OPEN BUG` + `TOP ITEM` sections (N4 re-census 2026-09-14): 1** — the `9` above is the 2026-08-31
...
a bundle.** Rows not re-measured: `R-TIERWEDGE`, `R-RWNF`, `R-JPCM`, `B-APRP`,
...
**Addendum 2026-09-07 (G5, ticket `84104a32`):** `R-TCVC` and `R-HNCG` independently re-measured against
HEAD and struck from the not-re-measured list above — both **ALREADY-SATISFIED**, confirming their
existing row verdicts. Mechanism check, per row:
- `R-TCVC`: `grep -n "CLASSIFIER_EXPENSIVE_VERIFY_KEYWORDS" extension/src/services/pickle-utils.ts` →
  live at `:664` (list) and `:723` (wired into the existing ±1 keyword delta in `classifyTicketTier`,
  `:711-729`). `grep -rn "CLASSIFIER_EXPENSIVE_VERIFY_KEYWORDS" ~/.claude/pickle-rick/extension/services/pickle-utils.js`
  → live at `:525`/`:582` in the deployed tree.
- `R-HNCG`: `grep -n "appendAutoHandoffFallback\|worker_artifact_progress" extension/src/bin/mux-runner.ts`
  → the auto-handoff fallback writer is live at `:10707` (definition) and `:10783` (call site), gated on
  a zero-artifact-progress spawn via `worker_artifact_progress` bookkeeping (`:10578-10639`). Same
  symbols confirmed live in the deployed tree (`~/.claude/pickle-rick/extension/bin/mux-runner.js:8849`
  `appendAutoHandoffFallback`).
- **Correcting a false-negative in the first pass of this ticket's own census:** a `ls`/`grep` against
  `extension/tests/worker-artifact-progress-tracking.test.js` (top-level `tests/`) came back absent,
  which would have wrongly read as an unpinned mechanism. The file is actually at
  `extension/tests/integration/worker-artifact-progress-tracking.test.js` (added alongside the
  `+40`-line `classify-ticket-tier.test.js` change, both in `6d80f415`, confirmed via
  `git show --stat 6d80f415` read in full rather than truncated). Textbook case of "if a census is
  all-negative, suspect the census" — the mechanism was never missing, the first grep's scope was.
- Both suites run green together: `node --test tests/classify-ticket-tier.test.js
  tests/integration/worker-artifact-progress-tracking.test.js` → **32/32 pass**, 0 fail, run
  2026-09-07 from `extension/`.
- `6d80f415` confirmed an ancestor of HEAD (`git merge-base --is-ancestor 6d80f415 HEAD`, exit 0). No
  fix required — the rows already carried the correct `✅ STALE — SHIPPED` verdict; this is
  independent confirmation, not a new closure. Net open: unchanged (**18**) — neither row was counted
  among "remaining" before this pass.
...
|---|------|-----|-------|--------|
| R-APBGL | **R-APBGL**: anatomy-park's R-APXG-3 deferral-cap gate re-run evaluates raw failures (`baseline_used: false`) and refuses to converge on `new_failures_vs_baseline: 0`. Pre-existing monorepo gate gaps (the `packages/app` missing `lint:quiet`, and lint/test `GATE_CHECK_TIMEOUT` at 60 s/300 s) therefore end the phase `finalize_gate_failed:error`. It also pushed a pickle worker to add a gate-appeasement script to the product repo. Observed on LOA-2425, session `2026-09-24-c13bddae`. | P1 | **OPEN**, filed 2026-09-25 (capture-only) | `prds/BUG-REPORT-2026-09-25-anatomy-park-cap-gate-ignores-baseline.md`, GH #48 |
...
| — | **B-CGHARD** codegraph harden-then-soak (v2.1 — supersedes the B-CGPROBE build; operator keep-and-refine ruling 2026-07-11) | P2 | ✅ **RE-MEASURED 2026-09-06 (TIER-3.13d drain, ticket `108bd54f`) — hardening SHIPPED, this row's "QUEUED — LAUNCHED" cell was stale.** `0ee0b241` (2026-07-17) already recorded WS-A/B shipped and narrowed remaining scope to WS-CGH-D; verified live at HEAD: query-hang surface subtracted (`grep -n "getImpactRadius\|runSyncQuery" extension/src/services/codegraph-service.ts extension/src/services/codegraph-query-runner.ts` → zero hits), verify-before-inject `stale_refs`/`dropped_stale` live (`spawn-morty.ts:1077,1093,1108`), input harvester fixed (`d2784466`), index freshness gates on HEAD-sha not just mtime (`setup.ts:181-197`, `9c7f1f10` — goes further than this row's `!isResume` description), sync-at-key-points live (`mux-runner.ts:12045` `await service.sync()`). The one open sub-item — the soak run — is addressed in the row below. | `archive/bundles/p2-codegraph-harden-then-soak-v2.1.md` |
| — | **B-CGPROBE** codegraph efficacy instrument + verdict run (v2.1 branch) | P2 | ✅ **CLOSED 2026-09-06 (TIER-3.13d drain, ticket `108bd54f`) — optional-revisit trigger can no longer fire.** `runProbe` confirmed still a stub (`codegraph-efficacy-probe.ts:173-175`, returns `loadCorpus(...)` only, no WITH/WITHOUT spawn-and-score loop — unchanged since 2026-07-11). Its revisit condition ("ONLY if the B-CGHARD soak telemetry is ambiguous") is now unreachable: no soak was ever run (see Soak RUN row), and separately [[B-CGCAP]]'s keep-vs-subtract question was already closed by **operator decision** ([[B-CGEN]], `MASTER_PLAN.md:2465-2469` — "the subtract arm is OFF the table"), not by probe evidence. Nothing left to gate. Stub + corpus left as-is; no build warranted. | PRD to author at launch (auto-author per babysitter protocol); evidence audit in the [[B-CGCAP]] row |
| — | **Soak RUN** — post-GA operator execution of the [[B-CGHARD]] WS-C soak protocol | P2 | ✅ **MOOT 2026-09-06 (TIER-3.13d drain, ticket `108bd54f`).** `find prds -iname "*codegraph-soak*"` confirms `prds/research/codegraph-soak-baseline.md` was never created — the soak was never run. Its purpose was to feed [[B-CGCAP]]'s keep-vs-subtract verdict; that verdict is already decided and executed by operator override ([[B-CGEN]]), irrespective of soak telemetry, so there is nothing left for a soak to gate. Separately, the "deliberate codegraph-enabled window" precondition this row assumed is retired: `install.sh:529` MANAGED_KEYS now forces `codegraph.enabled`/`index_at_setup` true on every deploy (CLAUDE.md Tune-Back CUJs, "CUJ #2 ... retired as of WS-B3") — codegraph is never off, so there is no window left to schedule. | README `### Soak protocol` + `archive/bundles/p2-codegraph-harden-then-soak-v2.1.md` |
...
| — | **B-CGCAP** codegraph default-on (v2.1) | P2 | ✅ **CLOSED 2026-09-06 (TIER-3.13d drain, ticket `108bd54f`) — this cell's evidence-gated NO-GO was stale; superseded by the operator-decision cell at `MASTER_PLAN.md:2442` ("SUPERSEDED ... KEEP + ENABLE. See [[B-CGEN]]"), which SHIPPED.** Verified live at HEAD: `install.sh:529` MANAGED_KEYS forces `.codegraph.enabled = true \| .codegraph.index_at_setup = true \| .codegraph.expose_mcp_to_workers = true` on every deploy (`70b0dcef` "enable codegraph by flipping the MANAGED_KEYS value (WS-B3)"); `pickle_settings.json:3-5` ships `enabled: true`; the guard test was inverted per plan (`extension/tests/codegraph-default-optin.test.js:16-21` now asserts `=== true`, was `=== false`); `CLAUDE.md:298` documents "Enabled by default (WS-B3)". The keep-vs-subtract decision this row tracked is closed — codegraph ships enabled unconditionally, decided by operator override rather than by the evidence audit this cell describes. | `p2-codegraph-default-on-capability-v2.1.md` *(pinned)* + evidence audit this branch |
| — | **B-GIMA** guard-inventory & finding-shape mining audit (v2.2) | P2 | 🔄 **RE-MEASURED 2026-09-06 (TIER-3.13d drain, ticket `108bd54f`) — contingency resolved, but NOT built (out of scope for a verify-only drain ticket).** `grep -rn "guard-inventory\|GuardInventory\|finding-shape mining" extension/src/` → zero hits; only PRD docs exist. Its WIN/LOSS/INCONCLUSIVE contingency on [[B-CGPROBE]]'s verdict is now moot: the LOSS branch ("codegraph subtracted, B-GIMA loses substrate") is impossible — [[B-CGEN]] already took subtraction "OFF the table" (`MASTER_PLAN.md:2468-2469`) by operator decision — and the WIN branch's precondition never fired (`runProbe` is still a stub, `codegraph-efficacy-probe.ts:173-175` — no verdict was ever produced). AC-GIMA-A5's substrate dependency is unconditionally satisfied regardless: codegraph ships `enabled: true` by default (see [[B-CGCAP]] row). Net: B-GIMA is unblocked backlog, not gated on an ambiguous probe result — still unscheduled P2 work, not built here. Sole survivor of the slop-gate comparative review: an inert, report-only tool that operationalizes subtract-before-add (DELETE never-fired-guards / JUDGE-CALIBRATION conf<80 drops / ADD sort-hints); do NOT build the throwaway scanner the PRD warns against. Kill-criteria baked in: report-only or dead. Evidence-gating insight from the same review → release-2 R-CWGE/R-DOTR, NOT here. | `p2-guard-inventory-mining-audit-v2.2.md` |
