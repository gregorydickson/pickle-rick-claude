...
2. **Review-phase gate gaps** (R-CECX run-3 follow-up #2, facets 4–6 — NEW, see drain row): citadel/anatomy/
   szechuan commit tsc/lint-RED code because their gates measure LLM/quality scores, not typecheck; a transient
   `529 Overloaded` aborts szechuan (`baseline_unmeasurable_unrecoverable`) instead of park-and-retry.
...
ranked intervention-rate report = the GA-readiness ledger. **Soak ledger (pre-B-DURA): 1 clean (R-WSDO,
claude, single-ticket, 4/4 hands-off) + codex multi-ticket 0-for-3** — LOA-1363 run 1 → [[R-CECX]]
