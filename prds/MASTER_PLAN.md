...
| B-WSPU | **B-WSPU — collapse the DUAL worker-spawn model (operator-flagged 2026-06-30, the concrete 5th-meta-defect / recovery-sprawl lever).** There are two parallel ways to spawn a ticket worker: the **synchronous** path (`runIteration` → manager `claude -p` orchestrates, naturally throttled by blocking on the manager turn) and the **detached** path (orchestrator spawns spawn-morty directly `detached:true` + a hand-rolled poll loop, AC-R-WPEXA-*). The detached half exists to escape the 600s Bash-tool ceiling for long (>600s) workers — but it is a whole second lifecycle with its own dispositions, and BOTH recent field bugs in this area live there: [[R-LTDM]] (premature poll verdict, beta.32) and [[R-WPEX]] (detached-log flush-death, beta.28). Every detached-path bug is a tax on maintaining two models that drift. The real subtraction is to collapse to ONE worker lifecycle — e.g. remove the 600s-ceiling constraint at its source so no detached path is needed, OR unify on a single orchestrator-owned spawn+poll for all tiers and retire the synchronous fork. This SUBTRACTS a code path + a failure-mode class rather than adding another guard (R-LTDM's poll-sleep was a patch, not the cure). | P2 | **OPEN — DESIGN + OPERATOR SIGN-OFF REQUIRED (capability-surface change).** Not auto-launchable: needs a design note enumerating which model survives, what breaks (AC-R-WPEXA-* tests, the manager-orchestration prompt contract, the 600s-ceiling assumption), and a migration that keeps `runIteration` invariants. Reliability-first: deferred behind shipping the in-flight reliability fixes; the poll-throttle (beta.32) holds the line until then. No PRD yet — author one with the design only if pursued. | `BUG-REPORT-2026-06-30-beta31-medium-tier-routes-detached-poll-falsefails-in-1s.md` (root-cause context) |
