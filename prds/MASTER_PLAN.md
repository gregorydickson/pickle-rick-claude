...
**Updated 2026-06-30.** Shipped + deployed + released through **v2.0.0-beta.31** — but the FIRST pipeline run on beta.31 surfaced **[[R-LTDM]] (P1)**: the beta.31 R-MWBG runtime half routes explicit-medium tickets through the detached poll path which false-fails them in ~980ms (blocks autonomous medium-tier builds; R-MWBG REOPENED). Detail below. (R-MWBG runtime half:
...
**A. Reliability fixes (drain first).** ⚠️ **NEW TOP P1 — [[R-LTDM]]: the beta.31 R-MWBG runtime half REOPENED a worse
break.** The first `/pickle-pipeline` run on beta.31 (2026-06-30, B-SSVR) proved that `tierExceedsBashCeiling` now routes
explicit-`medium` tickets through the LARGE detached poll path, which false-fails them in **~980ms** (premature no-progress
verdict before first-artifact latency) → false `EPIC_COMPLETED`, pickle `0/N`. **This blocks ALL autonomous medium-tier
mux/pipeline builds on the deployed runtime** (verified: `claude -p` healthy, foreground worker runs 110s+, reproduced 3×
incl. clean-state). Build it ≥`medium` with the detached path verified; `PICKLE_LARGE_TIER_DETACHED=off` is NOT an escape
hatch (it punts to interactive, doesn't build). Until R-LTDM ships, medium-tier bundles must be HAND-BUILT (B-SSVR was).
