...
| Item | Value |
|---|---|
| Version (source = deployed) | **v2.0.0-beta.30** — B-RELHYG reliability hygiene (R-MVFM microverse plateau-memory + date-fixture time-bomb audit + R-OMTD afterEach reap) + B-MWBG attempt/revert (net beta.29 mux-runner) + dogfood-by-default + CLAUDE.md token-optimize; built on claude via clean `/pickle-pipeline` (4/4, 178m), deployed via install.sh 2026-06-29. |
| Latest GitHub release | **v2.0.0-beta.30** (B-RELHYG; prerelease). Prior: beta.29 R-SIGF+R-MWBG-half-1 · beta.28 R-WPEX · beta.27 B-APNC · beta.26 B-CWGE · beta.25 B-PXBO. |
