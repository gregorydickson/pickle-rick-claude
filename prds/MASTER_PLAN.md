...
| B-RRPC | **B-RRPC** — recovery resolver-plumbing consistency *(pipeline-safe subtraction)*. (a) DELETE the dead `evaluateSimpleManagerRelaunch` boolean overload (`manager-relaunch.ts:171-195`, zero production callers, test-pinned only); (b) fold the hand-duplicated `resolveBreakerRecoveryGraceSeconds` (`mux-runner.ts:7619`) into `resolveHardeningSettings` (same `hardening:` JSON block — a documented irregularity); (c) move the bare `BOUNDED_ESCAPE_CAP` const (`mux-runner.ts:5897`) onto `hardening.bounded_terminal_escape_cap` for uniformity with its two ledger siblings. Resolver-layer only, does NOT touch the salvage path. | P3 | **ANALYSIS DONE 2026-07-07 — ready to scope. Pipeline-safe (NOT R-PSRB).** | `RECOVERY-SPRAWL-COLLAPSE-ANALYSIS-2026-07-07.md` |
| B-CSHYG | **B-CSHYG** — completion-seam hygiene *(pipeline-safe subtraction; the B-1SEAM residual)*. (a) `salvageCleanTree`'s `backfillDone` dep (`lib/salvage-ticket.ts:141`) is never wired in production (documented `committed-done` clean-tree disposition is permanently dead, drifted from its docstring) → wire `attributeBoundaryHeadMoved` through OR delete the branch + reconcile docs; (b) DELETE the orphaned zero-caller duplicate scanner `hasCommitReferencingTicketSince`/`findMatchingCommit` (`pickle-utils.ts:~1103-1165`). | P3 | **ANALYSIS DONE 2026-07-07 — ready to scope. Pipeline-safe (NOT R-PSRB).** | `RECOVERY-SPRAWL-COLLAPSE-ANALYSIS-2026-07-07.md` |
