...
## ⏯ RESUME HERE (updated 2026-06-27 — post codex-soak)

**2026-06-27 codex soak of B-PXBO: ATTEMPTED, REVERTED, findings banked.** Ran B-PXBO on `--backend codex`
(manager-decomposed; refinement produced no tickets). Build reached pickle→citadel→anatomy-park hands-off, BUT
the codex execution was broadly unreliable and was **reverted** (main back to `e107fab2` beta.24, nothing
shipped). Two findings logged — see
`BUG-REPORT-2026-06-27-codex-worker-gate-not-enforced-and-anatomy-guard-piling.md` + drain rows [[R-CWGE]]
(P1) and [[R-APNC]] (P2): **(A) the codex worker quality gate was not enforced** — 2 of 4 workstreams shipped
real defects (WS-1 broke 3 relaunch tests deterministically; WS-4 broke the `readEvidence` "explicit-SHA-wins"
+ R-CCQF invariants), all `Done`-flipped with `completion_commit`s over lint/test-RED code (R-DOTR class at the
worker-gate level); **(B) anatomy-park guard-piled** 27 commits into `readEvidence` (complexity 13→31) without
converging. **The B-PXBO DESIGN is sound and the PRD is unchanged/ready.**

**Next action: rebuild [[B-PXBO]] on `--backend claude`** (whose worker gate IS enforced), OR land [[R-CWGE]]'s
codex `runWorkerGate` fix first and re-soak codex. PRD:
`prds/p2-bug-fix-bundle-b-pxbo-phase-exit-boundary-oracle-2026-06-26.md` (authored 2026-06-26, `4437395a`).
Consolidates **R-DPGT** + **R-DOTR** + **R-CRSR** + LOA-1588 foreign-hash. Thesis: the pickle phase-exit /
per-ticket-budget boundary doesn't read the single `readEvidence` oracle. ACs grounded in real `file:line`
(mux-runner.ts:3134/4674/6362, pipeline-runner.ts:3832/3131, spawn-morty.ts:1255, ticket-completion-evidence.ts:421).
**Codex-soak lesson: never trust a codex worker `Done` as gate-green — run the full local gate before believing it.**
