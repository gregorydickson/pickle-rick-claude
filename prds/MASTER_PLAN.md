...
**▶ NEXT ACTION — [[R-MWBG]] runtime half (P1). PRD AUTHORED + ready to hand-build:**
`prds/p1-bug-fix-bundle-b-mwbg-runtime-detached-tier-gate-2026-06-29.md` (2026-06-29 babysitter tick).
half-1 (manager-prompt foreground-spawn discipline) SHIPPED beta.29 (`795e539e`) and is validated. The
**runtime half is OPEN and is the top drain item**: at the two detached-worker gates in `bin/mux-runner.ts`
(spawn ~:10766, poll ~:10803) replace the `state.current_ticket_tier === 'large'` literal with a budget
predicate `state.worker_timeout_seconds > BASH_TOOL_CEILING_SECONDS (600)` — so **medium (3600s)** routes through
the *same shipped* detached lifecycle (`spawnDetachedLargeTierWorker` / `state.detached_worker` / poll loop /
`routeLargeTierTicket` fallback / `largeTierDetachedEnabled` kill-switch — all reused verbatim). Per-tier table
(`pickle-utils.ts:505`): trivial 300 / small 600 / **medium 3600 ← the gap** / large 4800. **⚠️ CORRECTION
(verified from session `2026-06-29-e68fda19` this tick):** the prior claim that "the 4 large-tier workers ALSO
died → the large detached path is itself failing, uncovered by beta.28" is **WRONG**. The 4 large tickets routed
through the detached lifecycle CORRECTLY (`large_tier_worker_spawned` + `large_tier_worker_poll` bursts +
`ticket_ladder_exhausted` clean advance); they died with the **worker-process-cancellation** signature
(`"SessionEnd hook … Hook cancelled"`, `"no stdin in 3s"`) under the incident-time **five_hour rate util 0.95**
wall — the R-WSE/rate-limit class, NOT a detached-path defect. So the runtime half is ONLY the medium-tier gate
gap; the large-death story is out of scope (rate, env). **R-PSRB does NOT apply — run as a NORMAL pipeline**
(`/pickle-pipeline --scope branch`): the spawn-gate edit does not touch the salvage/completion path (so it can't
corrupt the in-flight worker building it), and the running pipeline executes the deployed beta.29 JS not the
source (zero mid-run effect — the diff lands only at the closer's `install.sh`). Dissolve the one residual
(deployed medium-no-detached-path biting a medium fix ticket) by pinning the load-bearing ticket
`complexity_tier: large`. Correction to the reflexive hand-build stamp — R-PSRB is the narrow salvage/completion
catch-22 (B-PCOMP), not "anything touching mux-runner."
