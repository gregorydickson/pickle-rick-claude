...
gap; the large-death story is out of scope (rate, env). **⚠️ R-PSRB self-modifying-recovery → HAND-BUILD in-process**
(edits the mux-runner worker-spawn lifecycle the pipeline's own workers run); repro-first per the R-WPEX precedent
