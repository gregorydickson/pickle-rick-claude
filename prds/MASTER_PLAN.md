...
## ⏯ RESUME HERE (updated 2026-07-07 — beta.41 R-MPGD WS-1 shipped+deployed; clean hands-off soak rep banked)

**▶ STATE: `origin/main` = v2.0.0-beta.41 (R-MPGD WS-1); tree clean; no active pickle-rick pipeline; deployed runtime = beta.41. R-MPGD WS-1 (`fbde530c`) shipped via a CLEAN hands-off `/pickle-pipeline` (4/4, 56m, zero interventions — the runtime self-recovered a spurious Failed-flip; a banked claude soak rep). WS-2 (setup_aborted labeling) DEFERRED reliability-first — prefer the subtractive root fix [[B-CSOR]] if the honesty gap bites. Prior: the R-SZGB fail-OPEN family is CLOSED (A/B/C/D, beta.39–40). NEXT ACTION (operator-steerable, reliability-first order): options are (a) [[B-CSOR]] — the subtractive root fix (citadel commits its own remediation → no dirty-tree → no abort → R-MPGD's net becomes vestigial); (b) more soak reps toward the GA bar (≥1 codex rep for repeatability); (c) the residual P3 queue ([[R-LTNC]] PRD authored, [[R-TCVC]]/[[R-HNCG]] bug-filed). NOTE for next contexts: `PICKLE_SIGF`, `PICKLE_CITADEL_MECHANICAL`, `PICKLE_RECOVERY_CONSOLIDATION`, `skip_readiness_reason`/`skip_ticket_audit_reason`, chain_meeseeks, `evaluateCodexManagerRelaunch`, and `getPhaseExitReason` (phantom — the real seam is `finalizePhaseSuccess`/`runPhaseIteration`) NO LONGER EXIST / never existed — do not cite them.**
...
**A. Reliability-fix queue — the R-SZGB fail-OPEN family drained through beta.40 (2026-07-05); ONE new open: [[R-MPGD]] (P2, PRD authored).**
The full R-SZGB family is CLOSED (A per-project package-root walk `e284c7ca` → B per-project uncertifiable
fail-CLOSED `cceef8b4` → C attrition-latch `80ec8ab8` → D per-CHECK unrunnable fail-CLOSED `46900269`,
beta.39–40) — the review-phase gate now bites tsc-RED at both per-project and per-check granularity. The
sole open verified reliability bug is **[[R-MPGD]]** (P2, field-surfaced 2026-07-06): the microverse
dirty-tree recovery gates on a naive `.git`-direct-child test, false-negating on monorepo package subdirs
so anatomy-park + szechuan silently no-op and report `completed successfully`. **✅ Mechanism VERIFIED
vs `8f94f620` + blast radius corrected (TWO sites: `preflightAutoCommit:2935` throws-abort +
`autoRescueDirtyTree:3619` silently-skips-rescue). Fix PRD authored**
(`prds/p2-bug-fix-bundle-r-mpgd-microverse-git-detect-false-negative-2026-07-06.md`): WS-1 one shared
`isInsideWorkTree` helper at both sites (net-subtraction, reuses the `git rev-parse --is-inside-work-tree`
signal already at `circuit-breaker.ts:225`) + WS-2 classify a 0-pass setup-abort as `setup_aborted`/skipped
(honesty, not `completed`). Pipeline-safe (NOT R-PSRB). Build on claude → deploy. Prior window (beta.37):
the three 2026-07-02 capture-only bugs
...
**▶ R-MPGD WS-1 SHIPPED beta.41 (2026-07-07). The IMMEDIATE-NEXT is operator-steerable (reliability-first): [[B-CSOR]] subtractive root fix, or bank another soak rep (codex for repeatability).** (Historical note — the original build directive follows.) (the R-SZGB fail-OPEN family is fully drained beta.39–40):
another silent-review-skip class — every monorepo-subdir pipeline (the common case) silently deletes its
two most expensive review phases on a green ledger, so like R-SZGB it makes "hands-off 4/4" soak reps
FALSE-green. Sequence: (1) build R-MPGD on claude (2 WS, pipeline-safe, PRD authored) → `install.sh`
deploy; (2) resume the soak on the FIXED gate: run `/szechuan-sauce` over a known-RED tree / the R-SIGF
diff as LIVE proof the R-SZGB gate now bites (doubles as the R-SIGF hardening pass + a soak rep); (3)
≥1 claude rep with a genuine >600s ticket + ≥1 more codex rep for repeatability. ✅ codex rep #1 already
BANKED CLEAN (B-SCOPESEED 2026-07-02: 4/4 hands-off, 139m) — but it shipped tsc-RED under the then-blind
gate, so it counts for phase-completion, NOT toolchain-green. The GA bar stays N bundles hands-off in a row.
...
2. **Recovery-sprawl functional seam-collapse — the last lever, now MAPPED + RANKED (2026-07-07, 4-agent analysis;
   `RECOVERY-SPRAWL-COLLAPSE-ANALYSIS-2026-07-07.md`).** The analysis re-confirmed B-GSUB's core lesson: the
   ~38-guard manager-loop-continuation cluster is a FALSE target (earned distinct detection signals, already
   sharing termination plumbing) — chasing the guard count would repeat the exact over-subtraction B-GSUB warned
   against. The honest yield is THREE bounded items: **[[B-RASO]]** (P2, the real win — collapse the two
   divergent-strictness attributable-work detectors into one oracle; fixes a latent false-hold bug on an
   unverified sha; R-PSRB hand-build); **[[B-RRPC]]** (P3, pipeline-safe — delete a dead relaunch overload + fold
   a duplicate resolver); **[[B-CSHYG]]** (P3, pipeline-safe — wire-or-delete a dead salvage branch + delete an
   orphaned scanner). The completion seam is already fully collapsed (B-1SEAM). Recommended: ship B-RRPC+B-CSHYG
   as one pipeline-safe subtraction bundle, then B-RASO as the correctness-bearing hand-build.
...
| B-RASO | **B-RASO** — recovery attributable-work single oracle *(the real recovery-sprawl win; correctness-bearing)*. `detectSilentDeathAttributableWork` (`mux-runner.ts:8117` → `hasFrontmatterCompletionSha:8067`, regex-only) and `detectFailedFlipEvidence` (`:8353` → `hasVerifiedFrontmatterCompletionSha:8291`, `git cat-file`-verified + `signal_committed` arm) are two parallel "is there salvageable work?" detectors with DIVERGENT strictness → silent-death can HOLD (suppress respawn) on a garbage/hallucinated sha the failed-flip path rejects (**latent false-hold bug**). Collapse to one `resolveAttributableWorkEvidence` both consume — the B-1SEAM pattern one level down ("is there SALVAGEABLE WORK", the sibling of "is this ticket DONE"). **Also carries B-CSHYG-a** (delete the dead `salvageCleanTree` back-fill branch — same `lib/salvage-ticket.ts` file, one salvage-path session). | P2 | **ANALYSIS DONE 2026-07-07 — ready to scope a PRD. ⚠️ R-PSRB HAND-BUILD** (silent-death/Failed-flip → Done-flip/salvage boundary). | `RECOVERY-SPRAWL-COLLAPSE-ANALYSIS-2026-07-07.md` |
| B-RRPC | **B-RRPC** — recovery resolver-plumbing consistency *(pipeline-safe subtraction)*. (a) DELETE the dead `evaluateSimpleManagerRelaunch` boolean overload (`manager-relaunch.ts:171-195`, zero production callers, test-pinned only); (b) fold the hand-duplicated `resolveBreakerRecoveryGraceSeconds` (`mux-runner.ts:7619`) into `resolveHardeningSettings` (same `hardening:` JSON block — a documented irregularity); (c) move the bare `BOUNDED_ESCAPE_CAP` const (`mux-runner.ts:5897`) onto `hardening.bounded_terminal_escape_cap` for uniformity with its two ledger siblings. Resolver-layer only, does NOT touch the salvage path. **PRD folds B-CSHYG-b (orphaned-scanner verify-before-remove) as WS-4.** | P3 | ✅ **PRD AUTHORED 2026-07-07 — ready to refine/build on claude. Pipeline-safe (NOT R-PSRB).** WS-1 fold `resolveBreakerRecoveryGraceSeconds` → `resolveHardeningSettings`; WS-2 collapse the dead `evaluateManagerRelaunch` boolean overload (verified 0 prod callers); WS-3 settings-ize `BOUNDED_ESCAPE_CAP`; WS-4 verify-before-remove the orphaned `hasCommitReferencingTicketSince` (5 test refs → careful). PRD `p3-subtraction-bundle-b-rrpc-recovery-resolver-plumbing-2026-07-07.md`. | `RECOVERY-SPRAWL-COLLAPSE-ANALYSIS-2026-07-07.md` |
| B-CSHYG | **B-CSHYG** — completion-seam hygiene (the B-1SEAM residual). **SPLIT on source verification (2026-07-07):** (a) dead `salvageCleanTree` `backfillDone` branch (`lib/salvage-ticket.ts:141`, 0 callers pass `backfillDone`) edits the R-PSRB salvage-path file → **folded into [[B-RASO]]** (same file, one hand-build session); (b) orphaned zero-runtime-caller scanner `hasCommitReferencingTicketSince`/`findMatchingCommit` (`pickle-utils.ts:1103-1165`, but 5 TEST refs incl. characterization) → **folded into [[B-RRPC]] WS-4** as a verify-before-remove. | P3 | **SPLIT + REHOMED 2026-07-07** — (a)→B-RASO, (b)→B-RRPC WS-4. No standalone bundle. | `RECOVERY-SPRAWL-COLLAPSE-ANALYSIS-2026-07-07.md` |
