...
|---|------|-----|-------|--------|
| R-APNC | **R-APNC (NEW — 2026-06-27 codex soak)** anatomy-park guard-piling non-convergence on codex. After pickle+citadel, anatomy-park made 27 commits piling one guard per "stale-evidence-replay" variant into `readEvidence` (complexity 13→31, 2× the eslint limit) and never converged (`extension` subsystem 14+ passes, `consecutive_clean`=0, `stall_counts`=0 because committing a guard counts as "progress"). The add-don't-subtract anti-pattern executed by the review phase, complexity-blind. | P2 | **OPEN — NEW.** Fix: anatomy-park needs a convergence/complexity guard (N passes without `consecutive_clean≥1`, or fixes that raise lint-complexity → halt-and-report non-convergent, not grind to cap) + a subtract-pass discipline (collapse the Nth same-theme guard, don't add the N+1th). Stall detection that counts "committed a fix" as progress can't see oscillation. | `p2-bug-fix-bundle-b-apnc-anatomy-park-convergence-guard-2026-06-28.md` · `BUG-REPORT-2026-06-27-codex-worker-gate-not-enforced-and-anatomy-guard-piling.md` |
| R-WUDC | **R-WUDC (NEW — 2026-06-28 B-APNC build)** doc-only deliverable left uncommitted → pickle `pipeline_phase_incomplete` at 0/4. WS-3 (`6803d887`, doc ticket) ran its full lifecycle + produced a correct `.claude/commands/anatomy-park.md` edit, but the work stayed uncommitted + ticket `Todo`; the worker Done-flip was refused (`done_without_commit_evidence`, correct) AND the B-DURA durable-boundary committer did NOT capture it (doc-only diff OUTSIDE `extension/` + clean manager-exit → exit-path committer trigger never armed). New variant of the [[R-WUWC]]/B-DURA uncommitted-work class for doc-only / non-`extension/` deliverables on the clean-exit path. | P2 | **OPEN — NEW.** Fix (reuse-first): extend the durable-boundary committer to commit + Done-flip a ticket with lifecycle artifacts (`conformance_*.md`) + an attributable dirty diff INCLUDING doc-only/non-`extension/` diffs, not just timeout/salvage triggers. Babysitter hand-recovered (commit `080e7e60` + Done-flip + relaunch → pipeline advanced). | `BUG-REPORT-2026-06-28-doc-deliverable-uncommitted-durable-boundary-coverage-gap.md` |
