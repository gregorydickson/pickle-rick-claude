...
|:---|:---|:---|:---|:---|:---|:---|
| 5 | (WS-CGH-D) | Fix INPUT terms — rank identifier-shaped tokens, drop bare line-refs/keywords/>40-char spans, strip call-expr noise; REUSE `resolveSymbolRef`/`countUnresolvedReferences` (`check-readiness.ts:525`) not a new tokenizer. AC verifies the OUTCOME (≥N derived terms resolve to real symbols; `be604d1d` fails-before/passes-after; zero line-refs/keywords/prose) | High | clean tree | derived terms resolve to real symbols; noise-only section not emitted (AC-CGH-D2); telemetry records resolved symbols not `hits_count` (AC-CGH-D3) | `extension/src/bin/spawn-morty.ts` (`deriveCodegraphTerms`), test file(s) |
