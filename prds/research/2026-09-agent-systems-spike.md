...
**Why multi-agent systems fail (MAST, arXiv 2503.13657, NeurIPS 2025) [M].** 1,600+ traces, 7 frameworks: system design 43.9%, inter-agent misalignment 32.4%, task verification 23.8%. Top modes: step repetition (15.7%), reasoning–action mismatch (13.2%), **not knowing when to stop (12.4%)**, incorrect verification (9.1%), missing verification (8.2%). Targeted fixes gave +9.4 to +15.6 points on ChatDev.

## 3. Findings mapped to our questions

### #43: parallel build workers, or finer review partitioning?

**Status (2026-09-26).** `v2.2.0-beta.1` is deployed and running the lane-concurrency experiment: soak bundles run anatomy-park with `anatomy_max_parallel_lanes: 2` (deployed `pipeline-runner.js:124` reads the key, default 1). Results — phase minutes, passes per lane, findings fixed, lane outcomes — land in the "2.2 beta soak ledger" in `prds/MASTER_PLAN.md`, empty at this writing. Verified with `grep '"version"' ~/.claude/pickle-rick/extension/package.json` and `grep -n anatomy_max_parallel_lanes ~/.claude/pickle-rick/extension/bin/pipeline-runner.js`.

**`--teams` is neither parallel nor runnable under the runner as built** [M, this repo, 2026-09-24].
- *Runnable.* The manager runs in print mode (`extension/src/services/backend-spawn.ts`, `args.push('-p', opts.prompt)`, six call sites). From `/tmp`, Claude Code 2.1.281: `claude -p "reply ok" --model claude-sonnet-5 --output-format stream-json --verbose --max-turns 1 2>/dev/null | head -1`, reading the init line's `tools`. **`TeamCreate`, `TeamDelete`, `TaskCreate`, `TaskUpdate`, `TaskList` and `Agent` are absent; `Task` and `SendMessage` present**, also with `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`. Tool counts (33, 25, 33) varied only by late-attaching connector MCP tools. The Teams Mode prompt block calls tools that do not exist in that mode.
...
Median 2.0×, pooled 26 / 14 = **1.86×** (five single-ticket sessions and one hardening-only session excluded). **This differs from the brief** (1.0, 1.0, 3.0, 1.5, 1.5, 3.0, 2.0), likely from parsing multi-line file lists. A whole-field parse gives 1.25, 1.00, 3.00, 1.50, 1.50, 3.00, 2.00; a `completion_commit` cross-check (undercounts multi-commit tickets) gives 2.5, 1.33, 1.5, 3.0, 3.0, 3.0, 2.0. **Every method bounds build parallelism at ≤3×, median ~1.5–2×.** Declared lists are plans, not diffs; hardening stays serial; and the build share caps the gain at 1277 → 1031 min (−19%) on #43's worst bundle.
...
- **Destefanis & Aste (2026-08-17)** [M]. Over 1,902 runs, direct messaging "increases close to quadratically with the number of agents"; a designated coordinator gave no reliable gain.
- **Kim et al. (v3 2026-04-08)** [M]: multi-agent gains run from +80.8% on decomposable tasks to −70.0% on sequential planning; tool-heavy tasks pay extra overhead. **AgenticFlict (v2 2026-05-12)** [M]: 27.67% of 107K simulated agent-PR merges conflict textually — a base rate, not a slope in N.
- **Read-out for lanes.** Every measured curve crossing 2→3 on *coupled* work declines, and CAID's second benchmark peaked at 2. None measures our configuration — **file-disjoint review lanes, no messaging, cherry-pick integration** — and the closest evidence (1/834 on disjoint real PRs) says our N is bounded less by coordination than by shared-file collisions at cherry-pick and rate limits. **2 lanes is the defensible start; go to 3 only on the soak's own conflict and dropped-lane rate.**

**The anatomy-park stall is structurally the C-compiler stall.** `discoverSubsystems` (`extension/src/bin/pipeline-runner.ts:471`) names a subsystem after a top-level directory or workspace package, so all of `extension/` is one lane (on `main`). Anthropic partitioned rather than adding agents [Mv].
...
- **Sun et al. (2025-08): weak.** Across 178 repos, hunk-level AI review comments were addressed 43.9% vs 13.9% file-level; tool and model differ, and "addressed" is not recall [M, confounded].
- **Implication.** Direction supported twice; magnitude does not transfer. If finds per pass are roughly fixed, **passes-to-clean scale with defect count, not partition size**, and finer partitions save wall-clock only through concurrency — what the 2-lane soak measures.

**`--teams` reliability risks** (vendor docs, design facts): experimental; in-process teammates lost on `/resume`; status "can lag"; teammates "stop after encountering errors" — each a halt or stall path.

**Strength.** Moderate-to-strong for "partition before parallelising, keep N small" (CAID, CooperBench, Cursor, C compiler, wave table). Weak for any recall gain from finer units on our workload.
...
- **What nobody claims.** No source measures isolation reducing defects versus trunk commits plus scope fences. The benefit is structural: rejection is free, and a scope violation becomes "not merged". Costs are measured: sequential integration made CAID slower and dearer [M]; conflict-preventing admission serialized nearly everything (Claim Plane) [M].
...
- **Relevance.** The smallest Move 5 makes current ticket *derived* (ticket-trailer commits + frontmatter status), removing the two-sources-of-truth bug. Design only; E4 measures it.

### #5 Moves 1–2: persistent knowledge

- **Evidence.** Self-built memory: 11/12 pairings failed to beat none [M]. Curated, verified experience: +1.1 to +4.5 [M]. Subtask-level memory: +4.7 on SWE-bench Verified [M, single paper]. Vendors persist knowledge via **small curated files loaded every session** (`AGENTS.md`, CLAUDE.md, microagents, playbooks) [D].
...
- **Our unit differs.** We review an accumulated whole-subsystem diff in serial loops; the field reviews one PR with a channel back to the author. SWR-Bench's fixed-finds-per-pass shape predicts our long tails.
- **Two supported cuts:** (a) smaller units run concurrently, at small N; (b) explicit per-unit stop conditions (MAST's third-ranked mode).
- **Harnesses shrank as models improved** (Anthropic dropped sprints; mini-SWE-agent, Agentless). Whether each of our 8 lifecycle phases earns its ~3 min is unmeasured.
...
- **E3b.** Revert real historical fix commits instead of planting mutations. The no-mutation control surfaced findings no mutated run reported, so planted defects crowd out real ones and zero false positives says nothing about precision.
...
**E2. `--teams` — blocked** under the `-p` manager. If unblocked, record min/ticket, early-stopped teammates, stuck statuses, interventions and same-file overlaps. **Success:** < 27 min/ticket **and** zero interventions. Given the wave table and knee evidence, cap `--max-parallel` at 3.

**E4. Derive current ticket from git and replay it.** Over all surviving sessions, compute current ticket from `git log` trailers plus frontmatter status (read-only, outside the runtime) and compare with `state.json.current_ticket` per iteration. **Success:** ≥ 95% agreement, every disagreement traced to a known drift bug. **Falsified if** git lacks needed state (e.g. in-flight tickets with no commit); that names the minimum log to keep.
...
Pages were read through a summarising fetch tool; every exact figure was checked against the primary text except the Anthropic research-system figures (§6). The 2026-09-26 additions come from arXiv abstract pages, the CooperBench full text and the Cursor post.
...
6. **Unmeasured repo claims:** whether each lifecycle phase earns its ~3 min; whether declared file lists match ticket diffs; whether a `-p`-compatible teams path would survive `/resume` and hands-off runs.
