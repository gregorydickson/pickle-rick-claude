...
- [ ] For **every** member `r` of mux-runner's exported terminal/crash-floor reason set, a `pickle` phase that exits non-zero with `state.exit_reason === r` halts the pipeline instead of advancing — Verify: a test that derives the subject list from the exported set at runtime (no hardcoded literals) and asserts halt for each member — Type: test
- [ ] For **every** reason NOT in that set, a non-zero `pickle` exit continues to citadel exactly as it does today (R-PHC-6 continue-by-default is preserved) — Verify: a test deriving the complement at runtime and asserting non-halt for each — Type: test
