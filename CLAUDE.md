...
PRD: `prds/p1-worker-source-state-recursion-contamination.md`. Closer manager-handoff runbook: `docs/closer-ticket-manager-handoff.md` (manager-owned residuals after `closer_handoff_terminal` / `manager_handoff_pending`).
...
| `PICKLE_LARGE_TIER_DETACHED` | `off` | Kill-switch for the B-WPEX-AUTO large-tier detached-worker lifecycle (R-WPEX #108). Default: large-tier workers spawn DETACHED (survive the 600s Bash ceiling), re-attach via the poll loop, orchestrator owns `state.detached_worker`. `off` → legacy synchronous `routeLargeTierTicket` punt. Resolver `largeTierDetachedEnabled` (`mux-runner.ts`). |
...
| `PICKLE_DETACHED_POLL_INTERVAL_MS` | int ms (default 180000) | R-LTDM inter-poll throttle for the large-tier DETACHED poll branch. Each live-worker poll sleeps this long before charging zero-progress, so the loop does not spin and saturate `PICKLE_WMW_SKIP_K` zero-progress polls in <1s (false-failing a just-spawned `tierExceedsBashCeiling` ticket — the beta.31 R-MWBG regression). Detached no-progress wedge window = interval × `WMW_SKIP_K`. Invalid/≤0/fractional → default. Resolver `resolveDetachedPollIntervalMs` (`mux-runner.ts`). |
| `PICKLE_LARGE_TIER_DETACHED_WORKER` | `1` (INTERNAL) | Not operator-facing; set by mux-runner's detached `workerEnv`, consumed by `shouldForceDetachForLargeTier` (`backend-spawn.ts`): `=== '1'` keeps the large-tier claude grandchild in spawn-morty's own session-scoped group (`detached:false`, inherits not leads) so one `process.kill(-spawn_morty_pid)` reaps the subtree (H2 aee2767b). Reads `backend-spawn.ts`, `spawn-morty.ts`, `mux-runner.ts`. |
