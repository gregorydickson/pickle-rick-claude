...
4. **Anatomy-park no-clean-pass ceiling (per-machine)**: `export PICKLE_APNC_MAX_PASSES_WITHOUT_CLEAN=<int>` (default 50). Raise it further when a large bundle's review is finding REAL defects every pass and you want the loop to keep draining rather than report `anatomy_non_convergent` at 50. It does not touch `anatomy_max_iterations` (100), which remains the actual loop ceiling. **A raise cannot be applied retroactively** — the value is read when the phase runs, so a run already past anatomy-park is unaffected. Because `launch.sh` is regenerated per session and exports nothing, the export must live in the shell that launches the pipeline (profile, or inline on the launch command); a non-login tool shell will NOT pick up `~/.zshrc`.
