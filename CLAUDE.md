...
7. **A clean git log on a core predicate is a RISK signal.** Incidents pool at a file's EDGES, so the
   centre looks maintained and goes unsuspected. **Hunt with `git log -L :fn:file`** — a predicate whose
   body changed only at its feature landing, inside a heavily-cited file, is the candidate. **`git log -S`
   is the WRONG tool here and will mislead you: it counts declaration churn, returning 1 for almost every
   function however often the body changed.** Subtraction prevents the next defect; only this finds the
   ones already there.
...
| Any path inside a repo's `.git/`, or a directory enclosing it | `allow_git_dir_write_reason` | `config-protection.ts` hook (R-WSRC-GR path axis) |
...
`npx tsc --noEmit && npx eslint src/ --max-warnings=0 && npx tsc && bash scripts/audit-test-tiers.sh && bash scripts/audit-test-isolation.sh && bash scripts/audit-subprocess-heavy-tests.sh && bash scripts/audit-fix-commits.sh && bash scripts/audit-bundle-thesis.sh && bash scripts/audit-quarantine.sh && bash scripts/audit-trap-door-enforcement.sh && bash scripts/audit-guarded-reset.sh && bash scripts/audit-un-terminalize-single-path.sh && bash scripts/audit-did-we-count.sh && bash scripts/audit-recorded-ceilings.sh && bash scripts/audit-ledger-probes.sh && bash scripts/audit-acceptance-assertion-coverage.sh && npm run test:fast:budget && npm run test:integration && npm run test:contract && RUN_EXPENSIVE_TESTS=1 npm run test:expensive`
...
| `hardening` | `object` | see notes | Runtime-recovery block (DISTINCT from `bmad_hardening`), all four fields resolved by the single `resolveHardeningSettings` in `services/pickle-settings.ts`. `silent_death_respawn_cap` 1 (0 disables), `failed_flip_suppression_cap` 2 (0 disables) — non-negative ints drawing down the persistent `state.recovery_attempts` ledger (survives relaunch/`--resume`). `breaker_recovery_grace_seconds` 30: grace window where a breaker-recovery spawn doesn't count as progress. `bounded_terminal_escape_cap` 3 (AC-A4): consecutive no-progress relaunches on the same In Progress ticket before the bounded escape forces it terminal. |
| `rate_limit` | `object` | see notes | B-RRH rate-limit park (`resolveRateLimitSettings`, `pickle-settings.ts`). `max_park_minutes` 360 (int floor 1) caps cumulative parked wall-clock per episode before `rate_limit_park_exhausted`. Absent/malformed → compiled default. |
| `default_refinement_model` | `string \| null` | `null` | Model for `spawn-refinement-team.js` analyst spawns. Absent/`null` = inherit the session model. `--model <id>` on `/pickle-refine-prd` or `/portal-gun` overrides it per run — use it as the recovery route when the default model refuses analysts. |
...
| `PICKLE_RATE_LIMIT_PROBE_INTERVAL_MS` | int ms ≥60000 (default 600000) | How often a rate-limit park re-probes the API before its scheduled resume. ONE resolver serves both waiting runners (`runMainLoopRateLimitPark`, `mux-runner.ts`; `runRateLimitWaitLoop`, `microverse-runner.ts`), so the knob cannot mean two things. Below floor clamps UP to 60000 — an override cannot turn a park into a spawn-burn; absent/blank/non-numeric/zero/negative → default. Fractional or trailing-garbage values are TRUNCATED by `parseInt`, deliberately looser than the sibling `PICKLE_TIER_STALL_THRESHOLD_MS` which rejects them. The first probe fires one full interval IN, not immediately: the park began with a rejection, so re-asking at once spends a spawn to learn what is already known. Each probe is a 120s-capped judge round trip logged to `<session>/rate_limit_probe.log`; an unreadable verdict continues the wait rather than resuming. Resolver `resolveRateLimitProbeIntervalMs` (`services/pickle-settings.ts`). Env-only — no `pickle_settings.json` arm (the `rate_limit` block governs park DURATION via `max_park_minutes`, not probe cadence) — and NOT in `PICKLE_GATE_SCRUBBED_ENV_KEYS`, so it inherits normally. |
